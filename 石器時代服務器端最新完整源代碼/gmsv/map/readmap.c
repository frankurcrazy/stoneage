...
#endif
{
    ...
    while( fgets( line , sizeof( line ) , file ) ){
		...
		if( line[0] == '$' ){
			...
			if( iRet < 1 ){
				print( "!!!!!錯誤 無法讀取戰鬥地圖(%s)( line %d )\n", filename, linenum );
			}
			for( i = 0; i < iRet; i ++ ){
				BattleMapNo[i] = iPre[i];
				if( BattleMapNo[i] < 0 ){
					print( "!!!!!錯誤 戰鬥地圖號碼為負值 (%s)( line %d )\n", filename, linenum );
				}
			}
...
static int MAP_idjumptbl[65535];
...
BOOL MAP_readMapOne( char*   filename )
{
    ...
    short   data[1024];         /*  撲亦□玄  心迕田永白央  */
    int     ret;                /*  忒曰襖熬仃潸曰迕        */
    int     i;                  /*  夥□皿  醒  */
    int     mapindex;

    int     id=0,xsiz=0,ysiz=0;     /*  穴永皿犯□正及域凜傘  樺赭  */

    unsigned short   *tile=NULL;
    unsigned short   *obj=NULL;
    MAP_Objlink**   olink=NULL;
    char    showstring[32];         /*  穴永皿犯□正及域凜傘  樺赭  */
    struct  stat    filestat;       /*  民尼永弁迕卞銀丹    */
    BOOL    invaliddata=FALSE;

    if( MAP_mapnum_index >= MAP_mapnum ){
        fprint( "這裏沒有足夠空間裝載地圖數組.\n" );
        return FALSE;
    }
...
*/
    ...
    if( tile == NULL ){
        fprint( "無法分配內存給地圖名稱:%s xsiz:%d ysiz:%d\n", filename, xsiz, ysiz);
        goto FREEOBJHP;
    }
...
//        tile[i] = ntohs( tile[i] );
    for( i = 0 ; i < xsiz * ysiz ; i ++ ){
        ...
        if( !IsValidImagenumber( tile[i] ) ){
            fprint("地圖的圖片有問題:%d x:%d y:%d 數量:%d\n",
                id, i % xsiz, (int)(i / xsiz) , tile[i]);
            invaliddata = TRUE;
        			}
        /*
        else{
            fprint("地圖的圖片為:%d x:%d y:%d 數量:%d\n",
                id, i % xsiz, (int)(i / xsiz) , tile[i]);
...
/*    if( ftell(f) != filestat.st_size)
        fprintf(stderr,"文件大小不正確(%s). 目標文件大小:%"
#ifdef linux
                "l"
#elif __FreeBSD__
                "ll"
...
FCLOSERETURNTRUE:
		...
		if( MAP_idtblsize <= MAP_map[mapindex].id ){
			MAP_idtblsize = MAP_map[mapindex].id + 1;
		}
...
//#define MAX_MAP_FILES 1300 // 地圖目錄最大檔案數
...
//            print(".");
    ...
    print( "正確地圖文件 %d...",MAP_mapnum_index );
...
int MAP_intPlayerMap( void )
{
	...
	while(NULL != (ent=readdir(pDir)))
	{
		...
		if (ent->d_type==8){
			...
			sprintf( filename, "playermap/%s",  ent->d_name);
			FILE *fp   =   fopen(filename,   "rb");
	    if (fp==NULL) continue;
	    fseek( fp, 0L, SEEK_END );
	    if(ftell(fp)==Player_Diy_Map_SIZE){
	    	fseek( fp, 0L, SEEK_SET );
				fread(map, sizeof(map), 1,fp);
				memcpy( &PlayerDiyMap[i] , &map, sizeof(map));
				int x,y;
				int floorindex = MAP_getfloorIndex( PlayerDiyMap[i].ff );
				int xsiz = MAP_map[floorindex].xsiz;
				int ysiz = MAP_map[floorindex].ysiz;
				if(strlen(PlayerDiyMap[i].mapname)>0){
					strcpy(MAP_map[floorindex].string, PlayerDiyMap[i].mapname);
				}
				for(x=0;x<Player_Diy_Map_X && x < xsiz;x++){
					for(y=0;y<Player_Diy_Map_Y && y < ysiz;y++){
						if(PlayerDiyMap[i].MapPic[x][y].tile>0){
							MAP_map[floorindex].tile[y*xsiz+x] = PlayerDiyMap[i].MapPic[x][y].tile;
						}
						if(PlayerDiyMap[i].MapPic[x][y].obj>0){
							MAP_map[floorindex].obj[y*xsiz+x] = PlayerDiyMap[i].MapPic[x][y].obj;
						}
					}
				}
				i++;
				if(i>=Player_Diy_Map_NUM)break;
	  	}
			fclose(fp);
		}
	}
...
int MAP_savePlayerMap( int charaindex, int ff ,int fx, int fy, int tile, int obj)
{
  ...
  xsiz = MAP_map[floorindex].xsiz;
  if( fx < 0 || fx >= xsiz || fx >= Player_Diy_Map_X ) {
        return FALSE;
	}
	ysiz = MAP_map[floorindex].ysiz;
  if( fy < 0 || fy >= ysiz || fy >= Player_Diy_Map_Y ) {
        return FALSE;
	}
  ...
  for(i=0;i<Player_Diy_Map_NUM;i++){
  	if(PlayerDiyMap[i].ff==ff){
  		if(strcmp(CHAR_getChar(charaindex, CHAR_CDKEY), PlayerDiyMap[i].cdkey )!=0){
  			CHAR_talkToCli( charaindex, -1, "該地圖不屬於你的，你無法在此DIY...",  CHAR_COLORRED);
  		}
  		if( tile != -1 ){
	  		MAP_map[floorindex].tile[fy*xsiz+fx] = tile;
...
BOOL MAP_SetExWarp(int mapid , int fl, int x, int y, int type)
{
	...
	if (tomapindex == -1)
	{
		print( "找不到目標地圖，請確認地圖ID是否正確！\n");
		return FALSE;
	}

	MAP_map[tomapindex].startpoint = (fl<<16)+(x << 8)+(y<<0);
	MAP_map[tomapindex].MapType = type;//地圖類型？ 目前不清楚乾嘛的 保留修改的權利
	return TRUE;
}
...
int MAP_makenew( int mapid ,char* map_name)
{
  ...
  if ((tomapindex = MAP_getfloorIndex(mapid)) == -1)
   ...
   int mapstartid = getCopymapstartingID();//獲得副本地圖起始ID
    for (j=mapstartid;j<mapstartid+9999;j++)//分配一個地圖ID
    {
    for(i=0;i<MAP_mapnum_index;i++)//遍曆所有地圖號ID
    	{
    		if (MAP_map[i].id == j) break;//如果地圖ID以被使用
    	}
     if (i == MAP_mapnum_index) {//沒被使用過的地圖ID
     	mymapid = j;
     	break;
      }
    }
    ...
    for(i=0;i<MAP_mapnum_index;i++)//先找一個空間，是否有被釋放，如果有，責不需要重新開闢空間了。
    	{
    		...
    		makemapindex = i;
    if( makemapindex >= MAP_mapnum ){//沒有足夠的空間裝載新的地圖，開闢10個新的空間來裝新地圖。
//    print( "沒有足夠的內存，重新分配內存。\n");
    MAP_Map* MAP_map2;//用來保存舊數據
    MAP_map2 = allocateMemory( sizeof( MAP_Map ) * MAP_mapnum);
    if( MAP_map2 == 0 ){
    	print( "緩存地圖失敗！\n");
    	return -1;
    }
	memcpy( MAP_map2 , MAP_map ,sizeof(MAP_Map) * MAP_mapnum);//保存之前的數據
    freeMemory( MAP_map );//釋放掉舊資料
    MAP_map = 0;
    MAP_mapnum += 100;
    MAP_map = allocateMemory( sizeof( MAP_Map ) * MAP_mapnum);//重新給地圖分配內存
    if( MAP_map == 0 ){
    	print( "重新分配地圖內存失敗！\n");
    	return -1;
    }
//    	print( "重新分配地圖內存成功！\n");
	memcpy( MAP_map , MAP_map2 ,sizeof(MAP_Map) * (MAP_mapnum-100));//保存之前的數據
    freeMemory( MAP_map2 );
    MAP_map2 = 0;
    }
    short   *tile=0;
    ...
    tile = allocateMemory( sizeof( short ) * MAP_map[tomapindex].xsiz * MAP_map[tomapindex].ysiz );//分配地圖內存
    if( tile == NULL ){
        print( "無法分配內存給地圖圖層\n");
        return -1;
    }

    obj = allocateMemory( sizeof( short ) * MAP_map[tomapindex].xsiz * MAP_map[tomapindex].ysiz );//分配地圖對象內存
    if( obj == NULL ){
         print( "無法分配內存給地圖對象\n");
        freeMemory( tile );
        return -1;
    ...
    olink = allocateMemory( sizeof(MAP_Objlink*) * MAP_map[tomapindex].xsiz * MAP_map[tomapindex].ysiz );//分配地圖鏈接內存
    if( olink == 0 ){
        print( "無法分配內存給地圖鏈接\n");
        freeMemory( obj );
        return -1;
    ...
    MAP_map[makemapindex].olink = olink;
		MAP_map[makemapindex].startpoint = (2006<<16)+(20 << 8)+(15<<0);//默認為傳送迴漁村村長傢
