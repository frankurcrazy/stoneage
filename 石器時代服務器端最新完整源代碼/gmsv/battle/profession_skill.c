...
// 勇士技能
	...
	{ "PROFESSION_REBACK",			PROFESSION_reback,			0 },	// 狀態迴復
	...
	{ "PROFESSION_CONVOLUTE",		PROFESSION_convolute,		0 },	// 迴鏇攻擊
...
// 獵人
	...
	{ "PROFESSION_PLUNDER",			PROFESSION_plunder,			0 },	// 鋪迓傭
...
int PROFESSION_SKILL_Use( int charaindex, int skill, int toNo, char *data )
{
	...
	if(getDisableProfessionSkill(CHAR_getInt( charaindex, CHAR_FLOOR ))==TRUE){
		CHAR_talkToCli( charaindex, -1, "該地圖已禁止使用職業技能!", CHAR_COLORYELLOW);
		return -1;
	}
#endif
...
//	print("\n技能熟練度上升 ==> 當前熟練度點數(%d) 下一級熟練度升級點數(%d)", skill_level_sum, next_level_need_point );
...
//		print("\n職業等級上升 ==> 當前職業等級點數(%d) 下一級職業等級升級點數(%d)", old_skill_point, next_skill_point );
