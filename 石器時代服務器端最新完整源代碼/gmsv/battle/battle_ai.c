...
int BATTLE_ai_all( int battleindex, int side, int turn)
{
	...
	for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
		...
		if( mode < 0 || mode >= arraysizeof( functbl)){
			print( "BATTLE_ai_all 中,戰鬥邏輯模式很奇怪(%s)(%d)\n",
...
#define B_AI_NORMAL_ATTACKOPTION	"at"	/*   猾左皿撲亦件 */
#define B_AI_NORMAL_GUARDOPTION		"gu"	/*   豢左皿撲亦件 */
#define B_AI_NORMAL_MAGICOPTION		"ma"	/* 熱諸左皿撲亦件 */
#define B_AI_NORMAL_ESCAPEOPTION	"es"	/*   僕月左皿撲亦件 */
#define B_AI_NORMAL_WAZAOPTION		"wa"	/*   左皿撲亦件 */
...
static int BATTLE_ai_normal( int turn, int charaindex,
							...
							struct B_AI_RESULT *result)
{
	int		at[B_AI_NORMAL_ATTACKSUBOPTIONNUM] = { 0,0,0}; 		/*   猾左皿撲亦件 */
	int		gu[B_AI_NORMAL_GUARDSUBOPTIONNUM] = {0}; 		/* 左皿撲亦件 */
	int		ma[B_AI_NORMAL_MAGICSUBOPTIONNUM] = {0}; 		/* 左皿撲亦件 */
	int		es[B_AI_NORMAL_ESCAPESUBOPTIONNUM] = {0}; 			/* 左皿撲亦件 */
	int		wa[B_AI_NORMAL_WAZASUBOPTIONNUM] = {0,0,0,0, 0,0,0};/* 左皿撲亦件 */
#ifdef _ENEMY_ATTACK_AI
