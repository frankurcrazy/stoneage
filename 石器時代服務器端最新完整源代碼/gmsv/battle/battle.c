...
{
	if(battleindex>=BATTLE_battlenum || battleindex<0 )return FALSE;
...
void BATTLE_BadStatusAllClr( int charaindex )
{
	...
	CHAR_setWorkInt( charaindex, CHAR_WORKPASSIVE_DUCK, 0);
	CHAR_setWorkInt( charaindex, CHAR_WORKPASSIVE_ACURATE, 0 );
	CHAR_setWorkInt( charaindex, CHAR_WORKPASSIVE_CRITICAL, 0);
	CHAR_setWorkInt( charaindex, CHAR_WORKPASSIVE_COUNTER, 0 );
	CHAR_setWorkInt( charaindex, CHAR_WORKPASSIVE_MULTIPLE, 0);
#endif
...
)
{
  ...
  for( i = 0 ; i < BATTLE_battlenum ; i ++ ){
	...
	print("分配 %4.2f MB 空間...", ((float)sizeof( Char )*BATTLE_battlenum)/1024.0/1024.0);
  return TRUE;
...
#ifdef _PROFESSION_ADDSKILL
BOOL BATTLE_BattleUltimate( int battleindex, int bid ) //檢查此位D上是否被打飛
{
	BATTLE_ENTRY *pEntry;
	...
	if( pEntry[bid].flg & BENT_FLG_ULTIMATE ) return FALSE;
	else return TRUE;
}
#endif
...
INLINE int _BATTLE_Exit( char *file, int line, int charaindex ,int battleindex)
{
		...
		for( i = 0 ; i < BATTLE_ENTRY_MAX ; i ++ ){
			...
			if( CHAR_CHECKINDEX( winindex ) == TRUE ){
				if( CHAR_getInt( winindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
					BattleFinishPvEFunction(battleindex, charaindex);
				}
			}
		}
	}
	...
	if( CHAR_getInt( charaindex, CHAR_BASEIMAGENUMBER) == 101749
	  ...
	  CHAR_setWorkInt( charaindex, CHAR_WORKFOXROUND, -1);
		//print("\n變迴去:%d",CHAR_getInt( charaindex, CHAR_BASEIMAGENUMBER));
	}
#endif
	...
	if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
		...
		CHAR_setWorkInt( charaindex, CHAR_WORKNOCAST, 0);
	}
#endif	
	pBattle = &BattleArray[battleindex];
	...
	if( CHAR_CHECKINDEX( CHAR_getCharPet( charaindex, 4) ) ){
		CHAR_sendPetItemData(charaindex,4);
				...
				if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
					...
					if( CHAR_getFlg( charaindex, CHAR_ISTALKIP)) flg |= CHAR_FS_TALKIP;
#endif
#endif
#endif
					if( CHAR_getFlg( charaindex, CHAR_AI_MOD)) flg |= CHAR_FS_AI;
			...
			if(BattleArray[battleindex].type != BATTLE_TYPE_WATCH){
...
#endif
#ifdef _VIP_ONLINE
					|| CHAR_getInt(charaindex, CHAR_VIPRIDE) > 0
#endif
#ifdef _FMPOINT_WELFARE
					|| (CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) == 1041 && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
					|| (CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) == 2031 && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
					|| (CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) == 3031 && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
					|| (CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) == 4031 && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( charaindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
#endif
					)
				{
					char petsend[3];
			    ...
			    CHAR_setFlg( charaindex, CHAR_ISDIE, 0 );
					for(i = 0 ; i < CHAR_MAXPETHAVE; i ++) {
						...
						CHAR_sendStatusString( charaindex , petsend );
...
BOOL BATTLE_RescueTry( int charaindex)
{
	...
	fd = getfdFromCharaIndex( charaindex);
	if( CONNECT_checkfd( fd ) == FALSE ) return FALSE;
#if 1	// 修正利用參戰重復加入戰鬥	Robin
	if( CHAR_getWorkInt( charaindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE) {
		CHAR_talkToCli( charaindex, -1, " 重復加入戰鬥! ", CHAR_COLORRED);
		print("\n 改封包!!重復加入戰鬥!:%s ", CHAR_getChar( charaindex, CHAR_CDKEY) );
  ...
  if( CHAR_getWorkInt( charaindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE ) {
	...
	if( cnt == 0 ) {
	    if( pfound == FALSE ) {
	        CHAR_talkToCli( charaindex, -1, "那裏沒有任何人。",
                            CHAR_COLORYELLOW);
	    }
		lssproto_EN_send( fd, FALSE, 0 );
	}else if( cnt == 1 ) {
...
)
{
	...
	BATTLE_ClearGetExp( charaindex );
		...
		if(BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P){
			if(CHAR_getInt( charaindex, CHAR_FLOOR ) == getBattleGetItemRateMap()){
	...
	for( i = 1; i < getPartyNum(charaindex); i ++ ){
		...
		if( CHAR_getWorkInt( work, CHAR_WORKBATTLEMODE ) != BATTLE_CHARMODE_NONE &&  CHAR_getWorkInt( work, CHAR_WORKBATTLEMODE ) != BATTLE_CHARMODE_FINAL){
			continue;
		}
...
static int BattleERSkill3[]={
13, 31, 41, 52, 60, 61, 80, 90, 100, 110, 120, 152, 210, 325, 500, 501, 504, 503, 504, 505, 506, 507, 508, 516, 610};

static int BattleERSkill4[]={
609,610,611};
...
int BATTLE_CreateVsEnemy( int charaindex, int mode, int npcindex )
{
	...
	if( CHAR_getWorkInt(charaindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_CLIENT) return BATTLE_ERR_END;
	...
	if( battleindex < 0 )return BATTLE_ERR_NOTASK;
		...
		if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ) {
			if( CHAR_getWorkInt( charaindex, CHAR_WORKBATTLEMODE ) == BATTLE_CHARMODE_NONE ){
				if(CharVsEnemyFunction( charaindex ) == TRUE)return 0;
			}
		}
#endif
...
#ifdef _BATTLE_OVER_STAR_TIME
	if( CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 120 > time(NULL)){
		char token[256];
		sprintf(token, "你當前需冷卻%d後纔能進行PK", CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 120 - time(NULL));
		CHAR_talkToCli( charaindex0, -1, token, CHAR_COLORYELLOW );
		return BATTLE_ERR_ALREADYBATTLE;
	}
	if( CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 120 > time(NULL)){
		char token[256];
		sprintf(token, "對方需冷卻%d後纔能進行PK", CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 120 - time(NULL));
		CHAR_talkToCli( charaindex0, -1, token, CHAR_COLORYELLOW );
		sprintf(token, "你當前需冷卻%d後纔能進行PK", CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 120 - time(NULL));
		CHAR_talkToCli( charaindex1, -1, token, CHAR_COLORYELLOW );
		return BATTLE_ERR_ALREADYBATTLE;
	}
#endif
...
#ifdef _TRADE_PK
	memcpy( &BattleArray[battleindex].TradeList[0], &TradeList1, sizeof( STradeList) );
	memcpy( &BattleArray[battleindex].TradeList[1], &TradeList2, sizeof( STradeList) );
#endif
...
static BOOL BATTLE_CommandWait( int battleindex, int side)
{
	...
	BOOL iRet = TRUE;
	BOOL TimeOut = FALSE;
	if( BATTLE_CHECKSIDE( side ) == FALSE )return TRUE;//檢查值是否在閤法  圍
...
#ifdef _BATTLE_BOUT_TIME
			if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
				if(CHAR_getWorkInt(charaindex, CHAR_BATTLE_BOUT_TIME) > (int)time(NULL) - 1){
					if(CHAR_getInt(charaindex, CHAR_MP)>0){
						CHAR_talkToCli( charaindex, -1, "係統監測到您正在快速戰鬥，把您的MP自動清0", CHAR_COLORYELLOW);
...
#endif
{
...
#endif
	if( getexp < 0 || CHAR_GetLevelExp( charaindex, CHAR_getInt( charaindex, CHAR_LV)+1)==-1){
#ifdef _ALLBLUES_LUA_1_9
...
#ifdef _ALLBLUES_LUA_1_9
	addexp = FreeModeExp(charaindex, getexp, modexp);
#else
	...
	if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER){
		...
		badindex = CHAR_getWorkInt(charaindex, CHAR_WORKPLAYERINDEX);
	}
	if( CHAR_CHECKINDEX( badindex ) == TRUE ){
#ifdef _VIP_BATTLE_EXP
		if( CHAR_getInt(badindex, CHAR_VIPTIME) > 0 
#ifdef _FMPOINT_WELFARE
			|| (CHAR_getWorkInt( badindex, CHAR_WORKFMFLOOR) == 1041 && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
			|| (CHAR_getWorkInt( badindex, CHAR_WORKFMFLOOR) == 2031 && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
			|| (CHAR_getWorkInt( badindex, CHAR_WORKFMFLOOR) == 3031 && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
			|| (CHAR_getWorkInt( badindex, CHAR_WORKFMFLOOR) == 4031 && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_NONE && CHAR_getInt( badindex, CHAR_FMLEADERFLAG ) != FMMEMBER_APPLY)
#endif
#ifdef _PLAYER_TITLE
        ...
        for( i = 0 ; i < CHAR_EQUIPPLACENUM ; i ++ ){
          ...
          arg = ITEM_getChar(id, ITEM_ARGUMENT );//道具參數
			    if( (P = strstr( arg, "EXPUP" )) ){
					if( (P = strstr( arg, "人" )) ){//隻對人有效
					    if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER )
						    addexp += getexp*atoi(P+=2)*0.01*getBattleexp();
					}
					else if( (P = strstr( arg, "寵" )) ){//隻對寵有效
                        if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET 
							&& (CHAR_getInt( midx, CHAR_RIDEPET) != charaindex ) ) 
						    addexp += getexp*atoi(P+=2)*0.01*getBattleexp();
					}
					else if( (P = strstr( arg, "騎" )) ){//隻對騎寵有效
						if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET 
							&& (CHAR_getInt( midx, CHAR_RIDEPET) == charaindex ) ) 
					        addexp += getexp*atoi(P+=2)*0.01*getBattleexp();
					}
	...
	CHAR_setWorkInt( charaindex, CHAR_WORKGETEXP, addexp );//迴存CHAR_WORKGETEXP
...
)
{
	...
	if(floor == 11111 || floor == 22222 || floor == 33333 || floor == 44444){
		...
		if( floor == 20000 && AutoPk_PKTimeGet() <= 0){
				if( dpadd == 0 ){
					CHAR_talkToCli(charaindex, -1, "戰績不理想，未加減竟賽積分！", CHAR_COLORRED);
				}else	if( dpadd < 0 ){
					int fl = 0, x = 0, y = 0;
					if(CHAR_CHECKINDEX(charaindex) != FALSE){
...
//					  saacproto_FormulateAutoPk_send(acfd, CHAR_getChar(charaindex, CHAR_CDKEY), CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK));
						CHAR_setInt( charaindex , CHAR_AMPOINT, CHAR_getInt( charaindex , CHAR_AMPOINT)+CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK));
#ifdef _AMPOINT_LOG
						LogAmPoint(CHAR_getChar( charaindex, CHAR_NAME ),
									         	CHAR_getChar( charaindex, CHAR_CDKEY ),
									          CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK),
									   				CHAR_getInt( charaindex, CHAR_AMPOINT ),
									         	"(自定義PK)",
									         	CHAR_getInt( charaindex,CHAR_FLOOR),
									         	CHAR_getInt( charaindex,CHAR_X ),
									         	CHAR_getInt( charaindex,CHAR_Y ));
#endif						
						CHAR_setInt( charaindex , CHAR_WORK_AUTOPK, 0);
#endif
					  CHAR_DischargeParty( charaindex, 0);
					  CHAR_talkToCli(charaindex, -1, "你已戰敗，係統自動把你送迴記錄點", CHAR_COLORYELLOW);
					  ...
					  for(i=0;i<playernum;i++){
						...
						if(num==1){
							int fl = 0, x = 0, y = 0;
							CHAR_getElderPosition(CHAR_getInt(lastindex, CHAR_LASTTALKELDER), &fl, &x, &y);
					  	CHAR_warpToSpecificPoint(lastindex, fl, x, y);
							AutoPk_ChampionShipSet( lastindex, CHAR_getWorkInt(lastindex,CHAR_WORK_AUTOPK),1);
...
#ifdef _FORMULATE_AUTO_PK
					CHAR_setWorkInt(charaindex,CHAR_WORK_AUTOPK,CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK) + getKillPoint());
#else
					CHAR_setWorkInt(charaindex,CHAR_WORK_AUTOPK,CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK) + 1);
#endif
					AutoPk_ChampionShipSet(charaindex, CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK), 0);
				}
		}else
#endif
		{
		...
		for( i = 0; i < GETITEM_MAX; i ++ ){
			...
			if( itemindex < 0 ){
				...
				if(floor == getBattleGetItemRateMap()){
					int rate = rand() % 10000;
					if(rate<2){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(0) );
					}else if(rate<5){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(1) );
					}else if(rate<10){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(2) );
					}else if(rate<18){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(3) );
					}else if(rate<28){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(4) );
					}else if(rate<40){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(5) );
					}else if(rate<65){
						itemindex = ITEM_makeItemAndRegist( getBattleGetItemRate(6) );
					}
				}
				int i;
				int playernum = CHAR_getPlayerMaxNum();
			...
			if( itemindex < 0 ){
				...
				if( 0 <= ret && ret < CHAR_EQUIPPLACENUM ){
					print( "itemget err: 在這裏不能取得item(%s)(%d)(%s)\n",
						CHAR_getUseName( charaindex ),
						ret,
...
static int BATTLE_Finish( int battleindex )
{
	...
	if(CHAR_CHECKINDEX(BattleArray[battleindex].rivalindex) && 
		...
		if(CHAR_getWorkInt( BattleArray[battleindex].rivalindex, CHAR_WORK_BATTLEPK)==TRUE ||
						CHAR_getWorkInt( BattleArray[battleindex].leaderindex, CHAR_WORK_BATTLEPK)==TRUE){
	...
	if(CHAR_CHECKINDEX(BattleArray[battleindex].TradeList[0].charaindex) && 
		...
		if(CHAR_getInt(BattleArray[battleindex].TradeList[0].charaindex, CHAR_FLOOR)==50000 &&
				CHAR_getInt(BattleArray[battleindex].TradeList[1].charaindex, CHAR_FLOOR)==50000){
					if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
						if( BattleArray[battleindex].winside == -1 ){
							TRADE_HandleTradeForPK( &BattleArray[battleindex].TradeList[0], &BattleArray[battleindex].TradeList[1] );
						}else if(BattleArray[battleindex].winside == 1){
							TRADE_HandleTradeForPK( &BattleArray[battleindex].TradeList[1], &BattleArray[battleindex].TradeList[0] );
						}
					}
		}
	}
	...
	if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
		...
		for( i = 0 ; i < BATTLE_ENTRY_MAX ; i ++ ){
			...
			if( CHAR_CHECKINDEX( winindex ) == TRUE ){
				...
				CHAR_setInt( winindex, CHAR_DUELSTWINCOUNT, CHAR_getInt( winindex, CHAR_DUELSTWINCOUNT ) + 1);
				
#ifdef _ALLBLUES_LUA_1_9
				BattleFinishFunction(winindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 1);
#endif
				
				CHAR_setInt( winindex, CHAR_DUELBATTLECOUNT, CHAR_getInt( winindex, CHAR_DUELBATTLECOUNT ) + 1);
				CHAR_setInt( winindex, CHAR_DUELWINCOUNT, CHAR_getInt( winindex, CHAR_DUELWINCOUNT ) + 1);
				
				CHAR_setInt( winindex, CHAR_DUELSTLOSECOUNT, 0);
				if(CHAR_getInt( winindex, CHAR_DUELSTWINCOUNT) > CHAR_getInt( winindex, CHAR_DUELMAXSTWINCOUNT)){
					CHAR_setInt( winindex, CHAR_DUELMAXSTWINCOUNT, CHAR_getInt( winindex, CHAR_DUELSTWINCOUNT ));
				}
			}
			...
			if( CHAR_CHECKINDEX( lostindex ) == TRUE ){
				...
				CHAR_setInt( lostindex, CHAR_DUELSTLOSECOUNT, CHAR_getInt( lostindex, CHAR_DUELSTLOSECOUNT ) + 1);
				
#ifdef _ALLBLUES_LUA_1_9
				BattleFinishFunction(lostindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 0);
#endif
				CHAR_setInt( lostindex, CHAR_DUELBATTLECOUNT, CHAR_getInt( lostindex, CHAR_DUELBATTLECOUNT ) + 1);
				CHAR_setInt( lostindex, CHAR_DUELLOSECOUNT, CHAR_getInt( lostindex, CHAR_DUELLOSECOUNT ) + 1);
...
#ifdef _BATTLE_BOUT_TIME
					if(CHAR_getInt( charaindex, CHAR_FLOOR) == 40001
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40002
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40003
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40004
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40005
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40006
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40007
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40008
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40009
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 40010
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 50001
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 50002
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 50003
						|| CHAR_getInt( charaindex, CHAR_FLOOR) == 50004
					){
						CHAR_setWorkInt( charaindex, CHAR_BATTLE_BOUT_TIME, (int)time(NULL));
					}else{
						CHAR_setWorkInt( charaindex, CHAR_BATTLE_BOUT_TIME, 0);
					}
#endif
				}
			}
...
int BATTLE_Loop( void )
{
	...
	for( i = 0; i < BATTLE_battlenum; i ++ ){//最多戰鬥場數
		...
		if( !BATTLE_CHECKINDEX( i ) )continue;
#ifdef _BATTLE_TIME
		struct timeval old;
		old.tv_sec  = BattleArray[i].tv_sec;
		old.tv_usec = BattleArray[i].tv_usec;
		if( getBattleTime() > 0 && time_diff_us(NowTime,old) < getBattleTime() * 1000.0 )continue;
 ...
 void BATTLE_PassiveSkill(int charaindex){
	...
	for( i = 0; i < CHAR_MAXPETSKILLHAVE; i ++){
		...
		if( skill_type == PETSKILL_FIELD_MAP){
			PETSKILL_Use( charaindex, i, NULL, NULL ) ;
		}
		else{
			continue;
		}
...
//重新計算被動加成
if (CHAR_getInt(charaindex,CHAR_WHICHTYPE)==CHAR_TYPEPET){
		BATTLE_PassiveSkill(charaindex);
}
#endif
	...
	case BATTLE_COM_ITEM:	// 失奶  丞毛銀丹樺寜
		work = CHAR_getWorkInt( charaindex, CHAR_WORKQUICK )+20;
...
)
{
	...
	if( CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER 
		...
		if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
			if( CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER){
				CHAR_setInt( enemyindex, CHAR_DUELBATTLECOUNT, CHAR_getInt( enemyindex, CHAR_DUELBATTLECOUNT ) + 1);
				CHAR_setInt( enemyindex, CHAR_DUELLOSECOUNT, CHAR_getInt( enemyindex, CHAR_DUELLOSECOUNT ) + 1);
			...
			if(( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER
				...
				&& ( CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER
				|| CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET
				|| CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYERNPC
				|| CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYERPETNPC
				)){
				int badindex;
				if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER){
					badindex = charaindex;
				...
				if(CHAR_getWorkInt( charaindex, CHAR_WORKCONFUSION ) <= 0){
					if(enemyindex != badindex){
			//			CHAR_warpToSpecificPoint( badindex, 117, 289, 168 );
			...
			//			BATTLE_Exit( badindex, battleindex );
						...
						*/
						char message[256];
						sprintf(message, "%s 被 %s 擊飛至遠方， %s 因飛人（寵），二人恩怨值火爆上升！！", CHAR_getUseName( enemyindex ), CHAR_getUseName( badindex ), CHAR_getUseName( badindex ));
					  int i;
					  int playernum = CHAR_getPlayerMaxNum();
					  for( i = 0 ; i < playernum ; i++) {
...
//		BATTLE_Exit( enemyindex, battleindex );
		...
		flg |= CHAR_BATTLEFLG_ULTIMATE;	// 失夥  奴丟永玄熬仃凶
...
//*************************************************************
{
	...
	&&  BattleArray[battleindex].norisk == 0
			...
			//			BATTLE_Exit( badindex, battleindex );
						...
						*/
						char message[256];
						sprintf(message, "%s 被 %s 擊飛至遠方， %s 因飛人，二人恩怨值火爆上升！！", CHAR_getUseName( enemyindex ), CHAR_getUseName( badindex ), CHAR_getUseName( badindex ));
					  int i;
					  int playernum = CHAR_getPlayerMaxNum();
					  for( i = 0 ; i < playernum ; i++) {
					    if( CHAR_getCharUse(i) != FALSE ) {
...
void BATTLE_EscapeDpSend( int battleindex, int charaindex )
{
	...
	if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
		if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER){
			CHAR_setInt( charaindex, CHAR_DUELBATTLECOUNT, CHAR_getInt( charaindex, CHAR_DUELBATTLECOUNT ) + 1);
			CHAR_setInt( charaindex, CHAR_DUELLOSECOUNT, CHAR_getInt( charaindex, CHAR_DUELLOSECOUNT ) + 1);
...
#ifdef _SHARE_EXP
		if(getExpShare()==1 && BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E){
			...
			int k = 0;
			for(i=0;i<BATTLE_ENTRY_MAX;i++){
				if( CHAR_CHECKINDEX( BattleArray[battleindex].Side[0].Entry[i].charaindex ) == FALSE )continue;
				if( BATTLE_CanMoveCheck( BattleArray[battleindex].Side[0].Entry[i].charaindex ) == FALSE ){
					continue;
				}
				if( CHAR_getInt( BattleArray[battleindex].Side[0].Entry[i].charaindex, CHAR_HP ) <= 0 ){
					continue;
				}
...
#ifdef _SKILLLIMIT	// (可開放) Syu ADD 不得攻擊我方限製
...
#define PETSKILLSERCHTIME 50
	...
	for( j = 0, i = 0, k = 0; k < PETSKILLSERCHTIME; i ++, k ++ ){
		...
		if(strcmp(PETSKILL_getChar( pskill_array, PETSKILL_FUNCNAME ), "PETSKILL_AttackMagic") == 0){
			continue;
		}
		
		if(strcmp(PETSKILL_getChar( pskill_array, PETSKILL_FUNCNAME ), "PETSKILL_Combined") == 0){
			continue;
		}
...
static int BATTLE_Battling( int battleindex )
{
	...
	for( j = 0; j < 2; j ++ ){
		for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
			...
			if(!CHAR_CHECKINDEX(pEntry[j][i].charaindex))continue;
	...
	for( i = 0; i < entrynum; i ++ ){
			...
			|| CHAR_getInt( charaindex, CHAR_BASEIMAGENUMBER)==101749) 
		{
			...
			if( !(COM == BATTLE_COM_ATTACK
				...
				|| COM == BATTLE_COM_NONE) ){
				CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE );
			    CHAR_talkToCli( charaindex, -1, "變成小狐狸後隻能攻擊、防禦和待機！", CHAR_COLORYELLOW );
			}
		}
#endif
#ifdef _PETSKILL_BECOMEPIG
		    ...
		    && (CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER) )
		{
			...
			if( !(COM == BATTLE_COM_ATTACK
				...
				CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_GUARD );
 			    //CHAR_talkToCli( charaindex, -1, "變成烏力後隻能攻擊、防禦、待機、逃跑和使用道具！", CHAR_COLORYELLOW );
...
#endif
		COM = CHAR_getWorkInt( charaindex, CHAR_WORKBATTLECOM1 );
		//printf("對像名=%s   COM=%d\n",CHAR_getChar(charaindex,CHAR_NAME),COM);
		...
		case BATTLE_COM_S_CONVOLUTE:		// 迴鏇攻擊
			{
#ifdef _PROFESSION_ADDSKILL
	/*			if( COM == BATTLE_COM_S_DOOM //世界末日集氣
		...
		case BATTLE_COM_S_BLOOD:			// 嗜血成性
			{
				CHAR_setFlg( charaindex, CHAR_ISATTACKED, 1 );
				defNo = CHAR_getWorkInt( charaindex, CHAR_WORKBATTLECOM2 );				
#if 1	// Robin fix 修正嗜血吸彆人的血
				if( defNo != attackNo && COM == BATTLE_COM_S_BLOOD ){
					print("\n 改封包!嗜血吸彆人的血!!:%s ", CHAR_getChar( charaindex, CHAR_CDKEY) );
					defNo = attackNo;

					CHAR_talkToCli( charaindex, -1, "因封包異常而斷綫。", CHAR_COLORYELLOW);
		...
		case BATTLE_COM_S_PETOUT:
			if( COM == BATTLE_COM_S_PETOUT ){
				...
				if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
					...
					char token[256];
					if((rand() % 100) > atoi(jl)){
						sprintf(token, "BK|%s 使用 %s 不成功|", CHAR_getUseName( defindex ), PETSKILL_getChar( array, PETSKILL_OPTION ));
					...
					for(petid=0;petid<CHAR_MAXPETHAVE;petid++){
						if( !CHAR_CHECKINDEX( CHAR_getCharPet( defindex, petid ) ) )continue;
						
						if( (strstr( skillarg, "休" ) ) != NULL ){
							if(CHAR_getWorkInt(defindex, CHAR_WORK_PET0_STAT+petid) != PET_STAT_NONE){
								continue;
							}
						}else if( (strstr( skillarg, "郵" ) ) != NULL ){
							if(CHAR_getWorkInt(defindex, CHAR_WORK_PET0_STAT+petid) != PET_STAT_MAIL){
								continue;
							}
						}else if( (strstr( skillarg, "待" ) ) != NULL ){
							if(CHAR_getWorkInt(defindex, CHAR_WORK_PET0_STAT+petid) != PET_STAT_SELECT){
								continue;
							}
						}else if( (strstr( skillarg, "騎" ) ) != NULL ){
							if( CHAR_getInt(defindex,CHAR_RIDEPET) != petid)continue;
						}
						...
						if( (strstr( skillarg, "收" ) ) != NULL )break;
						// shan End
						...
						lssproto_KS_send( getfdFromCharaIndex( defindex ), CHAR_getInt( defindex, CHAR_DEFAULTPET ) , TRUE );
						break;
					}
				}else{
					CHAR_talkToCli( charaindex, -1 , "該技能隻能對玩傢人物使用!" , CHAR_COLORYELLOW );
				}
				break;
			}
#endif
		...
		case BATTLE_COM_S_FIREKILL:
			if( COM == BATTLE_COM_S_FIREKILL ){
				...
				BATTLE_MultiAttMagic_Fire(battleindex,attackNo,defNo,2,200);//2與200代錶火屬性魔法200傷害
				BATTLESTR_ADD( "FF|" );
		...
		case BATTLE_COM_S_HECTOR:
...
#ifdef _PROFESSION_SKILL			// WON ADD 人物職業技能
				if( PROFESSION_BATTLE_StatusAttackCheck( charaindex, def_index, 2, fPer ) ){
          ...
          pszP = pszOption;
					...
					CHAR_setWorkInt( def_index, StatusTbl[status], turn);//設定麻痹
#else
					
					CHAR_setWorkInt( def_index, StatusTbl[2], 1);//設定麻痹
		...
		case BATTLE_COM_S_RESURRECTION:
			if( COM == BATTLE_COM_S_RESURRECTION ){
				...
				if( CHAR_getInt( BATTLE_No2Index( battleindex, defNo ), CHAR_HP ) <= 0 ){
					...
					BATTLE_MultiRessurect( battleindex, attackNo, defNo,
										 hp,  0, MAGIC_EFFECT_USER, SPR_fukkatu3 );
		...
		case BATTLE_COM_S_LOSTLOST:
			if( COM == BATTLE_COM_S_LOSTLOST ){
				...
				int attackHp = CHAR_getInt( BATTLE_No2Index( battleindex, attackNo ), CHAR_HP);
...
#endif
#ifdef _MASSAGE_PETSKILL
					if( CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM1) != BATTLE_COM_S_MASSAGE) 
#endif
#ifdef _GRAPPLING_PETSKILL
					if( CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM1) != BATTLE_COM_S_GRAPPLING) 
#endif
#ifdef _INVERSION_PETSKILL
					if( CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM1) != BATTLE_COM_S_INVERSION) 
#endif
	...
	if(COM == BATTLE_COM_S_INVERSION ){
		if((Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_MISS)  //沒有miss
			 && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_DODGE)
			 && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ALLGUARD) //沒有閃避過
...
#ifdef _PETSKILL_BECOMEFOX // 嚮對方使用媚惑術
		if ( (COM == BATTLE_COM_S_BECOMEFOX )
			 ...
			 && (BATTLE_TargetCheck( battleindex, defNo )) //還活著
			 && (rand()%100 < 31) 
			 && (CHAR_getInt( BATTLE_No2Index( battleindex, defNo ), CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER)  //隻有寵物(CHAR_TYPEPET)纔可以被媚惑
...
#endif
#ifdef _PETSKILL_BECOMEPIG // 嚮對方使用烏力化
...
#endif
			 && (BATTLE_TargetCheck( battleindex, defNo )) //還活著
		     && (CHAR_getInt( BATTLE_No2Index( battleindex, defNo ), CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER)  //隻有玩傢纔可以被烏力化
#ifdef _PREVENT_TEAMATTACK		     
		     && (BATTLE_CheckSameSide( charaindex, defNo) != 1 ) // 防止同隊互打
#endif
			 && CHAR_getInt( BATTLE_No2Index( battleindex, defNo ), CHAR_BECOMEPIG) < 2000000000 ) //防止破錶
		{    
			char temp[64];
...
/*
#ifndef _VERSION_25 // Robin fix 戰鬥不可換防具飾品
			{
...
*/
			...
			if( itemtype == CHAR_ARM 
				|| itemtype == CHAR_HEAD 
				|| itemtype == CHAR_BODY 
				|| itemtype == CHAR_DECORATION1 
		...
		case BATTLE_COM_S_STRENGTH:
			...
			defNo = CHAR_getWorkInt(charaindex,CHAR_WORKBATTLECOM2);
			skill = CHAR_GETWORKINT_LOW(charaindex,CHAR_WORKBATTLECOM3);
			ContFlg = PETSKILL_SetStrength_Battle( battleindex, attackNo, defNo, skill);
			break;
			}
#endif
#ifdef _OFFLINE_SYSTEM
		...
		case BATTLE_COM_S_REFRESH://狀態迴復 vincent add 2002/08/08
			{
              int array;
	...
	if(CHAR_CHECKINDEX(charaindex)){
		...
		{		
			...
			if(pBattle->turn - CHAR_getWorkInt( charaindex, CHAR_WORKFOXROUND) > 2)
			{
				...
				//print("\n變身人物迴復:%s,round:%d", CHAR_getChar( charaindex, CHAR_NAME), CHAR_getWorkInt( charaindex, CHAR_WORKFOXROUND));
...
//	}rePet[]={{1,SPR_fukkatu3,100},{-1,0,0}};
	...
	for( i=0;i<arraysizeof( rePet); i++){
		if( CHAR_getInt( petindex, CHAR_PETID) == rePet[i].petid )	//硃雀
			break;
	}
	if( i == arraysizeof( rePet) )
		return FALSE;
	//ReceveEffect = SPR_fukkatu3; 
	ReceveEffect = rePet[i].Effect;//硃雀動畫 未定
