...
{
	char			*functionname;		/*	澀爛白央奶夥卞踏仁楮醒及  蟆 */
	PETSKILL_CALLFUNC	func;				/*    端卞裟太請今木月楮醒 */
	int				hash;				/*  hash */
	int				status;				/*  仇及戊穴件玉毛矢永玄互隙爛仄凶執 */
...
#endif
#ifdef _PETSKILL_PASSIVE_PET_MATCH1
		{ "PETSKILL_PASSIVE_PET_MATCH1",	PETSKILL_PASSIVE_PET_MATCH1, 0},
#endif
...
#ifdef _PETSKILL_OPTIMUM // 寵技Table數為 最大技能ID +1
...
}
{
...
#ifdef _PETSKILL_OPTIMUM // 讀取本行寵技的ID, 直接以寵技ID當Table index
    ...
    print( "有效寵物技能數是 %d...", PETSKILL_petskillnum );
...
{
#ifdef _PETSKILL_OPTIMUM // 查詢寵技ID改為陣列式
...
#ifdef _EXPANSION_VARY_WOLF
	if( (pszP = strstr( pszOption, "圖%" ) ) != NULL ){
		int image=0;
	...
	char buf[256];
      ...
      if(CHAR_getInt(charaindex,CHAR_HP) > CHAR_getWorkInt(charaindex,CHAR_WORKMAXHP)*0.2 )
	  ...
	  else
		{
			...
			sprintf(buf, "%s自身耐久力不足 救援失敗!!", CHAR_getChar( charaindex, CHAR_NAME ));
...
int PETSKILL_Strength( int charaindex, int toNo, int array, char *data )
{
	...
	if( nums >= max(1, atoi(buf1)) ){
		...
		char buf[256];
		sprintf(buf,"此技能單場限用 %d 次。", max(1, atoi(buf1)));
...
#ifdef _SKILL_SPEEDY_ATT//疾速攻擊 vincent add 2002/05/20
...
)
{
	...
	MergeCallBack(CHAR_getWorkInt( charaindex, CHAR_WORKPLAYERINDEX),charaindex,data,ret);
#endif
	return ret;
}
...
)
{
	...
	if( (pszP = strstr( pszOption, "擊%" ) ) != NULL ){
...
)
{
	char *pszP;
	char szMagic[] = "magic";
	int magic = 313;	// magic id預設為火魔法等級一,item index預設為19659
//	int item = 19659;
	// 取得寵物技能命令
...
int PETSKILL_DamageToHp( int charaindex, int toNo, int array, char *data )
{
	...
	CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2, toNo );
	CHAR_setWorkInt( charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK );
	CHAR_SETWORKINT_LOW(charaindex,CHAR_WORKBATTLECOM3, array);	//記錄技能
	pszOption = PETSKILL_getChar( array, PETSKILL_OPTION );
...
int PETSKILL_Mdfyattack( int charaindex, int toNo, int array, char *data )
{
	...
	if( getStringFromIndexWithDelim( pszOption, "|", 2, buf1, sizeof( buf1)) == FALSE )
		return FALSE;
	nums=atoi( buf1);
	CHAR_SETWORKINT_HIGH( charaindex, CHAR_WORKBATTLECOM4, nums);//記錄數值
	return TRUE;
}
#endif
...
#define MAXINSLAY 15
	...
	for( i = 0; i < 15; i ++ ) {
		...
		if( ITEM_getInt( itemindex, ITEM_CANBEPILE) == 1 &&
			ITEM_getInt( itemindex, ITEM_USEPILENUMS) > 1 ){
				CHAR_talkToCli( charaindex, -1, "疊加物品無法進行精工。", CHAR_COLORYELLOW);
				return FALSE;
		}
#endif

		if( i >= MAXINSLAY )	{
			CHAR_talkToCli( charaindex, -1, "精工每次隻能選擇四個物品。", CHAR_COLORYELLOW);
			return FALSE;
		}
		...
		if( (buf1 = ITEM_getChar( itemindex, ITEM_TYPECODE)) == "\0" )	{
			print(" ITEM_TYPECODE == NULL 錯誤 !!\n");
			return FALSE;
		}
		if( !strcmp( buf1, "\0") || !strcmp( buf1, "NULL") )	{
			CHAR_talkToCli( charaindex, -1, "這樣物品不適閤精工。", CHAR_COLORYELLOW);
			return FALSE;
		}
		if( strstr( buf1, "INSLAY") != 0 )	{
			if( !ITEM_CHECKINDEX( inslayindex))	{
				...
				CHAR_talkToCli( charaindex, -1, "精工每次隻能選擇一個武器或防具。", CHAR_COLORYELLOW);
				return FALSE;
			}
		}
	...
	if( !ITEM_CHECKINDEX( inslayindex) )	{
		CHAR_talkToCli( charaindex, -1, "精工必須選擇一個武器或防具。", CHAR_COLORYELLOW);
		return FALSE;
	}
...
int PETSKILL_SetMagicPet( int charaindex, int toNo, int array, char *data )
{
	...
	if( nums >= 3 ){
		int toindex = CHAR_getWorkInt( charaindex, CHAR_WORKPLAYERINDEX);
		CHAR_talkToCli( toindex, -1, "此技能單場限用三次。", CHAR_COLORYELLOW);
		return FALSE;
	}
...
#define MAXFIXITEM 2
	...
	for( i = 0; i < 15; i ++ ) {
		...
		if( i >= MAXFIXITEM )	{
			CHAR_talkToCli( charaindex, -1, "修復每次隻能選擇兩個物品。", CHAR_COLORYELLOW);
			return FALSE;
		}
		for( j=0; j< MAXFIXITEM; j++ ) {
			if( itemindexs[j] == itemindex )	{
				print("修復物品index相同 ERROR !!\n");
				return FALSE;
			}
		}
		itemtype = ITEM_getInt( itemindex, ITEM_TYPE);
		if( itemtype == ITEM_DISH )	{
			CHAR_talkToCli( charaindex, -1, "料理不能做修復。", CHAR_COLORYELLOW);
			return FALSE;
		}
		...
		if( (itemtype >= ITEM_FIST && itemtype <= ITEM_AMULET) || 
			...
			if( !ITEM_CHECKINDEX( fixindex))	{
				...
				CHAR_talkToCli( charaindex, -1, "修復每次隻能選擇一個武器或防具。", CHAR_COLORYELLOW);
				return FALSE;
			}
		}
	...
	if( !ITEM_CHECKINDEX( fixindex) )	{
		CHAR_talkToCli( charaindex, -1, "修復必須選擇一個武器或防具。", CHAR_COLORYELLOW);
		return FALSE;
	}
	...
	CHAR_send_P_StatusString( charaindex, CHAR_P_STRING_ATK | CHAR_P_STRING_DEF 
		| CHAR_P_STRING_QUICK | CHAR_P_STRING_CHARM );
	CHAR_talkToCli( charaindex, -1, "修復完成。", CHAR_COLORYELLOW);
	return TRUE;
}
#endif
...
int PETSKILL_Lostlost( int charaindex, int toNo, int array, char* data)
{
	...
	if( nums >= max(1, atoi(skillarg)) ){
		int toindex = CHAR_getWorkInt( charaindex, CHAR_WORKPLAYERINDEX);
		char buf[256];
		sprintf(buf,"此技能單場限用 %d 次。", max(1, atoi(skillarg)));
		CHAR_talkToCli( toindex, -1, buf, CHAR_COLORYELLOW);
		return FALSE;
	}
...
int PETSKILL_Gyrate( int charaindex, int toNo, int array, char *data )
{
	...
	CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2, toNo ); //攻擊對象  23,24,25,26 為單排
	CHAR_setWorkInt( charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK );
...
int PETSKILL_BattleModel(int charaindex, int toindex, int array, char* data)
{
  ...
  pszOption = PETSKILL_getChar(array,PETSKILL_OPTION);
	if(pszOption == "\0"){
		printf("PETSKILL_BattleModel: 讀取 PETSKILL_OPTION 錯誤!!(數組:%d,文件:%s,第%d行)\n",array,__FILE__,__LINE__);
		return FALSE;
	}
	...
	CHAR_SETWORKINT_LOW(charaindex,CHAR_WORKBATTLECOM2,iType);				// 記錄攻擊類型
	CHAR_SETWORKINT_HIGH(charaindex,CHAR_WORKBATTLECOM2,iObjectNum);	// 記錄攻擊物件數量
...
int PETSKILL_PASSIVE_PET_MATCH1( int charaindex, int toNo, int array, char *data )
{
	...
	//CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_S_PASSIVE_PET_MATCH1 );
	//CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2, toNo );
	//CHAR_setWorkInt( charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK );
	...
	if( (pszP = strstr( pszOption, "迴%" ) ) != NULL ){
		sscanf( pszP+3, "%f", &fPer );
		datainfo[4]=(int)fPer;
	...
	char buf[256];
	sprintf(buf, "%s被動提升 %d%攻擊 %d%防敏 %d%敏捷 %d%命中 %d%迴避!!", CHAR_getChar( charaindex, CHAR_NAME ),datainfo[0],datainfo[1],datainfo[2],datainfo[3],datainfo[4]);
	CHAR_talkToCli( CHAR_getWorkInt(charaindex, CHAR_WORKPLAYERINDEX), -1, buf, CHAR_COLORYELLOW);
	return TRUE;
