...
char *aszStatusFull[] = { "全快", "毒", "麻痹", "睡眠", "石化", "酒醉", "混亂", "虛弱", "劇毒", "魔障", "沉默"
#ifdef _PET_SKILL_SARS			// WON ADD 毒煞蔓延
					  ,"毒煞"
#endif
#ifdef _PROFESSION_SKILL			// WON ADD 人物職業技能
...
#define BREAK_BRACELET			"護腕"
#define BREAK_MUSIC 			"樂器"
#define BREAK_NECKLACE 			"頭飾"
#define BREAK_RING 				"戒指"
#define BREAK_BELT 				"皮帶"
#define BREAK_EARRING 			"耳飾"
#define BREAK_NOSERING 			"鼻環"
#define BREAK_AMULET 			"護身符"
#define BREAK_OTHER 			"道具"

#ifdef _TAKE_ITEMDAMAGE
static char *aszCrushTbl[] = { "完好", "受損", "毀壞" , "不堪使用" , "碎片" };
#else
static char *aszCrushTbl[] = { "受損", "毀壞" };
#endif
...
int BATTLE_ItemCrush( int charaindex )
{
	...
	{
		if( crushlevel == 1 ){
			sprintf( szBuffer, "CrushLv2(毀壞道具Lv2)" );
		}else{
			sprintf( szBuffer, "CrushLv1(毀壞道具Lv1)" );
		}
		LogItem(
			CHAR_getChar( charaindex, CHAR_NAME ),
			CHAR_getChar( charaindex, CHAR_CDKEY ),
...
)
{
	...
	if( BATTLE_CanMoveCheck( defindex ) == FALSE ){
#ifdef _PROFESSION_ADDSKILL
		if( CHAR_getWorkInt( defindex, CHAR_DOOMTIME ) <= 0 )//集氣時可閃避(並且沒有處於天羅地網和盾擊的狀態)
			return FALSE;
		else{
...
#ifdef _PROFESSION_ADDSKILL
	if( CHAR_getWorkInt( attackindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_CHAOS ){ //混亂攻擊時命中降低
		per += (per*0.4);	
	}
#endif
...
#ifdef _EQUIT_NEGLECTGUARD //忽視目標防禦力%
...
#ifdef _ADD_DEAMGEDEFC //額外傷害 & 附加攻擊
	{
...
// Terry add fix 使用捨已為人技能時,若攻擊使用此技能的人client會當
...
#ifdef _PROFESSION_SKILL			// WON ADD 人物職業技能
	|| CHAR_getWorkInt( GuardianIndex, CHAR_WORKDIZZY ) > 0 	// 暈眩
	|| CHAR_getWorkInt( GuardianIndex, CHAR_WORKDRAGNET ) > 0 	// 天羅地網
	|| CHAR_getWorkInt( GuardianIndex, CHAR_WORKINSTIGATE ) > 0 // 挑撥
#endif
#ifdef _PROFESSION_ADDSKILL
	|| CHAR_getWorkInt( GuardianIndex, CHAR_DOOMTIME ) > 0 //世界末日集氣
#endif
	){
...
static int BATTLE_AttackSeq( int attackindex, int defindex, int *pDamage, int *pGuardian,	int opt )
{
	...
	if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET 
		...
		if( CHAR_getWorkInt( defindex, CHAR_WORKPLAYERINDEX ) == attackindex ){
			CHAR_PetAddVariableAi( defindex, AI_FIX_SEKKAN );	// 修正忠誠度
		}
	}
	...
	if( opt == BATTLE_COM_S_DAMAGETOHP2 ){ //暗月狂狼 (會心一擊上升30%)  && CHAR_getInt( attackindex, CHAR_HP) <= CHAR_getWorkInt( attackindex, CHAR_MAXMP)*0.5
		//print("\n1會心一擊:%d",perCri);
		//print("\n1攻:%d", CHAR_getWorkInt( attackindex, CHAR_WORKATTACKPOWER));
		//print("\n1敏:%d", CHAR_getWorkInt( attackindex, CHAR_WORKQUICK));
		perCri = perCri + (perCri*0.3); //會心一擊上升30%
...
#ifdef _PETSKILL_REGRET
	if( opt == BATTLE_COM_S_REGRET2 ){// 人物傷害減少
		(*pDamage) = (*pDamage)*0.8;
	}else
#endif			
#ifdef _SONIC_ATTACK				// WON ADD 音波攻擊
	if( opt == BATTLE_COM_S_SONIC2 ){// 人物傷害減半
		(*pDamage) = (*pDamage)*0.5;
	}else
#endif
	if( CHAR_getWorkInt( defindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_GUARD
...
#ifdef _PREVENT_TEAMATTACK	// 防止同隊互打
	if (CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ||
		CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET ) {
		...
		if( myside == CHAR_getWorkInt( attackindex, CHAR_WORKBATTLESIDE)){
...
#ifdef _PROFESSION_SKILL			// WON ADD 人物職業技能
					|| CHAR_getWorkInt( attackindex, CHAR_WORKINSTIGATE) > 0	// 挑撥
#endif
#ifdef _PROFESSION_ADDSKILL
			...
			}else if( CHAR_getWorkInt( attackindex, CHAR_WORKTEMPTATION ) > 0 ){//水漾誘惑
#endif
			}else if( CHAR_getInt( attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPET &&
				CHAR_getWorkInt( attackindex, CHAR_WORKBATTLEFLG) & CHAR_BATTLEFLG_AIBAD ){	//nono寵
			}else if( CHAR_getInt( defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && 
				CHAR_getWorkInt( defindex, CHAR_WORKPLAYERINDEX) == attackindex ){		// 自已寵物
			}else {
				int myhp, mymaxhp;
...
#endif
#ifdef _PETSKILL_ACUPUNCTURE
	if( CHAR_getWorkInt( charaindex, CHAR_WORKACUPUNCTURE ) > 0 ){	//針刺外皮
	    //print("\n針刺:BATTLE_GetDamageReact");
...
#endif
#ifdef _PETSKILL_ACUPUNCTURE
	if( *pRefrect == BATTLE_MD_ACUPUNCTURE ){ //針刺外皮
		
		if( damage%2 != 0 )
			damage+=1;
        playerdamage = damage;

		//先扣被攻擊方的血,因為之後的動作都是對攻擊方做的
		hp = CHAR_getInt( defindex, CHAR_HP );
		if( attackpet == -1 ) {
			hp -= damage;
			if(hp<0)hp=0;
		...
		//if( CHAR_getWorkInt( defindex, CHAR_WORKSLEEP ) > 0 )//檢查有無昏睡
	    //    CHAR_setWorkInt( defindex, CHAR_WORKSLEEP, 0);
...
#ifdef _PETSKILL_FIREKILL //火綫獵殺專用
...
int BATTLE_Attack( int battleindex, int attackNo, int defNo )
{
...
#ifdef _PETSKILL_ACUPUNCTURE
    if( react == BATTLE_MD_ACUPUNCTURE ){ //針刺外皮時會無法正常解除被打方的狀態,所以把index換迴來
	    defindex = toindex = BATTLE_No2Index( battleindex, defNo );
        statusDefNo = defNo;
	}
#endif
	...
	if(CHAR_getWorkInt( attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_GRAPPLING ){
		...
		if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
			...
			int itemindex;
			int lj;
			char item[8][32]={"頭盔","鎧甲","武器","首飾左","首飾右","腰帶","盾","鞋子"};
			...
			if( CHAR_CHECKITEMINDEX( defindex, itemindex) == FALSE ){
				sprintf(token, "BK|%s 未裝備 %s |", CHAR_getUseName( defindex ), item[fromid]);
			}else{
				
				if(toid == -1){
					sprintf(token, "BK|%s無空位卸下%s|", CHAR_getUseName( defindex ), item[fromid]);
				}else{
					if(rand() % 100 <= lj){
						sprintf(token, "BK|%s的%s已卸下|", CHAR_getUseName( defindex ), item[fromid]);
						CHAR_moveEquipItem(defindex, fromid, toid);
					}else{
						sprintf(token, "BK|%s的%s差點被卸下|", CHAR_getUseName( attackindex ), item[fromid]);
					}
				}
			}
		}else{
			sprintf(token, "BK|非人物無法卸下|");
		}
...
// 雷爾不能被打飛
...
#ifdef _PETSKILL_ACUPUNCTURE
    if( react == BATTLE_MD_ACUPUNCTURE ){ //針刺外皮不會反射特殊狀態,所以要改迴來
	    defindex = toindex = BATTLE_No2Index( battleindex, defNo );
        statusDefNo = defNo;
	}
	//注意:之後要不要再改迴原本反射.......????
	...
	if( gBattleStausChange == -1 && CHAR_getWorkInt( attackindex, CHAR_SUITPOISON ) > 0 )//帶毒裝備
...
#ifdef _WOLF_TAKE_AXE				// WON ADD 抓雙頭狼的限製
	...
	for( i=0; i<MAXCAPTRUEFREE; i++ ){
		...
		for( j = 0; j <  CheckCharMaxItem(attackindex); j ++ ){
			...
			if( ITEM_getInt( itemindex, ITEM_ID ) == NeedEnemy[ ti].ItemId[i] ){
				...
				//break;//Change fix 抓到一隻寵隻刪除一個道具(最後還是決定全刪)
			}
		}
	}
	return TRUE;
...
)
{
	...
	if( BATTLE_CaptureItemCheck( attackindex, defindex ) == FALSE 
		...
		if( BATTLE_CaptureCheck( attackindex, defindex, &per ) == FALSE ){
			//snprintf( szBuffer, sizeof(szBuffer), "(%s)無法捕獲(%s)(%.2f%%)",
			//	CHAR_getUseName( attackindex ),
			//	CHAR_getUseName( defindex ),
			//	per
			//);
	...
	if( flg == 1 ){
		...
		if( pindex == -1 ){
			//snprintf( szBuffer, sizeof(szBuffer), "(%s)呈現無法捕獲的狀態。",
			//	CHAR_getUseName( attackindex ) );
			flg = 0;
		}else{
			int ai;
...
int BATTLE_PetIn( int battleindex, int attackNo )
{
	...
	& CHAR_BATTLEFLG_NORETURN
		...
		//snprintf( szBuffer, sizeof(szBuffer), "(%s)無法迴復成(%s)",
		//	CHAR_getUseName( attackindex ),
	...
	//snprintf( szBuffer, sizeof(szBuffer), "(%s)迴復成(%s)",
	//	CHAR_getUseName( attackindex ),
	//	CHAR_getUseName( petindex )
	//);
	//BATTLE_BroadCast( battleindex, szBuffer,
...
int BATTLE_PetOut( int battleindex, int attackNo )
{
	...
	if( CHAR_getInt( attackindex, CHAR_DEFAULTPET ) < 0 ){
		...
		//snprintf( szBuffer, sizeof(szBuffer), "(%s)呼喚(%s)",
		//	CHAR_getUseName( attackindex ),
		//	CHAR_getUseName( petindex )
		//);
...
int BATTLE_S_Roar( int battleindex, int attackNo, int defNo, int marray )
{
	...
	index2 = BATTLE_No2Index( battleindex, defNo);//被擊中目標之index
	...
	if( CHAR_getInt( index2, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ) {
		...
		masteridx = BATTLE_No2Index(battleindex,defNo-5);//作用在寵物
...
//送訊息至玩傢
	if( FINDPET == TRUE )	{//若為年獸
		...
		BATTLESTR_ADD(szWork);
		BATTLESTR_ADD( "FF|" );
		sprintf(szWork,"BQ|e%X|",defNo);//逃跑動畫
		BATTLESTR_ADD(szWork);
...
)
{
	...
	for( i = 1; i < BATTLE_ST_END; i++ ){//若已有異常狀態則return
...
int BATTLE_Combo( int battleindex, int *pAttackList, int defNo )
{
	...
	//BATTLE_BroadCast( battleindex, "發動必殺技！！",
...
void BATTLE_StealMoney( int battleindex, int attackNo, int defNo)
{
	...
	if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER )	{
		...
		if( defNo>=safeSide*SIDE_OFFSET && defNo<((safeSide*SIDE_OFFSET)+SIDE_OFFSET)){//同邊
			per = 0;
		}else	{	//不同邊
			per = 50;
			per = (((per+LV)/4)+10)>>1;
...
void BATTLE_Steal( int battleindex, int attackNo, int defNo )
{
	...
	if( RAND( 1, 100 ) < per ){
		...
		flg = 0;
		//snprintf( szBuffer, sizeof(szBuffer), "(%s)沒從(%s)獲得東西(%d%%)",
...
int BATTLE_S_FallGround( int battleindex, int attackNo, int defNo, int skill_type)
{
	...
	react = 0;
	react = BATTLE_GetDamageReact( defindex);//有光鏡守
	//取得攻擊後狀態
	iWork = BATTLE_AttackSeq( attackindex, defindex, &damage,
	...
	if( skill_type == BATTLE_COM_S_FALLRIDE && damage > 0 && react == 0
		...
		if( 
			...
			if( CHAR_getInt( defindex , CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER )	{
#ifdef _FIXPETFALL //Syu ADD 修正落馬術
...
int BATTLE_S_DamageToHp( int battleindex, int attackindex, int defindex, int Damage, int skill)
{
	...
	if( BATTLE_GetDamageReact( defindex) > 0 )//有光鏡守
		return A_HP;
...
void BATTLE_S_ToothCrushe( int battleindex, int attackindex, int defindex, int damage, int skill)
{
	...
	//char *strCrushTbl[] = { "完好", "受損", "毀壞" , "不堪使用" , "碎片" };
	if( CHAR_getInt( defindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER 
#ifdef _TAKE_ITEMDAMAGE_FOR_PET
			&& CHAR_getInt( defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET
#endif
	...
	case BATTLE_COM_S_PROPERTYSKILL:
		{
			...
			strcpysafe( ch->charfunctable[CHAR_BATTLEPROPERTY].string,
				sizeof( ch->charfunctable[CHAR_BATTLEPROPERTY]), pszP);//戰鬥
...
int BATTLE_S_AttackDamage( int battleindex, int attackNo, int defNo, int skill_type, int skill)
{
	...
	attackindex = BATTLE_No2Index( battleindex, attackNo );	//攻方index
	defindex = BATTLE_No2Index( battleindex, defNo ); //守方index
	if( (ReactType=BATTLE_GetDamageReact( defindex)) > 0 ){	//有光鏡守
		react = ReactType;
...
#ifdef _SKILL_TOOTH
	case BATTLE_COM_S_TOOTHCRUSHE:	//艸菔 tooth
		flg |= BCF_TOOTH;
	...
	case BATTLE_COM_S_TIMID:
		{
			...
			if( timid < 15 && damage > 1 ){//怯戰
				...
				if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET ){
					...
					BATTLE_Exit( defindex, battleindex);
					CHAR_DischargePartyNoMsg( defindex);//解散團隊
				}
			}
		}
		break;
	...
	case BATTLE_COM_S_2TIMID:
		...
		case BATTLE_COM_S_ANTINTER:
		{
			...
			if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET ){
				int defkoyaindex = CHAR_getWorkInt( defindex, CHAR_WORKPLAYERINDEX );
				BATTLE_PetDefaultExit( defkoyaindex, battleindex );//寵物離開戰場
				CHAR_setInt( defkoyaindex, CHAR_DEFAULTPET, -1 );//設定無參戰寵物
			}
		}
		break;
#endif
		...
		case BATTLE_COM_S_REGRET:
			{
				...
				if( (PROFESSION_BATTLE_StatusAttackCheck( attackindex, defindex, 12, Success ) == 0 )
					...
					CHAR_setWorkInt( defindex, StatusTbl[12], 2 );//暈眩1迴閤
					CHAR_setWorkInt( defindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE );
...
BOOL BATTLE_S_Temptation(	int battleindex,int attackNo,int defNo,	int skill )
{
	...
	if( CHAR_getWorkInt( defindex , CHAR_WORKFIXAI ) <= atoi(magicarg)){
		//技能攻擊
		CHAR_setWorkInt( defindex, CHAR_WORKTEMPTATION , 1 ); //用來解除打自己方1滴血的限製
		...
		img1 = 101505;	//珠珠動畫101637; //準備期動畫
		img2 = 101502;	//珠珠動畫101645; //擊中後動畫
		flg |= BCF_NO_DAMAGE;
		flg |= BCF_B_SKILLACT; //擊中後秀圖
...
//toindex = BATTLE_No2Index( battleindex, ToList[i] );
	if( toNo < 20 ){//單一目標
		...
		if( CHAR_getWorkInt( toindex, CHAR_WORKBATTLESIDE) == MySide ) return 1;//同邊
	}else if( toNo){
...
int CHAR_BattleStayLoop( int charaindex)
{
	...
	if( getStayEncount( fd) ) {
		...
		strcpysafe( ch->charfunctable[CHAR_LOOPFUNCTEMP1].string,
			sizeof( ch->charfunctable[CHAR_LOOPFUNCTEMP1]), "");//戰鬥
		...
		case BATTLE_COM_S_CHAIN_ATK_2:	// 雙重攻擊
			{
				...
				//int reflec_num  = CHAR_getWorkInt( defindex, CHAR_WORKDAMAGEREFLEC );	// 鏡
				...
				//if( reflec_num > 0 )	CHAR_setWorkInt( defindex, CHAR_WORKDAMAGEREFLEC, max(reflec_num - 1, 0)  );//讓鏡有效,這行要拿掉
		...
		case BATTLE_COM_S_DEAD_ATTACK:	// 瀕死攻擊
			{
				...
				if( old_hp <= 10 ){
					CHAR_talkToCli( charaindex, -1, "HP不足，無法使用技能", CHAR_COLORYELLOW);
					return iRet;
				}
...
//			skill_type=-1;
	...
	//取得攻擊後狀態
	iWork = BATTLE_AttackSeq( attackindex, defindex, &damage, &Guardian, skill_type );
	if( skill_type == BATTLE_COM_S_ENRAGE_PET ){//讓激怒寵物技能不要真的打死寵物
		if( CHAR_getInt( defindex, CHAR_HP ) <= damage ){
			damage = 0;
		}
	}
	if( skill_type == BATTLE_COM_S_CAVALRY)		// 座騎攻擊
	...
	// 將 NORMAL 及 CRITICAL 的 flg 改成 職業的flg
	if( img2 != 0  ){
		if( flg == BCF_NORMAL || flg == BCF_KAISHIN ){
			if( (pszP = strstr( pszOption, "前" ) ) != NULL )
				flg |= BCF_F_SKILLACT; //擊中前秀圖
			else
				flg |= BCF_B_SKILLACT; //擊中後秀
		}
	}
...
int battle_profession_attack_magic_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	if( toindex < 20 ){					// 單體
		attIdx = 0;
	}else if( toindex>=23 && toindex<=26 ){ // 單排
		attIdx = 1;
	}else if( toindex==20 || toindex==21 ){	// 一方全體
		attIdx = 2;
	}
...
int battle_profession_assist_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	// 使用迴閤數
	if( (pszP = strstr( pszOption, "迴%" ) ) != NULL )	sscanf( pszP+3, "%d", &turn );
	if(turn < 1)	turn=1;
		...
		case BATTLE_COM_S_THUNDER_ENCLOSE:	// 雷附體
			{
				...
				// 改變狀態
				if( ((pszP = strstr( pszOption, "纏" ) ) != NULL ) ||
					...
					((pszP = strstr( pszOption, "羅" ) ) != NULL ) ||
					((pszP = strstr( pszOption, "擊" ) ) != NULL ) ||
					((pszP = strstr( pszOption, "凍" ) ) != NULL ) ||
...
//					}
				...
				// 迴閤數
				if( (skill_type == BATTLE_COM_S_INSTIGATE) && (skill_level == 10) ){		// 挑撥
					turn = 4;
				...
				}else if( skill_type == BATTLE_COM_S_OBLIVION ){			// 遺忘
				...
				for( i = 0; ToList[i] != -1; i ++ ){
					...
					if( (PROFESSION_BATTLE_StatusAttackCheck( charaindex, toindex, status, Success ) == 0 )
						...
						//snprintf( szBuffer, sizeof(szBuffer), "(%s)無法將(%s)做成(%s)(%d%%)",
						...
						if( status == BATTLE_ST_DRAGNET ){//天羅地網
							CHAR_setWorkInt( toindex, CHAR_DOOMTIME, 0 );//世界末日集氣
						...
						if( skill_type == BATTLE_COM_S_FIRE_ENCLOSE ){			// 火附體
						...
						}else if(skill_type == BATTLE_COM_S_ICE_ENCLOSE ){		// 冰附體
						...
						}else if(skill_type == BATTLE_COM_S_THUNDER_ENCLOSE ){	// 雷附體
...
#endif
						//snprintf( szBuffer, sizeof(szBuffer),
						//	"(%s)將(%s)做成(%s)(%d%%)", CHAR_getUseName( charaindex ),
		...
		case BATTLE_COM_S_SHIELD_ATTACK:	// 盾擊
			{
				...
				if( ( itmid < 0 ) || ITEM_WSHIELD != ITEM_getInt( itmid , ITEM_TYPE ) ){
					CHAR_talkToCli( charaindex, -1, "未裝備盾牌，無法使用此技能", CHAR_COLORYELLOW);
					iRet = FALSE;	
					break;
				} 				
				...
				// 改變狀態
				if( (pszP = strstr( pszOption, "暈" ) ) != NULL ){
					for( i = 1; i < BATTLE_ST_END; i ++ ){
						if( strncmp( pszP, aszStatus[i], 2 ) == 0 ){
							status = i;
							break;
		...
		case BATTLE_COM_S_BOUNDARY:         // 四屬性結界
            {
				...
				if( ((pszP = strstr( pszOption, "地結界" ) ) != NULL ) )
				    status = CHAR_WORKFIXEARTHAT_BOUNDARY;
				else if( ((pszP = strstr( pszOption, "水結界" ) ) != NULL ) )
				    status = CHAR_WORKFIXWATERAT_BOUNDARY;
				else if( ((pszP = strstr( pszOption, "火結界" ) ) != NULL ) )
				    status = CHAR_WORKFIXFIREAT_BOUNDARY;
				else if( ((pszP = strstr( pszOption, "風結界" ) ) != NULL ) ){
				...
				if( ((pszP = strstr( pszOption, "破結界" ) ) != NULL ) ){
				...
				if( ((pszP = strstr( pszOption, "破結界" ) ) == NULL ) ){//先把所有結界清除
...
void BATTLE_BatFly( int battleindex, int attackNo,int myside)
{
	...
	PROFESSION_magic[2].uiPrevMagicNum = myside ? 101861:101862;		// 前置動畫
#else
	PROFESSION_magic[2].uiSpriteNum = myside ? 101806:101807;
	PROFESSION_magic[2].siSx = 320;
	PROFESSION_magic[2].siSy = 240;
	PROFESSION_magic[2].uiPrevMagicNum = myside ? 101808:101809;		// 前置動畫
#endif
	PROFESSION_magic[2].uiPostMagicNum = 0xffffffff;								// 無後置動畫
...
typedef struct _tsAttackObject{
	int index;					// 攻擊物件編號
	int target;					// 目標編號
	int actionNumber;		// 攻擊物件圖號
}AttackObject;
// BATTLE_BattleModel 用的攻擊函式
...
void BATTLE_BattleModel(int battleindex,int attackNo,int myside)
{
	...
	iType = CHAR_GETWORKINT_LOW(charaindex,CHAR_WORKBATTLECOM2);				// 取得攻擊類型
	iObjectNum = CHAR_GETWORKINT_HIGH(charaindex,CHAR_WORKBATTLECOM2);	// 取得攻擊物件數量
	...
	// 判斷目標數量和攻擊物件數量
	// 當攻擊物件數量大於等於在場敵方數量，則攻擊所有目標，有的目標可能受到二次以上的攻擊
	if(iObjectNum >= i){
		i0 = i;		// 記錄場上目標隻數
	...
	// 當攻擊物件數量小於在場目標數量，則以攻擊類型決定攻擊方式
		...
		i0 = i;		// 記錄場上目標隻數
		// 型態 2 :每個攻擊物件打完該攻擊目標後便不再攻擊其它目標,每個目標隻會受一次攻擊
		// 當iType & 0x00000001 為 true 之外的值都以型態 2 處理
		for(i=0;i<iObjectNum;i++){
			// 設定攻擊目標內容
			if(AAttackObject[i].index != -1){
				AAttackObject[i].target = iToList[i];
