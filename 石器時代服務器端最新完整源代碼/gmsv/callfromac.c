...
#endif
{
  ...
  print("與AC服務器連接成功！\n");
	...
	print( "服務端版本: <%s>\n" , SERVER_VERSION);
	print( "主頁：http://www.allblues.com.cn\n" );
	print( "論壇：http://www.allblues.com.cn\n" );
...
void saacproto_ACCharList_recv(int fd, char* result, char* data,int retfd)
{
    ...
    if( CONNECT_checkfd(clifd) == FALSE )
		...
		if(FreeLoginCheck( clifd )== 0){
			//lssproto_CharList_send( clifd, FAILED, "你的遊戲賬號登陸已上限，請不要過量進入遊戲！" );
			return;
		}
#endif
#endif
...
void saacproto_ACCharLoad_recv(int fd , char* result,char* data,int retfd,int saveindex)
{
    ...
    if(( strcmp( result , SUCCESSFUL ) == 0 )&&(data[0])) {
		...
		char cdkey[64];
#ifndef _FIX_MESSAGE             // WON ADD 修改WORKER封包內容
		data = "";		
#endif
		CONNECT_getCdkey( clifd, cdkey , sizeof( cdkey ));
		print(" (%s)ACCharLoad錯誤:%s ", cdkey, data);
...
void saacproto_ACCharSave_recv(int fd ,char* result, char* data,int retfd)
{
    ...
    case WHILECLOSEALLSOCKETSSAVE:
        ...
        if( SERVSTATE_getCloseallsocketnum() == 0 ) {
            ...
            print( "所有人物存檔\n" );
            print("\n 關閉服務器時間 =%d",i_shutdown_time);//ttom
...
{
  // 更新這個 server 的 莊園 pk scheduleman
  int i;
  char token[256], skip[256];
...
#endif
				{
					CHAR_talkToCli(i, -1, "因帳號重復登陸而掉綫!",CHAR_COLORYELLOW);
					...
					LogCharOut(charname,cdkey,__FILE__,__FUNCTION__,__LINE__,"因封包異常而斷綫");
#endif
			...
			for(i=0;i<MAX_USER;i++){
				...
				if(i_use){
					...
					if( !strcmp( cdkey, data) ){
						...
						if( retfd == -1 ){
							print("因重復登入");
							CHAR_talkToCli( fd_charaindex, -1, "因重復帳號登入而斷綫！",CHAR_COLORYELLOW);
						}else{
							print("因做壞事 ");
							CHAR_talkToCli(fd_charaindex, -1, "因做壞事而斷綫。",CHAR_COLORYELLOW);
						}
...
void saacproto_ACCharGetPoolItem_recv(int fd ,char* result, char* data,int retfd, int meindex) 
{
	...
	if( CHAR_CheckDepotItem( charaindex) ) return;//倉庫已存在
	...
	if( CHAR_makeDepotItemStringToChar( charaindex, data) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "道具倉庫資料讀取失敗！", CHAR_COLORYELLOW);
...
void saacproto_ACCharGetPoolPet_recv(int fd ,char* result, char* data,int retfd, int meindex) 
{
	...
	if( !CHAR_CHECKINDEX( charaindex) ) return;
	clifd = getfdFromCharaIndex( charaindex);
	if( CHAR_CheckDepotPet( charaindex) ) return;//倉庫已存在
	...
	if( CHAR_makeDepotPetStringToChar( charaindex, data) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "寵物倉庫資料讀取失敗！", CHAR_COLORYELLOW);
...
void saacproto_ACMissionTable_recv( int fd, int num, int type, char *data, char* angelinfo)
{
	...
	if( type == 1 ) { // get data list
		...
		sprintf( msgbuf, "收到AC迴應(add data) num=%d nameinfo=%s ", num, angelinfo);
		print( msgbuf);
		LogAngel( msgbuf);
		...
		CHAR_talkToCli( angelindex, -1, "精靈正在召喚你。", CHAR_COLORYELLOW);
	}
	else if( type == 3) {
		if( num <0 || num >= MAXMISSIONTABLE)	return;

		sprintf( msg, " AC清除 任務:%d %s %s ", num, missiontable[num].angelinfo, missiontable[num].heroinfo);
		print( msg);
		LogAngel( msg);
...
void saacproto_ACCharLogin_recv(int fd , int clifd, int flag)
{
	...
	if(flag==0){
		...
		lssproto_CharList_send( clifd, FAILED, "賬號或密碼不能為空！" );
	}else if(flag==2){
		lssproto_CharList_send( clifd, FAILED, "你的遊戲賬號被鎖定，無法登陸！" );
	}else if(flag==3){
		lssproto_CharList_send( clifd, FAILED, "你的遊戲IP已被鎖定，無法登陸！" );
	}else if(flag==4){
		lssproto_CharList_send( clifd, FAILED, "你的遊戲賬號登陸已上限，請不要過量進入遊戲！" );
	}else if(flag==5){
		lssproto_CharList_send( clifd, FAILED, "您的賬號尚未注冊，請登陸www.09sa.com進行注冊！" );
	}else if(flag==6){
		lssproto_CharList_send( clifd, FAILED, "您的賬號或密碼錯誤！" );
	}
}
...
void saacproto_QueryPoint_recv(int fd , int point)
{
	...
	char token[64];
	sprintf( token, "你目前的個人重迴幣數為：%d", point);
...
void saacproto_NewVipShop_recv( int fd, int point, char* buf, int flag)
{
	int charaindex = CONNECT_getCharaindex(fd);
	if(point == -1){
		CHAR_talkToCli( charaindex, -1, "你的重迴幣不足！",  CHAR_COLORYELLOW);
		return;
	}
	if(strcmp(buf, "") == 0){
		CHAR_talkToCli( charaindex, -1, "沒有該項物品數據！",  CHAR_COLORYELLOW);
		return;
	}
	char token[256];
	if(flag==0){
	  ...
	  itemindex = CHAR_findEmptyItemBox( charaindex );
		if( itemindex < 0 )	{
			CHAR_talkToCli( charaindex, -1, "物品欄空間不足！！",  CHAR_COLORYELLOW);
			return;
		}
...
extern int CheckCharMaxItem(int charindex);
	  ...
	  if( i == CHAR_MAXPETHAVE )      {
		...
		if( i == CHAR_MAXPETHAVE )
			CHAR_talkToCli( charaindex, -1, "領取寵物失敗！", CHAR_COLORYELLOW);
	...
	snprintf( token,sizeof( token), "您當前重迴幣剩餘 %d", point);
	CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORYELLOW);
}
...
void saacproto_OnlineBuy_recv(int fd, char *data)
{
	...
	if(data[0]=='0'){
		...
		if( i == enemynum ){
			CHAR_talkToCli( charaindex, -1, "此寵物不存在，請與本服管理員聯係！", CHAR_COLORGREEN);
			return;
		}
	
		ret = ENEMY_createPetFromEnemyIndex(charaindex, i);
		if( !CHAR_CHECKINDEX( ret)){
			CHAR_talkToCli( charaindex, -1, "領取寵物失敗", CHAR_COLORGREEN);
			return;
		}
		snprintf( token,sizeof( token), "領取寵物 %s ！",ENEMY_getChar( i, ENEMY_NAME));
		CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORGREEN);
		for( i = 0; i < CHAR_MAXPETHAVE; i ++ ){
			if( CHAR_getCharPet( charaindex, i ) == ret )break;
		}
		if( i == CHAR_MAXPETHAVE ){
			CHAR_talkToCli( charaindex, -1, "您的寵物已滿！", CHAR_COLORGREEN);
			return;
		}
	  ...
	  int emptyitemindexinchara, itemindex;
		...
		if( emptyitemindexinchara < 0 ){
			CHAR_talkToCli( charaindex, -1, "您的物品已滿！", CHAR_COLORYELLOW);
			return;
		}
		...
		if( itemindex != -1 ){
			...
			snprintf( token, sizeof( token), "領取物品 %s", ITEM_getChar( itemindex, ITEM_NAME));
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORGREEN);
		}else{
			CHAR_talkToCli( charaindex, -1, "此物品不存在，請與本服管理員聯係！", CHAR_COLORYELLOW);
		}
...
void saacproto_LotterySystem_recv( char *data)
{
	...
	for( i = 0 ; i < playernum ; i++) {
		if( CHAR_getCharUse(i) != FALSE ) {
			char token[256];
			sprintf(token, "%04d%02d%02d期期中奬號碼是%0d,%0d,%0d,%0d,%0d,%0d,%0d", now.tm_year+1900,
																																							now.tm_mon+1,
																																							now.tm_mday,
																																							todayaward[0],
																																							todayaward[1],
			...
			CHAR_talkToCli( i, -1, "請中奬的朋友到NPC上換兌奬品！", CHAR_COLORGREEN);
		}
	}
}
#endif
