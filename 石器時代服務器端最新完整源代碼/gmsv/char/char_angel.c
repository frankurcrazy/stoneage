...
void selectAngel( int charaindex, int heroindex, int mission, int gm_cmd)
{
	...
	if( mission == -1 )
	{
		int mlist[MAXMISSION]; // 符閤條件的任務列錶
		char eventlist[1024];
		char buf[64];
		int i, j, mindex =0;
	...
	{
		sprintf( msg, " 産生 %s 天使候補: %s %s Lv:%d 勇者候補: %s %s Lv:%d 任務:%d ci=%d hi=%d ",
				gm_cmd ? ("(GM指令)") : (" "),
	...
	lssproto_WN_send( getfdFromCharaIndex(angelindex), WINDOW_MESSAGETYPE_MESSAGE, 
		...
		for( i =0; i <maxchar; i++)
		{
			...
			if( ADDRESSBOOK_AutoaddAddressBook( angelindex, i) == TRUE ){
				char token[256];
				sprintf( token, "係統自動為你和英雄 %s 交換名片。", CHAR_getChar( i, CHAR_NAME ));
				CHAR_talkToCli( angelindex, -1, token, CHAR_COLORRED);
			}
			
			if( ADDRESSBOOK_AutoaddAddressBook( i, angelindex) == TRUE ){
				char token[256];
				sprintf( token, "係統自動為你和使者 %s 交換名片。", CHAR_getChar( angelindex, CHAR_NAME ));
	...
	if( !strcmp( nameinfo, missiontable[mindex].angelinfo) ) { // 使者使用時
		...
		for( i =0; i <maxchar; i++)
		{
			...
			if( checkUnlawWarpFloor( fl) ) {
				CHAR_talkToCli( charaindex, -1, "對象所在的地方無法傳送。", CHAR_COLORYELLOW );
				return;
			}
...
//	print(" 使用勇者信物 ");
	...
	if( !strcmp( nameinfo, missiontable[mindex].heroinfo) ) { // 勇者使用時
		...
		for( i =0; i <maxchar; i++)
		{
			...
			if( checkUnlawWarpFloor( fl) ) {
				CHAR_talkToCli( charaindex, -1, "對象所在的地方無法傳送過去。", CHAR_COLORYELLOW );
				return;
			}
