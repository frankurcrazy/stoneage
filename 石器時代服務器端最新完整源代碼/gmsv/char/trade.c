...
#define TRADE_LOCKPETITEM	"寵物身上有綁定的道具無法交易！"
#endif
#define TRADE_NOITEM	"存在無法交易的物品！"
#define TRADE_NOPET	"存在無法交易的寵物！"
...
#ifdef _LOCK_PET_ITEM
{
			...
			if( petname[0] == '*' ){
				CHAR_talkToCli( meindex, -1, "綁定寵物無法交易！", CHAR_COLORYELLOW);
				return FALSE;
			}
#ifdef _PET_ITEM
{
			...
			for( i = 0 ; i < CHAR_MAXPETITEMHAVE ; i ++ ){
				...
				char *arg = ITEM_getChar(itemindex, ITEM_NAME);
				if(arg[0] == '*'){
					CHAR_talkToCli( meindex, -1, "寵物身上有綁定的道具無法交易！", CHAR_COLORYELLOW);
					return FALSE;
				}
			}
}
   ...
   CHAR_getCoordinationDir(CHAR_getInt(meindex, CHAR_DIR),
	...
	if (cnt == 1){
		...
		if(CHAR_getInt(meindex, CHAR_LOCKED)==1){
			...
			char message[256];
			sprintf( message, "對方 %s 安全鎖定，無法與你交易！\n", CHAR_getChar(toindex, CHAR_NAME));
		...
		if( (ti = TRADE_getTradeListIndex()) < 0 ||
			...
			CHAR_talkToCli( meindex, -1, "係統忙碌中。", CHAR_COLORYELLOW);
			CHAR_setWorkInt( meindex, CHAR_WORKTRADEMODE, CHAR_TRADE_FREE);
		...
		//TRADE_setTradeListIndex( ti, meindex, toindex); //重復寫，去掉
#endif
		snprintf(msgbuf, sizeof(msgbuf), TRADE_WAIT, tocharaname);
   		CHAR_talkToCli(meindex, -1, msgbuf, CHAR_COLORYELLOW);    //主動交易方,發送連絡xx中
...
void TRADE_Will(int fd, int meindex, char* message)
{
	...
	if (*message == 0)	return;
	strcpy(mycharaname, CHAR_getChar(meindex, CHAR_NAME));
	toindex = TRADE_getMyTarget( meindex);//獲取跟自己交易的對像？
	if( CHAR_CHECKINDEX( toindex) ){
		...
		if(strcmp( msg, "0" )==0)
		{
			...
			CHAR_talkToCli(toindex, -1, msgbuf, CHAR_COLORYELLOW);    //拒絕交易
   ...
   CHAR_talkToCli(toindex, -1, msg, CHAR_COLORYELLOW);   
	...
	if (result == 1){
			if(CHAR_charSaveFromConnect(meindex, FALSE)){
				CHAR_talkToCli(meindex, -1, "係統自動為您存檔!", CHAR_COLORRED);
			}
...
BOOL TRADE_HandleTradeForPK( STradeList *temp1, STradeList *temp2 )
{
	...
	CHAR_talkToCli( meindex, -1, "恭喜您！您贏得對方物品！",  CHAR_COLORWHITE);
	CHAR_talkToCli( toindex, -1, "很遺憾！對方贏得您的物品！",  CHAR_COLORWHITE);
