...
#ifdef _LOCK_PET_ITEM
{
			...
			if( petname[0] == '*' ){
	   	CHAR_talkToCli(cindex, -1, "綁定寵物無法傳遞郵件！", CHAR_COLORYELLOW);
	   	return	FALSE;
					}
}
#endif
...
if (getPetMailFlg()==1){
	int flg = 0;
	for (i = 0; i < 30; i++) {
		if (CHAR_getInt(petindex, CHAR_PETID) == getPetMailPetid(i)) {
			flg = 1;
			break;
		}
	}
	if (flg == 0) {
		CHAR_talkToCli(cindex, -1, "該寵物無法寄送寵物郵件！", CHAR_COLORYELLOW);
		return FALSE;
	}
}
#endif
			...
			if(arg[0] == '*'){
				CHAR_talkToCli( cindex, -1, "綁定物品無法傳遞", CHAR_COLORYELLOW );
				return	FALSE;
			}
#endif
// Terry add fix can send mail to offline character 2004/2/5
	...
	if(i == playernum){
		CHAR_talkToCli(cindex, -1, "該玩傢不在此星球上！", CHAR_COLORYELLOW);
   	return	FALSE;
	}
// end
#ifdef _MAP_TIME
	...
	if( ( CHAR_getInt(recvindex,CHAR_FLOOR) == 1042
			|| CHAR_getInt(recvindex,CHAR_FLOOR) == 2032
			|| CHAR_getInt(recvindex,CHAR_FLOOR) == 3032
			|| CHAR_getInt(recvindex,CHAR_FLOOR) == 4032) )
	{
		char	msgbuf[512];
		snprintf( msgbuf, sizeof( msgbuf), "對方在族戰戰場，不得寄送道具！");
		CHAR_talkToCli( cindex, -1, msgbuf, CHAR_COLORRED);
		return FALSE;
	}
		...
		if( CHAR_getWorkInt( cindex, CHAR_PETMAILNUMS) > 6 ){
			...
			char Mess1[256];
			sprintf( Mess1,"需等待%d秒纔能再次寄送寵物郵件！",
			...
			CHAR_talkToCli(cindex, -1, "目前係統郵件過多，請稍後再寄。", CHAR_COLORYELLOW);
			return	FALSE;
		}
#endif
	{
		...
		if( haveitemindex != -1 ) {
			...
			if(CHAR_charSaveFromConnect(cindex, FALSE)){
				CHAR_talkToCli(cindex, -1, "係統自動為您存檔!", CHAR_COLORRED);
			}
#endif
		}
		{
...
void PETMAIL_Loopfunc( int index)
{
	...
	switch( mode ){
	  ...
	  case CHAR_PETMAIL_IDLE2:
		...
		break;
	  case CHAR_PETMAIL_RETURNWAIT://迴來等待
...
static int PETMAIL_getIdleTime( int index)
{
#define		PETMAIL_DIVRANGE		25		//	坌喃允月r
  	...
  	unsigned int t = CHAR_getInt( index, CHAR_PETMAILIDLETIME);
	{
		...
		if( tocharaindex != -1 ) {
			...
			if( NowTime.tv_sec > t + PETMAIL_IDLE_RETURNOWNER ) {//檢查等待主人時間
				int itemindex;
				CHAR_warpToSpecificPoint( index, //迴到原先主人所在座標
...
void CHAR_AutoPickupMailPet( int charaindex, int petindex )
{
	...
	if( havepetindex == -1 ) {
		...
		CHAR_talkToCli(charaindex,-1,"寵物欄已滿！無法自動拾迴郵件寵物！",CHAR_COLORYELLOW);
		// WON ADD
		LogPet(
	...
	if( PetMailTotalnums < 0 ) PetMailTotalnums = 0;
	{
		...
		snprintf( mesg,sizeof(mesg), "拾迴郵件寵物 %s", p);
