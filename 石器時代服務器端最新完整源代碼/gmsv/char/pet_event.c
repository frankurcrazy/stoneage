...
#define RAND(x,y)   ((x-1)+1+ (int)( (double)(y-(x-1))*rand()/(RAND_MAX+1.0)) )
  ...
  j=0;
	...
	while( getStringFromIndexWithDelim( buf1,"OVER",talkNo, buf2, sizeof( buf2) ) != FALSE){
		...
		if( NPC_ActionPassCheck( meindex, talkerindex, buf3) == FALSE )	continue;//判斷玩傢FREE條件
                           ...
                           int* opt, int optlen )
{
	...
	if( CHAR_getInt( petindex, CHAR_MAILMODE) == CHAR_PETMAIL_IDLE3 ||
		...
		if( act == CHAR_ACTWALK && CHAR_CHECKINDEX( workpindex) && workpindex == pindex ){
			...
			|| CHAR_getWorkInt( pindex, CHAR_WORKPETFOLLOW)==petindex
			|| CHAR_getWorkInt( pindex, CHAR_WORKPETFOLLOW1)==petindex
			|| CHAR_getWorkInt( pindex, CHAR_WORKPETFOLLOW2)==petindex
			|| CHAR_getWorkInt( pindex, CHAR_WORKPETFOLLOW3)==petindex
			|| CHAR_getWorkInt( pindex, CHAR_WORKPETFOLLOW4)==petindex
#endif
			) {
			switch( act) {
			  case CHAR_ACTATTACK:
		...
		if( CHAR_getInt( petindex, CHAR_MAILMODE) != CHAR_PETMAIL_NONE) {
			...
			if( NowTime.tv_sec >= (petputtime + 6000*6000) ) {
