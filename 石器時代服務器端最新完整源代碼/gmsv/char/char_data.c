    ...
    /* 賡渝祭 */
{
    ...
    print( "有效不可戰鬥有效數是 %d...", CHAR_invareanum );
    ...
    }
{
    ...
    print( "有效顯示位置範圍數是 %d..", CHAR_appearnum );
#ifdef DEBUG
...
*/
#endif
static int LevelUpTbl[] = { // 經驗值 等級 換算錶
0,			//  0 EX=(lv^4)/10
0,			//  1
	        2,        8,       25,       62,      129,
					...
					if (CHAR_getInt(ownerindex, CHAR_FMLEADERFLAG) > 0
...
#ifdef _NEW_MANOR_LAW
						{
							...
							for(i=0;i<FAMILY_MAXHOME;i++){
								// 任何一個莊園有在挑戰排程,氣勢值一改變就嚮AC要最新氣勢資料
...
int PETFUSION_FusionPetSub( int charaindex, int Subindex1, int Subindex2, int *work, int *skill)
{
	...
	if( PET_getBaseAndSkill( charaindex, Subindex1, base, petskill, 0) == FALSE )
		return 0;
	if( CHAR_getInt( Subindex1, CHAR_LV) < 120 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.7;
		}
	}else	if( CHAR_getInt( Subindex1, CHAR_LV) < 140 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.8;
		}
	}
	...
	if( PET_getBaseAndSkill( charaindex, Subindex2, base, NULL, 0) == FALSE )
		return 0;
	if( CHAR_getInt( Subindex2, CHAR_LV) < 120 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.7;
		}
	}else if( CHAR_getInt( Subindex2, CHAR_LV) < 140 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.8;
		}
	}
...
BOOL PETFUSION_FusionPetMain( int charaindex, int Mainindex, int *work, int *skill)
{
	...
	if( PET_getBaseAndSkill( charaindex, Mainindex, base, petskill, 0) == FALSE )
		return -1;
	if( CHAR_getInt( Mainindex, CHAR_LV) < 120 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.7;
		}
	}else if( CHAR_getInt( Mainindex, CHAR_LV) < 140 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.8;
		}
	}else
...
void CHAR_PlayerRide( int charaindex )
{
		int i,j;
		int MetamoList[5][13]={
		/*{ 小矮子   賽亞人  辮子男孩  酷哥   熊皮男   大個    小矮妹  熊皮妹  帽子妹  短發夾妹  手套女   辣妹    虎}, 此行為說明行 */
			{ 100000, 100025, 100055, 100060, 100095, 100100, 100135, 100145, 100165, 100190, 100200, 100230, 2483},	//紅
			{ 100005, 100030, 100050, 100065, 100085, 100115, 100120, 100140, 100170, 100195, 100210, 100225, 2481},	//綠
			{ 100010, 100035, 100045, 100070, 100090, 100110, 100125, 100150, 100160, 100185, 100215, 100220, 2484},	//黃
			{ 100015, 100020, 100040, 100075, 100080, 100105, 100130, 100155, 100175, 100180, 100205, 100235, 2482},	//灰
