...
//#define _NEW_SA80                // 新石器8.0
	...
	#define _VERSION_80
			...
			#define _ONLINE_SHOP             // 8.0在綫商城
		#endif
		#define _ONLINE_TALK_IP          // 玩傢舉報
		#define _ONLINE_TALK_GM          // 在綫聯係GM
		#define _NEW_ONLINE_SHOP         // 新在綫商城
	#endif
	...
	#define _CHAR_NEWLOGOUT          // (可開放) ANDY 登齣新機製
	...
	#define _PET_FUSION              // (可開放) ANDY 寵物融閤 enemybase1.txt
	#define _NPC_FUSION              // (可開放) ANDY NPC寵物融閤
	...
	#define _SHOW_FUSION             // (可開) Change 融閤寵物在CLIENT端隻顯示轉生數,修改為融閤寵顯示為融閤(client端也要開)
...
#define _PUB_ALL
	...
	#define _BATTLE_FLOOR            // 自定義強製戰鬥地圖
...
#define _VIP_ALL
	...
	//#define _FM_METAMO               // 傢族專用變身戒指
	#define _ITEM_METAMO_TIME        // 可自定義變身的戒指
	#define _SHOW_VIP_CF             // 自定義調整VIP，愛人，傢族，名稱顯示樣式
	#define _GM_KING                 // GM大魔王
	#define _ANGEL_TIME              // 自定義精靈召喚時間
	#define _FM_POINT_PK             // 莊園傢族互搶莊園
	#if _ATTESTAION_ID != 1
		#define _PLAYER_MOVE             // 玩傢順移功能
	#endif
	#define _BATTLE_PK               // 強製PK道具，輸瞭迴記錄點
	#ifdef _PET_FUSION
		#define _FUSIONBEIT_TRANS        // 融閤寵物轉生
		#define _FUSIONBEIT_FIX          // 取消融閤寵的限製
	#endif
	...
	#define _FM_JOINLIMIT            // 退族後再次加入傢族的時間限製
	...
	#define _ONLINE_COST             // 在綫充值(需SAAC配閤)
			...
			//#define _TRANS_POINT_UP          // 每轉點數上限
				...
				#define _INSLAY_NOT_PILENUMS     // 不能精工疊加物品
					...
					#define _NPC_INT_SET           // NPC整型設置
						...
						#define _PLAYER_QUESTION_ONLIEN  // 玩傢在綫答題
					#endif
				#endif
							...
							#define _FILTER_TALK    		   // 過濾說話
									...
									#define _ALL_SERV_SEND       // 星球頻道
										...
										#define _PET_TRANS_ABILITY       // 寵物轉生能力定義上限
											...
											#define _FIMALY_PK_TIME        // 傢族開片時間設定
										#endif
									#endif
							#endif
						#endif
...
//	#define MANOR_MAX_NUM 			2        // (4.0) 傢族據點
#else
...
//#define _CHARADATA_SAVE_SQL      //將數據寫入SQL
		...
		#define _SASQL                // 加入MYSQL功能
			...
			#define _ONLINE_SHOP_MYSQL       // 在綫商城改成MYSQL重迴幣
		#endif
...
//#define _ABSOLUTE_DEBUG              // 絕對調試
	...
	#define _FMRIDE_NOT_FMSPRITE	// 傢族騎寵不分光明黑暗
	#define _NOT_SHOW_ONLINE_NUM	// 不顯示在綫人數
	...
	//#define _UNIFIDE_MALINASI			// 統一瑪麗娜絲魚村齣生
	//#define _FIX_PET_SKILL				// 修復寵物技能
	//#define _NO1_B2_NONO					// B2學在第一位時自動鎖定21忠
	//#define _FMPOINT_APPEND_ABI		// 莊園傢族成員附加能力
...
//	#define _PET_VALIDITY  				// 寵物有效期
	//#define _PET_FUSION_LUA  			// 融閤寵LUA
	//#define _PET_ENEMY_DEVELOP_UP //全麵提升寵物成長
...
#define _REDUCE_MERGE_FAME				//降低無掛服的傢族閤成聲望等級所需聲望
#define _ALLDOMAN                // (可開放) Syu ADD 排行榜NPC
#define _ALLDOMAN_DEBUG          // (可開) Change 修正重啓server後排行榜消失bug
