		...
		case ONLINESHOP_PET:
			{
				...
				if(bj >= price * num){
					...
					for(j=0;j<num;j++){
						...
						if( i == enemynum ){
							CHAR_talkToCli( charaindex, -1, "該寵物不存在。", CHAR_COLORYELLOW);
						break;
						}
					
						int ret = ENEMY_createPetFromEnemyIndex(charaindex, i);
						if( !CHAR_CHECKINDEX( ret)){
							CHAR_talkToCli( charaindex, -1, "寵物欄位不足。", CHAR_COLORYELLOW);
						break;
						}else{
...
#endif
							char token[256];
							sprintf(token, "成功購買 %s 寵物", CHAR_getChar(ret, CHAR_NAME));
							CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
						}
					}
				}else{
					CHAR_talkToCli( charaindex, -1, "你的點數不足!", CHAR_COLORYELLOW);
				}
			}
			break;
				...
				if(bj >= price * num){
					...
					for(i=0;i<num;i++){
						...
						if( emptyitemindexinchara < 0 ){
							CHAR_talkToCli( charaindex, -1, "物品欄位不足。", CHAR_COLORYELLOW);
							break;
						}
...
#endif
							char token[256];
							sprintf(token, "成功購買 %s 道具", ITEM_getChar(itemindex, ITEM_NAME));
							CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
						}else{
							CHAR_talkToCli( charaindex, -1, "該物品不存在!", CHAR_COLORYELLOW);       
							break;
						}
					}
				}else{
