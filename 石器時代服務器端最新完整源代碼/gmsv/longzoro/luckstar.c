				...
				if(luckstar[luckid].pet > 0){
					...
					if( i != enemynum ){
						...
						if( CHAR_CHECKINDEX( ret)){
							snprintf( token,sizeof( token), "恭喜你，獲得本次幸運贈送寵物 %s ！",ENEMY_getChar( i, ENEMY_NAME));
							...
							CHAR_talkToCli( charaindex, -1, "很抱歉，由於你身上寵物已滿，已錯過這次幸運贈送寵物！",  CHAR_COLORYELLOW);
						}
						snprintf( strPet,sizeof( strPet), "寵物：%s", ENEMY_getChar( i, ENEMY_NAME));
					}else{
						snprintf( strPet,sizeof( strPet), "寵物：編號 %d 不存在", luckstar[luckid].pet);
					}
				}
						...
						if( emptyitemindexinchara >= 0 ){
							if( itemindex != -1 ){
							  ...
							  CHAR_sendItemDataOne( charaindex, emptyitemindexinchara);
								...
								snprintf( token, sizeof( token), "恭喜你，獲得本次幸運贈送物品 %s", ITEM_getChar( itemindex, ITEM_NAME));
								CHAR_talkToCli( charaindex, -1,token, CHAR_COLORGREEN);
							}
						}else{
							CHAR_talkToCli( charaindex, -1, "很抱歉，由於你身上物品已滿，已錯過這次幸運贈送物品！",  CHAR_COLORYELLOW);
						}
						snprintf( strItem,sizeof( strItem), "物品：%s", ITEM_getChar( itemindex, ITEM_NAME));
					}else{
						snprintf( strPet,sizeof( strPet), "物品：編號 %d 不存在", luckstar[luckid].item);
					}
				}
/*
				if(luckstar[luckid].gold > 0){
					...
					snprintf( strGold,sizeof( strGold), "石幣：%d", luckstar[luckid].gold);
					sprintf( token, "恭喜你，獲得本次幸運贈送 %d 石幣！", luckstar[luckid].gold);
				...
				if(luckstar[luckid].vippoint > 0){
					...
					snprintf( strVippoint,sizeof( strVippoint), "積分點：%d", luckstar[luckid].vippoint);
					sprintf( token, "恭喜你，獲得本次幸運贈送 %d 積分點！", luckstar[luckid].vippoint);
					CHAR_talkToCli( charaindex, -1,token, CHAR_COLORPURPLE );
				}
*/
				break;
		...
		if(k < 3){
			sprintf(token, "讓我們一起祝賀 %s 成為本次幸運星玩傢~獲得以下奬品：", CHAR_getChar( charaindex, CHAR_NAME));
		}else{
			sprintf(token, "很遺憾，本次幸運星未産生~祝願您成為下一次幸運星獲得者！");
		}
