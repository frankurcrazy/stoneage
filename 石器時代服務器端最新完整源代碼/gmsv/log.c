...
)
{
	...
	sprintf( buf, " (%d-%d-%d %d:%d:%d) " , tm1.tm_year + 1900,
																					...
																					tm1.tm_sec);
	if( flg == 0 ) {
 		printl( LOG_PKCONTEND, "[%32s 勝 %32s],(%5d,%4d,%4d)%s" ,
			teamname1, teamname2, 
			floor, x, y, buf);
	}else{
...
/*------------------------------------------------------------
 * 澀爛卞仄凶互勻化允屯化及白央奶夥毛田永弁失永皿
 * 白央奶夥反弁夫□術今木化中卅仃木壬弁夫□術允月
 * 婁醒  struct tm
 *  卅仄
 * 忒曰襖
 *  左□皿件仄凶白央奶夥及醒
...
void LogPetFeed( 
	...
	sprintf( buf, " (%d-%d-%d %d:%d:%d) " , tm1.tm_year + 1900,
 	...
 	printl( LOG_PET, "%s\t%s\t%s:%d 喂蛋=%s (%d,%d,%d)%s %s " , 
...
void LogCharOut(
	...
	memcpy( &tm1, localtime( (time_t *)&NowTime.tv_sec), sizeof( tm1));
	char buf[512];
	snprintf(buf,sizeof(buf),"(%d年%d月%d日 %d:%d) 帳號:%s 遊戲名:%s 文件:%s 函數:%s 行數:%d 原因:%s",
