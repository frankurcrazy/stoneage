...
void NPC_PoolItemShop_printWindow_Start( int meindex, int talkerindex)
{
	...
	if (CHAR_getWorkInt(talkerindex, CHAR_WORKTRADEMODE) != CHAR_TRADE_FREE){
		CHAR_talkToCli( talkerindex, -1, "交易中無法使用道具倉庫!",  CHAR_COLORRED);
	   return;
	}
#ifdef _ROOKIE_ITEM
	if( CHAR_getInt( talkerindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
...
{
	char token[256];
	sprintf( token, "倉庫道具%s，編碼重復！(係統清除)",
...
#ifdef _SAMETHING_SAVEPOINT
{
	if(CHAR_charSaveFromConnect(talkerindex, FALSE)){
		CHAR_talkToCli(talkerindex, -1, "係統自動為您存檔!", CHAR_COLORRED);
	}
	  ...
	  for( j= 0;j<AUTODELITEMNUM;j++){
	  	itemid = ITEM_getInt( itemindex, ITEM_ID);
			if( itemid != -1 && itemid == getAutoDelItem(j) ) {
				...
				ITEM_endExistItemsOne( itemindex);
				CHAR_talkToCli(talkerindex,-1,"係統清除你身上的物品",CHAR_COLORRED);
				return TRUE;
			}
		}
}
	...
	if( ITEM_getInt( itemindex, ITEM_DROPATLOGOUT) || // 登齣後消失
		...
		if( getPoolItemBug()==2 || getPoolItemBug()==3 )
		{
			if(getPoolItemBug()==3){
				int i;
				for(i = 0; i < 32; i ++){
					if(getPoolItem(i) == ITEM_getInt( itemindex, ITEM_ID)){
						break;
					}
				}
				if(i == 32){
