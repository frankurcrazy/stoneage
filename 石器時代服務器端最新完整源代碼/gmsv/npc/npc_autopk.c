								...
								int select, char *data)
{
	...
	switch( seqno)	{
		...
		case NPC_AUTOPK_JOINPK:
			if(autopktime>2){
				...
				char buf2[255];
#ifdef _NEW_AUTO_PK				
				if(CHAR_getInt(talkerindex,CHAR_TRANSMIGRATION)<getAutoPkTrans()){
					sprintf(buf1,"您的轉數小於%d轉，無法參加比賽！", getAutoPkTrans());
					CHAR_talkToCli( talkerindex, -1, buf1, CHAR_COLORYELLOW);
					break;
				}
				if(CHAR_getInt(talkerindex,CHAR_LV)<getAutoPkLv()){
					sprintf(buf1,"您的等級小於%d轉，無法參加比賽！", getAutoPkLv());
					CHAR_talkToCli( talkerindex, -1, buf1, CHAR_COLORYELLOW);
					break;
				}
#endif
				int i;
				CHAR_DischargePartyNoMsg( talkerindex);//解散團隊
				CHAR_warpToSpecificPoint( talkerindex, 20000, 27, 15 );
	#ifdef _GMRELOAD
				for (i = 0; i < GMMAXNUM; i++){
					if (strcmp( CHAR_getChar( talkerindex, CHAR_CDKEY), gminfo[i].cdkey) == 0){
						CHAR_setWorkInt(talkerindex,CHAR_WORK_AUTOPK,-1);
						break;
					}
				}
				if(i == GMMAXNUM)
	#endif
				{
					CHAR_setWorkInt(talkerindex,CHAR_WORK_AUTOPK,0);
					sprintf(buf1,"玩傢%s參加比賽！",CHAR_getChar( talkerindex, CHAR_NAME));
					sprintf(buf2,"玩傢%s參加比賽,你還沒參加嗎？本次奬品豐富，你還等什麼？快來吧！",CHAR_getChar( talkerindex, CHAR_NAME));
...
static void NPC_AutoPk_selectWindow( int meindex, int toindex, int num,int select)
{
	...
	case WINDOW_SELECT:
		...
		if(select==1){
			if(autopktime>2){
				sprintf(token, "              ★亂舞格鬥ＰＫ賽管理員★\n\n"
				               "當前離比賽開始時間還剩%d分鍾，你是否要參加當前比賽嗎？？？",autopktime);
				buttontype = WINDOW_BUTTONTYPE_YESNO;
				windowno = NPC_AUTOPK_JOINPK;
			}else if(autopktime>0){
				sprintf(token, "              ★亂舞格鬥ＰＫ賽管理員★\n\n"
				               "PK比賽將在%d分鍾後開始!\n現在已無法進入比賽場地！",autopktime);
				buttontype = WINDOW_BUTTONTYPE_CANCEL;
			}else{
				sprintf(token, "              ★亂舞格鬥ＰＫ賽管理員★\n\n"
				               "目前還未舉行PK比賽!\n請關注PK比賽最新消息！");
			...
			sprintf(token, "              ★亂舞格鬥ＰＫ賽參賽名單★\n");
			strcat(token, "轉生  等級    玩    傢\n");
			AutoPk_PKNum(token);
			buttontype = WINDOW_BUTTONTYPE_CANCEL;
			windowno = NPC_AUTOPK_PKLIST;
		}else if(select==3){
			char buff[64];
			sprintf(token, "              ★亂舞格鬥ＰＫ賽奬品領取★\n");
			strcat(token, "“獲勝”“奬　　　　　　　品”\n");
			if(strlen(award[0].awardpet)>0){
				sprintf(buff, "　冠軍　寵物：%s\n", award[0].awardpet);
			}else{
				sprintf(buff, "　冠軍　寵物：無\n");
			}
			strcat(token, buff);
			if(strlen(award[0].awarditem)>0){
				sprintf(buff, "　冠軍　物品：%s\n", award[0].awarditem);
			}else{
				sprintf(buff, "　冠軍　物品：無\n");
			}
			strcat(token, buff);
			if(strlen(award[1].awardpet)>0){
				sprintf(buff, "　亞軍　寵物：%s\n", award[1].awardpet);
			}else{
				sprintf(buff, "　亞軍　寵物：無\n");
			}
			strcat(token, buff);
			if(strlen(award[1].awarditem)>0){
				sprintf(buff, "　亞軍　物品：%s\n", award[1].awarditem);
			}else{
				sprintf(buff, "　亞軍　物品：無\n");
			}
			strcat(token, buff);
			if(strlen(award[2].awardpet)>0){
				sprintf(buff, "　季軍　寵物：%s\n", award[2].awardpet);
			}else{
				sprintf(buff, "　季軍　寵物：無\n");
			}
			strcat(token, buff);
			if(strlen(award[2].awarditem)>0){
				sprintf(buff, "　季軍　物品：%s\n", award[2].awarditem);
			}else{
				sprintf(buff, "　季軍　物品：無\n");
			}
			strcat(token, buff);
			if(strlen(award[3].awardpet)>0){
				sprintf(buff, "　優勝　寵物：%s\n", award[3].awardpet);
			}else{
				sprintf(buff, "　優勝　寵物：無\n");
			}
			strcat(token, buff);
			if(strlen(award[3].awarditem)>0){
				sprintf(buff, "　優勝　物品：%s\n", award[3].awarditem);
			}else{
				sprintf(buff, "　優勝　物品：無\n");
			strcat(token, buff);
			}
			...
			strcat(token, buff);
			if(baward == FALSE){
				if((!strcmp(ship[0].cdkey, CHAR_getChar( toindex, CHAR_CDKEY))
			...
			CHAR_setWorkInt( toindex, CHAR_WORK_LISTPAGE, 1);
			if(AutoPk_GetHistoryStr(token,CHAR_getWorkInt( toindex, CHAR_WORK_LISTPAGE))){
				buttontype = WINDOW_BUTTONTYPE_CANCEL;
			}else{
	...
	case WINDOW_HISTORY_PREV:
		windowtype = WINDOW_MESSAGETYPE_MESSAGE;
		CHAR_setWorkInt( toindex, CHAR_WORK_LISTPAGE,CHAR_getWorkInt( toindex, CHAR_WORK_LISTPAGE)-1);
		if(CHAR_getWorkInt( toindex, CHAR_WORK_LISTPAGE)<1)CHAR_setWorkInt( toindex, CHAR_WORK_LISTPAGE,1);
		if(AutoPk_GetHistoryStr(token,CHAR_getWorkInt( toindex, CHAR_WORK_LISTPAGE)) || CHAR_getWorkInt( toindex, CHAR_WORK_LISTPAGE)==1)
			buttontype = WINDOW_BUTTONTYPE_CANCEL|WINDOW_BUTTONTYPE_NEXT;
		else
			buttontype = WINDOW_BUTTONTYPE_CANCEL|WINDOW_BUTTONTYPE_PREV|WINDOW_BUTTONTYPE_NEXT;
		windowno = NPC_AUTOPK_HISTORY;
...
BOOL AutoPk_GetHistoryStr( char *token, int page )
{
	...
	if(strlen(history[4].name)==0)
		strcpy(history[4].name,"無");
	sprintf(token, "              ★亂舞格鬥ＰＫ賽曆史★\n"
	               "             第 %s 屆亂舞格鬥獲奬名單\n",hstime);
	strcat(token, "“名次”“得分”“玩    傢”\n");
	sprintf(buf,  "　冠軍%7d     %s\n"
								"　亞軍%7d     %s\n"
								"　季軍%7d     %s\n"
								"　優勝%7d     %s\n"
...
void AutoPk_PKNum( char *token )
{
	...
	sprintf(buf, "當前參加PK比賽人數為:%d人",num);
	strcat(token,buf);
}
...
void AutoPk_PKSystemInfo()
{
	...
	if(autopktime>0){
		char buf1[64];
		char buf2[128];
		sprintf(buf1, "距離截止參加PK比賽時間還剩%d分鍾！",autopktime);
		if(awardpetid!=-1 && awarditemid==-1)
			sprintf(buf2,"距離截止參加PK比賽時間還剩%d分鍾，本次比賽奬品為:編號:%d,寵物:%s",autopktime-2,awardpetid,awardpet);
		else if(awardpetid==-1 && awarditemid!=-1)
			sprintf(buf2,"距離截止參加PK比賽時間還剩%d分鍾，本次比賽奬品為:編號:%d,物品:%s",autopktime-2,awarditemid,awarditem);
		else if(awardpetid!=-1 && awarditemid!=-1)
			sprintf(buf2,"距離截止參加PK比賽時間還剩%d分鍾，本次比賽奬品為:編號:%d,寵物:%s和編號:%d,物品:%s",autopktime-2,awardpetid,awardpet,awarditemid,awarditem);
		else
			sprintf(buf2, "距離正式PK比賽時間還剩%d分鍾！",autopktime);
		AutoPk_PKSystemTalk(buf1, buf2);
	}else if(autopktime==0){
		...
		if(num>1){
			AutoPk_PKSystemTalk("比賽正式開始咯！","比賽正式開始咯！");
			setAward(TRUE);
		}else{
			...
			AutoPk_PKSystemTalk("由於參加比賽人數過少，所以取消比賽！","由於參加比賽人數過少，所以取消比賽！");
			baward = FALSE;
		}
	}
}
...
void AutoPk_ChampionShipSet( int charaindex, int winnum, int win )
{
	...
	if(win==0){
		...
		if(winnum>ship[0].winnum){
			sprintf( token, "%s排名第一，殺人得分總數%d分！", name, winnum);
			...
			sprintf( token, "%s排名第二，殺人得分總數%d分！", name, winnum);
			AutoPk_PKSystemTalk(token,token);
			...
			sprintf( token, "%s排名第三，殺人得分總數%d分！", name, winnum);
			AutoPk_PKSystemTalk(token,token);
...
void AutoPk_GetChampionShip( void )
{
	...
	if(strlen(ship[0].cdkey)>0){
		sprintf( token, "恭喜%s獲得本次PK比賽冠軍，殺人得分總數%d分！", ship[0].name, ship[0].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[1].cdkey)>0){
		sprintf( token, "恭喜%s獲得本次PK比賽亞軍，殺人得分總數%d分！", ship[1].name, ship[1].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[2].cdkey)>0){
		sprintf( token, "恭喜%s獲得本次PK比賽季軍，殺人得分總數%d分！", ship[2].name, ship[2].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	sprintf( token, "恭喜%s和%s獲得本次PK比賽優勝奬！", ship[3].name,ship[4].name);
	AutoPk_PKSystemTalk(token,token);
...
BOOL AutoPk_AddItem( int charindex, int index )
{
  ...
  itemindex = CHAR_findEmptyItemBox( charindex );
	if( itemindex < 0 )	{
		CHAR_talkToCli( charindex, -1, "物品欄空間不足！！",  CHAR_COLORYELLOW);
		return FALSE;
	}
