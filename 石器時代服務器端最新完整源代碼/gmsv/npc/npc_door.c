 ...
 *
 * passflg 反｝1分勻凶日天允歹□升勾五匹丐月啦  卞卅月［2分勻凶日
 * 濮覆夫日井卅中［ 0分勻凶日由旦伐□玉匹卅中［
 *
 * 毛隙爛允月［  引月引匹及凜棉反犯白巧夥玄匹反1凜棉［
...
#define NPC_DOOR_CLOSEMSG "玉失毛｝仄戶凶!"
...
BOOL NPC_DoorInit( int meindex )
{
...

    {
        ...
        case 11924: dir = 6; break;  /* 贏中嗡及玉失 夫分曰仄凶 */
...
void NPC_DoorPostOver( int meindex , int movedindex )
{
    if( CHAR_getWorkInt( meindex , CHAR_WORKDOORSOONFLG )){
        ...
        CHAR_setFlg( meindex , CHAR_ISOVERED , 0 );	/* 騷木卅仁允月 */
    }
}
...
#define		NPC_DOOR_EXPIRE_MSG		"%s 的房間有效期限至 %d/%d %d:%d"
        else {
    ...
    if( needkey >= 0 ){
        ...
        CHAR_talkToCli( lookedindex , -1 , "這扇門似乎怎麽也打不開！" , CHAR_COLORWHITE);
    }
}
void NPC_DoorTalked( int meindex , int talkerindex , char *msg , int color )
{
    ...
    if( strcmp( m , msg ) == 0 ){
        ...
        CHAR_talkToCli( talkerindex , -1 , "密碼不正確啦！", CHAR_COLORWHITE );
    }
}
