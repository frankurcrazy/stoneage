...
//瑪雷菲雅判斷 成立 寵物轉生 不成立 人物轉生
  ...
  for(i=0 ; i < CHAR_MAXPETHAVE ; i++)	{
    ...
    if( CHAR_getInt( petindex, CHAR_PETID) == 718
		...
		|| CHAR_getInt( petindex, CHAR_PETID) == 401
		  ...
		  if( CHAR_getInt( talkerindex, CHAR_LV) < 80 )	{
				CHAR_talkToCli( talkerindex, -1, "由於你的等級小於80,沒能力進行轉寵！",  CHAR_COLORWHITE);
				return;
		    ...
		    CHAR_talkToCli( talkerindex, -1, "清除你身上非法瑪雷菲雅", CHAR_COLORRED);
		    return;
		  }else	if( CHAR_getInt( petindex, CHAR_LV) == 79 ) {
				if( strcmp( CHAR_getChar( petindex, CHAR_OWNERCDKEY), CHAR_getChar( talkerindex, CHAR_CDKEY) ) ||
					...
					CHAR_talkToCli( talkerindex, -1, "你身上的瑪雷菲雅，並不是你的啊！",  CHAR_COLORWHITE);
					return;
				}
				petNum ++;
		  }
	...
	if( petNum > 1 )	{
		CHAR_talkToCli( talkerindex, -1, "你怎會有那麽多瑪雷菲雅呢？",  CHAR_COLORWHITE);
		return;
  }else if( petNum == 1 )	{
								...
								int seqno, int select, char *data)
{
	...
	switch( seqno) {
	  ...
	  if( CHAR_getInt(talkerindex,CHAR_TRANSMIGRATION) == 5){
...
#ifdef _TRANS_6
#ifdef _TRANS_7
	if(CHAR_getInt(	talker, CHAR_TRANSMIGRATION) >= 7){
		return 7;
	}
#else
...
//			if(CHAR_getInt( petindex, CHAR_LV) < PETLEVEL) continue;
	...
	else if( num == 5 )
	{
		...
		int i,j,k;
		for(j = 0; j < 4; j++){
			int petidfinal[4] = {2474, 1610, 2589, 1845};
			//隙爛矢永玄毛手勻化中月井矢永玄民尼永弁
			for(i=0 ; i < CHAR_MAXPETHAVE ; i++){
				petindex = CHAR_getCharPet( talker, i);
				if( petindex == -1  )  continue;
				...
				petwork[j] = petidfinal[j];
				break;
			}
		}
		for(k=0; k < 4 ;k++)
		{
			if(petwork[k] == -1) break;
		}
		if(k == 4){
			CHAR_talkToCli( talker,-1,"你沒有帶齊2轉的四大聖獸哦!",CHAR_COLORWHITE);
			return -1;
		}
...
#ifdef _TRANS_6
#ifdef _TRANS_7
	if(CHAR_getInt(	toindex, CHAR_TRANSMIGRATION) > 7)
		CHAR_setInt(toindex ,CHAR_TRANSMIGRATION,7);
	if( CHAR_getInt(toindex,CHAR_TRANSMIGRATION) == 7 )//6轉後所送的寵
		present = 9999;
#else
...
#ifdef _VIP_TRANS
	if(CHAR_getInt( toindex,CHAR_VIPRIDE )>0){
		return 20;
	}
#endif
	int i;
...
#ifdef _PET_TRANS
#ifdef _PROSK99
	int endevflg[]={69,70,71,72,109,118,122,131,127,158,159,181,-1};//181:白狼奬品
#else
#ifdef _CLR_PETTRANS_
	int endevflg[]={71,72,109,118,122,131,127,158,159,-1};
#else
	int endevflg[]={69,70,71,72,109,118,122,131,127,158,159,-1};
#endif
#endif	
#ifdef _CLR_PETTRANS_
	int nowevflg[]={71,72,-1};
#else
...
#ifdef _TRANS_6
#ifdef _TRANS_7
	int flg[7] = {48,49,50,51,52,179,999};
#else
...
#endif
#ifndef _NO_CLS_EVENT_255
#ifdef _ADD_NEWEVENT_1024
	CHAR_setInt(toindex,CHAR_ENDEVENT9,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT9,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT10,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT10,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT11,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT11,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT12,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT12,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT13,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT13,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT14,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT14,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT15,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT15,0);
	CHAR_setInt(toindex,CHAR_ENDEVENT16,0);
	CHAR_setInt(toindex,CHAR_NOWEVENT16,0);
#endif
#endif
#ifdef _PET_TRANS
	i=0;
...
#endif
	{
		NPC_EventSetFlg(toindex, 63); //不給玩傢解英雄的祝福
		NPC_EventSetFlg(toindex, 136);//不給玩傢解淚之石
...
void s_eventsetend( int charaindex, int shiftbit )
{
	...
	if( array>=event_num ){
		//print("錯誤！！所設的任務旗標編號已超過範圍(0~%d)。",32*event_num-1);
		return;
	}
