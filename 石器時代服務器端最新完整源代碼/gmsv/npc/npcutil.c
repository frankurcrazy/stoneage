  ...
  int x, y;
	for(i=1; i <=distance; i ++)
	{
		CHAR_getCoordinationDir( CHAR_getInt(index1,CHAR_DIR),
								 ...
								 i, &x, &y );
		if(CHAR_getInt(index2,CHAR_X) == x && CHAR_getInt(index2,CHAR_Y) == y){
			return TRUE;
		}
		...
		CHAR_getCoordinationDir( CHAR_getInt(index2,CHAR_DIR),
								 ...
								 i, &x, &y );
		if(CHAR_getInt(index1,CHAR_X) == x && CHAR_getInt(index1,CHAR_Y) == y){
			return TRUE;
		}
	}
#else
...
int NPC_Util_GiveAllItemToChar( int give , int take )
{
    ...
    int count=0;
#ifdef _NEW_ITEM_
	int itemMax = CheckCharMaxItem(take);
	for(i=0;i<itemMax;i++){
#else
	for( i=0 ; i<CHAR_MAXITEMHAVE ; i++ ){
#endif
