...
#define		NPC_SYSINFO_SHUTDOWN_MSG		"再過 %d 分鍾後，即開始進行server係統維護。"
...
static NPC_Sysinfo_Msg		msgtable[] = {
	...
	{ "shutdown_msg",		"現在開始要進行server係統維護請盡速logout！！"},
};
enum{
...
static void NPC_Sysinfo_Msg_Help( int meindex, int tindex, char *msg)
{
	...
	{ "shutdown min          min 代錶分。"},
	{ "msgcol col            設定傳送給玩傢訊息的顔色。顔色的指定有red,green,yellow,blue,purple,cyan,white。"},
	{ "sysinfo               係統狀態"},
	{ "info                  玩傢狀態"},
	{ "endinfo               迴復普通的狀態。"},
...
static void NPC_Sysinfo_Msg_Player( int meindex, int tindex, char *msg)
{
	...
	snprintf( buff, sizeof( buff), "玩傢有%d人。", cnt);
	...
	if( getStringFromIndexWithDelim( msg, " ", 2, buff, sizeof( buff)) == TRUE )
	{
		...
		for( i = 0; i < 7; i ++ ) {
			if( strstr( buff, colset[i].str) != NULL ) {
				...
				CHAR_talkToCli( tindex, meindex, "已改變訊息的顔色。",
								CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
				break;
			}
		}
...
static void NPC_Sysinfo_All_NoSee( int meindex, int tindex, char *msg ){
	...
	if( getStringFromIndexWithDelim( msg, " ", 2, buff, sizeof( buff)) == TRUE )
	{
		if( strncmp( buff, "on", strlen( buff ) ) == 0 ){
			all_nosee = 1;
			snprintf( buff, sizeof( buff), "已開啓all_nosee 。" );
			CHAR_talkToCli( tindex, meindex, buff,
					CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
		}else
	...
	if( all_nosee == 0 ){
		...
		snprintf( buff, sizeof( buff), "all_nosee開啓著。" );
			CHAR_talkToCli( tindex, meindex, buff,
			CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
	}
}
...
static void NPC_Sysinfo_All_NoBody( int meindex, int tindex, char *msg ){
	...
	if( getStringFromIndexWithDelim( msg, " ", 2, buff, sizeof( buff)) == TRUE )
	{
		if( strncmp( buff, "on", strlen( buff ) ) == 0 ){
			all_nobody = 1;
			snprintf( buff, sizeof( buff), "已開啓all_nobody 。" );
			CHAR_talkToCli( tindex, meindex, buff,
					CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
		}else
		if( strncmp( buff, "off", strlen( buff ) ) == 0 ){
	...
	if( all_nobody == 0 ){
		...
		snprintf( buff, sizeof( buff), "all_nobody 開啓著。" );
		CHAR_talkToCli( tindex, meindex, buff,
				CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
	}
}
...
static void NPC_Sysinfo_Enemy_State( int meindex, int tindex, char *msg ){
	...
	if( all_nobody == 0 ){
		...
		snprintf( buff, sizeof( buff), "allnobody 開啓著。" );
		CHAR_talkToCli( tindex, meindex, buff,
				CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
	}
	if( all_nosee == 0 ){
		...
		snprintf( buff, sizeof( buff), "allnosee 開啓著。" );
		CHAR_talkToCli( tindex, meindex, buff,
				CHAR_getWorkInt( meindex, CHAR_WORK_MSGCOLOR));
	}
