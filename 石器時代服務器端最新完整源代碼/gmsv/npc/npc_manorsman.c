...
enum {
	...
	,NPC_WORK_SUPERFMPOINT = CHAR_NPCWORKINT7	// 記錄
#endif
};
...
BOOL NPC_ManorSmanInit( int meindex )
{
  ...
  CHAR_setWorkInt(meindex, NPC_WORK_CHALLENGEWAIT, 0);
...
void NPC_ManorSmanTalked(int meindex, int talkerindex, char *msg, int color)
{
  ...
  case FMPKS_FLAG_NONE:
    ...
    saacproto_ACFMPointList_send(acfd);
		sprintf(buf,"莊園所有權爭奪戰的挑戰資格\n\n"
								...
								"待確認時間還剩餘%d秒", max(0, fmpks[fmpks_pos+1].dueltime - NowTime.tv_sec));
  ...
  case FMPKS_FLAG_MANOR_PEACE:
	case FMPKS_FLAG_MANOR_PEACE_SAVE:
    {
      ...
      if(strlen(buf2) == 0) strcpy(buf, "請稍等，讓我準備一下申請踢館的錶格。");
      ...
      if ((CHAR_getInt(talkerindex,CHAR_FMLEADERFLAG)==FMMEMBER_MEMBER) &&
        ...
        if (hadfmindex != -1) {
          if (hadfmindex-1 != tkfmindex){
            int check=0,i;
#ifdef _SUPER_FMPOINT
						if(CHAR_getWorkInt(meindex, NPC_WORK_SUPERFMPOINT) == 1){	// 記錄開打時的日期)
							 for (i=0; i<MANORNUM; i++) {	// 9個莊園
  ...
  case CHAR_WINDOWTYPE_CHECKMAN_MAIN:	  
    if (select==WINDOW_BUTTONTYPE_YES) {
	  ...
	  if(p->tm_hour<20 || p->tm_hour>23){
		  sprintf(buf, "請於下午２０：００至晚上２４：００再來約戰吧！");
			  lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
              	    	WINDOW_BUTTONTYPE_OK,
    			        CHAR_WINDOWTYPE_CHECKMAN_END,
    			        CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),
...
{
			int i;
      for (i=1; i<=MANORNUM; i++) {	// 是否已經挑戰其他莊園
...
/*
    ...
    lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
		...
		CHAR_talkToCli(talkerindex,-1,"扣除個人聲望500點",CHAR_COLORYELLOW);
	}
	*/
#endif
		...
		switch (atoi(data)){
  		...
  		case 4:
  			...
  			break;
  		case 5:
  			dueltime = NowTime.tv_sec - tm1.tm_min * 60 + ((24 - tm1.tm_hour) * 3600) + 3600 * 22 + 1800;
  			break;
  		default:
  ...
  case FMPKS_FLAG_MANOR_PREPARE:
    if(fmpks[fmpks_pos+1].dueltime <= NowTime.tv_sec){
      ...
      NPC_talkToFloor(CHAR_getInt(meindex, CHAR_FLOOR) , fmpks[fmpks_pos].host_index,
			...
			char token[256];
    	sprintf(token, "『%s ＶＳ %s』莊園所有權爭奪戰即將開始～",
							    		fmpks[fmpks_pos+1].guest_name,
							    		fmpks[fmpks_pos+1].host_name);
  ...
  case FMPKS_FLAG_MANOR_BATTLEEND:
		{
			...
			//fmpks[fmpks_pos+1].dueltime = NowTime.tv_sec - tm1.tm_min*60 + iOffsetTime + 86400 + (18 * 3600);	//一天
			fmpks[fmpks_pos+1].dueltime = NowTime.tv_sec - tm1.tm_min*60 + iOffsetTime  + (18 * 3600);
