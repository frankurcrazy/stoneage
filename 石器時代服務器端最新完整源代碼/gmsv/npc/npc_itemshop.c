...
static void NPC_ItemShop_selectWindow( int meindex, int talker, int num,int select)
{
	switch( num) {
	  ...
	  case 1://進入買視窗
		CHAR_sendStatusString( talker,"I");//傳送玩傢身上所有的道具給Client
...
#ifdef _VERSION_25
		sprintf( token, "0|1|%d|%s|%s|%-72s聲望:%d|", CHAR_WINDOWTYPE_WINDOWITEMSHOP_STARTMSG,
...
*/
	...
	if(iTotalCostFame > 0){
		...
		char token[256];
		sprintf(token, "成功扣除聲望:%dＦ", iTotalCostFame / 100);
...
#ifdef _VERSION_25
		sprintf( token, "1|1|%d|%s|%s|%-72s聲望%d|", CHAR_WINDOWTYPE_WINDOWITEMSHOP_STARTMSG,
				CHAR_getChar( meindex, CHAR_NAME), buff, buff2, CHAR_getInt(talker,CHAR_FAME)/100);
#else
