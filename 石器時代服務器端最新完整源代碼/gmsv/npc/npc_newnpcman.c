...
int PlayerBBI[6*2][4]={
		...
		{ 100190, 100195, 100185, 100180 },	//短發發夾妹
...
#ifdef _GET_MAGICAMOR
		case 3:
			if( CHECK_MAGICAMOR( toindex) == TRUE ){
				CHAR_talkToCli( toindex, -1, "檢查完畢!", CHAR_COLORYELLOW);
			}
			return;
			break;
...
#ifdef _PET_LOSTPET
		{
			...
			if( getStringFromIndexWithDelim( petstring[ti-1], "#", 2, petstring1, sizeof( petstring1)) == FALSE ){
				...
				if( CHAR_getInt( toindex, CHAR_GOLD) < cost ) {
					sprintf( buf1, "寵物已寄放%d天，共需%d石幣纔可領迴。", ltime, cost);
...
BOOL CHECK_MAGICAMOR( int toindex)
{
	...
	if(NPC_EventCheckFlg( toindex, 71) == FALSE){
		CHAR_talkToCli( toindex, -1, "條件不足!", CHAR_COLORRED);
		return TRUE;
	}
#ifdef _NEW_ITEM_
	int maxitem = CheckCharMaxItem( toindex);
