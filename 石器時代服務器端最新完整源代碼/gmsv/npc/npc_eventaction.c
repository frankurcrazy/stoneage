    ...
    for(i=0;i<playernum;i++){
			...
			if(CHAR_getInt( i, CHAR_FLOOR) == fl)
			{
				if(CONNECT_get_userip(getfdFromCharaIndex(i)) == CONNECT_get_userip(getfdFromCharaIndex(talker))){
					num++;
					if(num>=ipnum){
						CHAR_talkToCli( talker, -1, "你的IP數進入已滿", CHAR_COLORYELLOW);
						return EvFlg;
					}
				}
			}
...
BOOL ActionNpc_CheckMenuFree( int meindex, int toindex, char *npcarg, int nums)
{
	...
	if( NPC_Util_GetStrFromStrWithDelim( npcarg, "TALKCHECK", buf1, sizeof( buf1) ) != NULL){
		...
		if(CHAR_getWorkInt(meindex, CHAR_WORK_TALKCHECK) != CHAR_getWorkInt(toindex, CHAR_WORK_TALKCHECK)){
			char buf[256];
			CHAR_setWorkInt(meindex, CHAR_WORK_TALKCHECK, getTalkCheck());
			sprintf(buf, "請輸入驗證數字:%d", CHAR_getWorkInt(meindex, CHAR_WORK_TALKCHECK));
...
BOOL NPC_ActionIntSet( int talker,char *buf)
{
	...
	for(i=0;i<CHAR_DATAINTNUM;i++){
		if(strcmp(key, CHAR_setintdata[i]) == 0){
		  ...
		  char token[128];
			sprintf(token, "你當前值為:%d", CHAR_getInt( talker, i));
...
BOOL NPC_ActionDelFame( int talker,char *buf)
{
	...
	char buf1[256];
	sprintf( buf1, "扣除個人聲望：%d", Fame/100);
	CHAR_talkToCli( talker, -1, buf1, CHAR_COLORYELLOW);
...
BOOL NPC_ActionMomentum( int talker,char *buf)
{
	...
	if( PMomentum < Momentums )	{
		CHAR_talkToCli( talker, -1, "你的個人氣勢值不夠喔！", CHAR_COLORYELLOW);
		return FALSE;
	}else	{
		char buf1[256];
		sprintf( buf1, "扣除個人氣勢值%d", Momentums/100);
		CHAR_talkToCli( talker, -1, buf1, CHAR_COLORYELLOW);
	}
...
BOOL NPC_ActionDP( int talker,char *buf)
{
	...
	if( PDP < DPs )	{
		CHAR_talkToCli( talker, -1, "你的DP不夠哦！", CHAR_COLORYELLOW);
		return FALSE;
	}else	{
		char buf1[256];
...
BOOL NPC_ActionDiyMapCheck(int meindex,int talker,int diymap,int flg)
{
  int i;
	...
	CHAR_talkToCli( talker, -1, "很抱歉，你不是該地圖的主人，無法為你服務~", CHAR_COLORRED);
	return FALSE;
}
	...
	if(CHAR_getInt(talker, CHAR_AMPOINT)>=point && CHAR_getInt(talker, CHAR_FAME)>=fame){
		...
		for(i=0;i<Player_Diy_Map_NUM;i++){
		  if(PlayerDiyMap[i].ff==ff){
		  	if(PlayerDiyMap[i].datetime==-1){
					CHAR_talkToCli( talker, -1, "很抱歉，該地圖玩傢永久使用中，無法購買~", CHAR_COLORRED);
					return TRUE;
				}
		  	if(PlayerDiyMap[i].datetime<time(NULL)){
		  		find=TRUE;
				}else{
					CHAR_talkToCli( talker, -1, "很抱歉，該地圖已有玩傢使用中，請在失效期後購買~", CHAR_COLORRED);
					return TRUE;
				}
				break;
		  }
	  ...
	  if(find==TRUE){
		  ...
		  if(fame>0){
			...
			sprintf(buf1, "恭喜你，獲得地圖 %d DIY權限 %d 天，最高支持人數 %d 人", point, day, playernum);
	  	...
	  	CHAR_talkToCli( talker, -1, "很抱歉，已經達到DIY地圖使用權上限，操作不成功~", CHAR_COLORRED);
	  }
	}else{
		CHAR_talkToCli( talker, -1, "很抱歉，你無法滿足條件購買DIY地圖使用權~", CHAR_COLORRED);
	}
	return TRUE;
}
...
BOOL NPC_ActionPlayerDiyMapLetter( int talker, char *buf)
{
  ...
  if(i==Player_Diy_Map_NUM){
  	CHAR_talkToCli( talker, -1, "很抱歉，該地圖並非你所有~", CHAR_COLORRED);
  	return FALSE;
  }
	...
	sprintf( token, "由 %s 簽發的地圖 %d 邀請函", PlayerDiyMap[i].cdkey, PlayerDiyMap[i].ff);
	...
	for(i=0;i<Player_Diy_Map_NUM;i++){
		if(PlayerDiyMap[i].ff==ff){
					...
					if( strcmp(ITEM_getChar( itemindex, ITEM_USEFUNC), "PLAYERDIYMAPLETTER")==0){
						if( strcmp(ITEM_getChar( itemindex, ITEM_ARGUMENT), PlayerDiyMap[i].cdkey)==0){
							...
							if(k<CHAR_getPlayerMaxNum()){
								CHAR_talkToCli( talker, -1, "你無法進入，地圖人數已滿，請稍候再來~", CHAR_COLORRED);
								return TRUE;
							}
							LogItem(
			...
			if(j ==  CheckCharMaxItem(talker) ){
				CHAR_talkToCli( talker, -1, "你身上並無該地圖使用者的邀請函~", CHAR_COLORRED);
				return TRUE;
			}
		}
	}
				...
				if(CHAR_getInt(talker, CHAR_AMPOINT)>=point){
					...
					LogAmPoint(CHAR_getChar( talker, CHAR_NAME ),
									          ...
									          -point,
									         	...
									         	CHAR_getInt( talker,CHAR_Y ));
#endif
					sprintf(token, "扣除積分點 %d", point);
					CHAR_talkToCli( talker, -1, token, CHAR_COLORYELLOW);
					PlayerDiyMap[i].playernum += playernum;
					sprintf( token,"恭喜你，你的DIY地圖最大支持人數提升至 %d 人~", PlayerDiyMap[i].playernum);
					CHAR_talkToCli( talker, -1, token, CHAR_COLORYELLOW );
				  ...
				  fclose(fp);
					...
					CHAR_talkToCli( talker, -1, "很抱歉，你的積分點數不足，無法提升人數~", CHAR_COLORRED);
  				return FALSE;
				}
		...
		if(PlayerDiyMap[i].ff==ff){
			if(strcmp(CHAR_getChar(talker, CHAR_CDKEY), PlayerDiyMap[i].cdkey)==0){
				if(CHAR_getInt(talker, CHAR_AMPOINT)>=point){
					...
					sprintf( token,"恭喜你，你的DIY地圖經驗獲取提升至 %d ~", PlayerDiyMap[i].exp);
					CHAR_talkToCli( talker, -1, token, CHAR_COLORYELLOW );
				  ...
				  fclose(fp);
					...
					CHAR_talkToCli( talker, -1, "很抱歉，你的積分點數不足，無法提升地圖經驗獲取~", CHAR_COLORRED);
  				return FALSE;
				}
		...
		if(PlayerDiyMap[i].ff==ff){
			if(strcmp(CHAR_getChar(talker, CHAR_CDKEY), PlayerDiyMap[i].cdkey)==0){
				if(CHAR_getInt(talker, CHAR_AMPOINT)>=point){
					...
					sprintf( token,"恭喜你，你的DIY地圖使用權增加 %d 天~", day);
					CHAR_talkToCli( talker, -1, token, CHAR_COLORYELLOW );
				  ...
				  fclose(fp);
					...
					CHAR_talkToCli( talker, -1, "很抱歉，你的積分點數不足，無法提升地圖增加使用權天數~", CHAR_COLORRED);
  				return FALSE;
				}
...
BOOL NPC_ActionLotteryChange( int talker, char *buf)
{
	...
	if(now.tm_year==tm1.tm_year && now.tm_mon==tm1.tm_mon && now.tm_mday==(tm1.tm_mday + 1)){
		...
		if(num>0){
			...
			sprintf(token, "恭喜你中得 %s等奬 ,奬勵積分為 %d 點", tmp[7-num], award[7-num]);
			lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE, 
			...
			int playernum = CHAR_getPlayerMaxNum();
			sprintf(token, "恭喜玩傢 %s 中得 %s等奬 ,奬勵積分為 %d 點", CHAR_getChar(talker, CHAR_NAME), tmp[7-num], award[7-num]);
			for( i = 0 ; i < playernum ; i++) {
			...
			sprintf(token, "賬號 %s 玩傢 %s 中得 %s等奬, 奬勵積分為 %d × %d = %d 點\n", CHAR_getChar(talker, CHAR_CDKEY),
																																				CHAR_getChar(talker, CHAR_NAME), 
																																				tmp[7-num], 
																																				award[7-num],
																																				lotterytime,
			...
			lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE, 
											WINDOW_BUTTONTYPE_OKCANCEL,	-1,	-1,	"很抱歉，你沒有中奬！");
		}
	}else{
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE, 
											WINDOW_BUTTONTYPE_OKCANCEL,	-1,	-1,	"很抱歉，你還未購買這期開奬的彩票！");
	}
...
BOOL NPC_ActionLotteryAddTime( int talker, char *buf)
{
	...
	if(strcmp(lotteryvalue,"")==0 || lotterydate==0 || lotterytime==0){
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OKCANCEL,	-1,	-1,	"你未購買彩票");
	}else{
		char token[256];
		...
		}
		{
			...
			int playernum = CHAR_getPlayerMaxNum();
			char token[256];
			sprintf(token, "玩傢 %s 在投注站增加 %d 倍彩票", CHAR_getChar(talker, CHAR_NAME), atoi(buf));
			for( i = 0 ; i < playernum ; i++) {
				if( CHAR_getCharUse(i) != FALSE ) {
					CHAR_talkToCli( i, -1, token, CHAR_COLORBLUE);
				}
		...
		sprintf(token, "你購買的%04d%02d%02d期的彩票 × %d;號碼為：\n %0d,%0d,%0d,%0d,%0d,%0d,%0d", tm1.tm_year+1900,
																																													tm1.tm_mon+1,
																																													tm1.tm_mday,
																																													lotterytime,
																																													value[0],
...
BOOL NPC_ActionShowItemName( int talker ,char *buf)
{
	...
	if( !ITEM_CHECKINDEX( itemindex ) ) {
		CHAR_talkToCli( talker, -1, "請將道具放在第一欄上",  CHAR_COLORRED);
		return TRUE;
	}
	if(ITEM_getInt(itemindex ,ITEM_ID ) == id){
		...
		CHAR_talkToCli( talker, -1, "已成功為你道具刻名",  CHAR_COLORGREEN);
	}else{
		CHAR_talkToCli( talker, -1, "很抱歉,你的道具不符閤刻名",  CHAR_COLORRED);
	}
  ...
  if( i == CHAR_MAXPETHAVE ){
	...
	if(strstr(buff3,"MM1") || strstr(buff3,"MM2")){
		...
		CHAR_setInt( ret, CHAR_ALLOCPOINT, LevelUpPoint);
			...
			while(CHAR_getInt( ret, CHAR_LV)<79){	//升級
				int LevelUpPoint;
				LevelUpPoint = ( 50<< 24) + ( 50<< 16) + ( 50<< 8) + ( 50<< 0);
				CHAR_setInt( ret, CHAR_ALLOCPOINT, LevelUpPoint);
				CHAR_PetLevelUp( ret );
...
BOOL NPC_ActionVipItem( int talker, char *buf)
{
  ...
  itemindex = CHAR_findEmptyItemBox( talker );
	if( itemindex < 0 )	{
		CHAR_talkToCli( talker, -1, "物品欄空間不足！！",  CHAR_COLORYELLOW);
		return FALSE;
	}
  ...
  pVipPoint = CHAR_getInt( talker, CHAR_AMPOINT);
	if( pVipPoint < VipPoints )	{
		CHAR_talkToCli( talker, -1, "你身上的積分點數不夠哦", CHAR_COLORYELLOW);
		return FALSE;
	}
  ...
  if( itemindex != -1 ){
		...
		LogAmPoint(CHAR_getChar( talker, CHAR_NAME ),
									          ...
									          -VipPoints,
									         	...
									         	CHAR_getInt( talker,CHAR_Y ));
#endif		
		sprintf( token, "扣除積分點數%d", VipPoints);
		CHAR_talkToCli( talker, -1, token, CHAR_COLORYELLOW);
...
BOOL NPC_ActionVipPet( int talker, char *buf)
{
  ...
  if( i == CHAR_MAXPETHAVE )      {
	...
	if( i == CHAR_MAXPETHAVE )
		CHAR_talkToCli( talker, -1, "領取寵物失敗！", CHAR_COLORYELLOW);
	...
	LogAmPoint(CHAR_getChar( talker, CHAR_NAME ),
									          ...
									          -VipPoints,
									         	...
									         	CHAR_getInt( talker,CHAR_Y ));
#endif	
	snprintf( msgbuf,sizeof( msgbuf), "扣除積分點數%d", VipPoints);
	CHAR_talkToCli( talker, -1, msgbuf, CHAR_COLORYELLOW);
	for(j = 0; j < CHAR_MAXPETHAVE; j++){
		petindex = CHAR_getCharPet(talker, j);
		if( !CHAR_CHECKINDEX( petindex) )
...
BOOL NPC_ActionNewVipItem( int talker, char *buf)
{
  ...
  int itemindex = CHAR_findEmptyItemBox( talker );
	...
	CHAR_talkToCli( talker, -1, "取獲數據中，請稍待...",  CHAR_COLORRED);
	int Points=0;
	...
	if(osfd == -1){
		OtherSaacConnect();
		CHAR_talkToCli( talker, -1, "點捲服務器未正常連接!", CHAR_COLORRED );
		return FALSE;
	}
...
BOOL NPC_ActionNewVipPet( int talker, char *buf)
{
  ...
  if( i == CHAR_MAXPETHAVE )      {
		snprintf( buf1,sizeof( buf1), "寵物已滿！！");
		CHAR_talkToCli( talker, -1, buf1,  CHAR_COLORYELLOW);
		return FALSE;
	}
...
BOOL NPC_ActionChangePlayerBBI( int meindex, int charindex, char *Img)
{
	...
	if( CHAR_getInt( charindex, CHAR_RIDEPET) != -1 )	{//騎寵不換圖
		...
		//change -----> 修改bug(落馬後,寵物選項的狀態依然為"騎乘",這裏修正過來
...
#ifdef  _NPC_ADDLEVELUP				// (不可開) ANDY 外部測試機用來增加玩傢等級
