...
#ifdef _OFFLINE_SYSTEM
					if(getPlayerNum()>0){
						sprintf(temp,"目前綫上人數為 %d 人。", playeronlinenum + luaplayernum + getPlayerNum());
					}else{
						sprintf(temp,"目前綫上人數為 %d 人。", playeronlinenum + luaplayernum);
					}
#else
					extern unsigned int StateTable[];
					if(getPlayerNum()>0){
						sprintf(temp,"目前綫上人數為 %d 人。",abs(StateTable[LOGIN]) + getPlayerNum() + rand() % 10);
					}else{
						sprintf(temp,"目前綫上人數為 %d 人。",abs(StateTable[LOGIN]));
					}
#endif	
#else
					extern unsigned int StateTable[];
