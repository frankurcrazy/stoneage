...
BOOL NPC_WarpMsg(int meindex,int talker,char *arg)
{
	...
	if( check_TimeTicketMap( fl) ) {
		...
		for( havei = CHAR_STARTITEMARRAY ; havei < itemMax ; havei++ ){
#else
...
#endif
			CHAR_talkToCli( talker, -1, "請先準備專用門票纔可進入。", CHAR_COLORYELLOW);
			return FALSE;
		}
	}
#endif
	...
	if( parent < 0 )
		return TRUE;
	if( NPC_Util_GetStrFromStrWithDelim( arg, "PartyMsg", buf, sizeof( buf)))
		return TRUE;
