...
int NPC_Lua_NLG_DelPet(lua_State *_NLL)
{
	...
	for(i = 0; i < CHAR_MAXPETHAVE ; i++)
	{
		...
		{
			char TM_MsgBuff[128];
			snprintf(TM_MsgBuff, sizeof(TM_MsgBuff), "刪除寵物時産生錯誤 錯誤號: %d。", TM_Ret);
...
int NPC_Lua_NLG_DelItemByPos(lua_State *_NLL)
{
	...
	if( TM_ItemPos < CHAR_STARTITEMARRAY || TM_ItemPos > itemMax -1 ) {
		CHAR_talkToCli( TM_Index, -1, "位置不正確。",  CHAR_COLORWHITE);
		return;
	}
...
int NPC_Lua_NLG_GivePet(lua_State *_NLL)
{
	...
	{
		char TM_Buff[128];
		snprintf(TM_Buff, sizeof(TM_Buff), "獲得 %d 隻 %s。", TM_Count, CHAR_getChar( TM_PetIndex, CHAR_NAME));
		CHAR_talkToCli( TM_Index, -1, TM_Buff, CHAR_COLORYELLOW);
	}
	LRetInt(_NLL, TM_Count);
}
...
int NPC_Lua_NLG_GiveOnePet(lua_State *_NLL)
{
	...
	char TM_Buff[128];
	snprintf(TM_Buff, sizeof(TM_Buff), "獲得1隻 %s。", CHAR_getChar( TM_PetIndex, CHAR_NAME));
	CHAR_talkToCli( TM_Index, -1, TM_Buff, CHAR_COLORYELLOW);
...
int NPC_Lua_NLG_GiveItem(lua_State *_NLL)
{
	...
	{
		char TM_Buff[128];
		snprintf(TM_Buff, sizeof(TM_Buff), "獲得 %d 個 %s。", TM_Count, ITEM_getChar( TM_ItemIndex, ITEM_NAME));
		CHAR_talkToCli( TM_Index, -1, TM_Buff, CHAR_COLORYELLOW);
	}
	LRetInt(_NLL, TM_Count);
}
...
int NPC_Lua_NLG_GiveRandItem(lua_State *_NLL)
{
	...
	if(TM_Int>=0){
		char TM_Buff[256];
		snprintf(TM_Buff, sizeof(TM_Buff), "獲得 %s。", ITEM_getChar( TM_Int, ITEM_NAME));
		CHAR_talkToCli( TM_Index, -1, TM_Buff, CHAR_COLORYELLOW);
/*		if(TM_Type>0){
			time_t timep;
			struct tm *p;
			...
			sprintf(TM_Buff,"[限時]%s",ITEM_getChar(TM_Int,ITEM_NAME));
...
int NPC_Lua_NLG_TalkToCli(lua_State *_NLL)
{
	...
	{
		LRetErrInt(_NLL , -2, "未知原因導緻發送說話失敗。");
	}
}
...
int NPC_Lua_NLG_SearchWatchBattleRandIndex(lua_State *_NLL)
{
	...
	{
		char TM_Buff[64];
		snprintf(TM_Buff, sizeof(TM_Buff), "無戰鬥可看！錯誤號:%d", TM_Floor);
		LRetErrInt(_NLL, -4, TM_Buff);
	}
	LRetInt(_NLL, TM_Ret);
}
...
int NPC_Lua_NLG_GivePetOne(lua_State *_NLL)
{
	...
	if(TM_Int >= 0)
	{
		TM_PetIndex = TM_Int;
		char TM_Buff[128];
		snprintf(TM_Buff, sizeof(TM_Buff), "獲得 1 隻 %s。", CHAR_getChar( TM_PetIndex, CHAR_NAME));
		CHAR_talkToCli( TM_Index, -1, TM_Buff, CHAR_COLORYELLOW);
	}
	LRetInt(_NLL, TM_PetIndex);
}
...
int NPC_Lua_NLG_WalkJc(lua_State *_NLL)//檢查前方障礙
{
	CheckEx(_NLL, 5);
	CheckIndexNull(_NLL, 1);
	int TM_Index = (int)lua_tointeger(_NLL, 1);
...
int NPC_Lua_NLG_KickPlayer(lua_State *_NLL)
{
	...
	LogCharOut(charname,cdkey,__FILE__,__FUNCTION__,__LINE__,"NPC_Lua_NLG_KickPlayer 處理下綫");
#endif
...
int NPC_Lua_NLG_LevelUpTo(lua_State *_NLL)
{
	...
	if (CHAR_getInt( TM_index, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER)//如果是人物的話
	{
		...
		LRetBool(_NLL, TRUE);
	}else if (CHAR_getInt( TM_index, CHAR_WHICHTYPE ) == CHAR_TYPEPET)//如果是寵物的話
	{
		char token[128];
		for( i = 1; i <= TM_LV; i ++ ){	//升級
		...
		int PLAYER = CHAR_getWorkInt(TM_index, CHAR_WORKPLAYERINDEX);//獲得主人的索引
		for( i = 0; i < CHAR_MAXPETHAVE; i ++ )
		{
...
int NPC_Lua_NLG_AddExp(lua_State *_NLL)
{
	...
	if (CHAR_getInt( TM_index, CHAR_WHICHTYPE ) == CHAR_TYPEPET)//如果是寵物的話
	{
		char token[128];
		int PLAYER = CHAR_getWorkInt(TM_index, CHAR_WORKPLAYERINDEX);//獲得主人的索引
		for( i = 0; i < CHAR_MAXPETHAVE; i ++ )
...
int NPC_Lua_NLG_DelPetByPos(lua_State *_NLL)
{
	...
	if (TM_PetID < 0 || TM_PetID > 4) LRetErrInt(_NLL , -1, "指定的寵物欄位沒有寵物！");
...
int NPC_Lua_NLG_GivePetByIndex(lua_State *_NLL)
{
	...
	if( havepetelement < 0 ) {
		LRetErrInt(_NLL , -1, "目標寵物欄位已滿！");
		return;
	}
	CHAR_setWorkInt( TM_PetIndex, CHAR_WORKPLAYERINDEX, TM_Index);//寵物主人
