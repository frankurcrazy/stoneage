...
//	NPC_Lua_DoFile(_DoFile);
	...
	{
		print("NPC_Lua_Create err: 無法獲取初始化函數指針。\n");
		lua_pop(M_Script_Lua, 1);
		LRetErrInt(M_Script_Lua, -2, "無法獲取初始化函數指針。");
	}else
	{
		//保存函數指針
		lua_pop(M_Script_Lua, 1);
	...
	TM_obj.type = OBJTYPE_CHARA;					//對象類型
	TM_obj.index= TM_charaindex;					//對象對應的Char索引
	TM_obj.x = CHAR_getInt(TM_charaindex, CHAR_X);		//對象所在地圖的X坐標
	TM_obj.y = CHAR_getInt(TM_charaindex, CHAR_Y);		//對象所在地圖的Y坐標
	TM_obj.floor = CHAR_getInt(TM_charaindex, CHAR_FLOOR);//對象所在地圖
	TM_objindex = initObjectOne( &TM_obj );				//産生一個對象實例（調用完之後NPC實體産生）
	if( TM_objindex == -1 )
	{
		CHAR_endCharOneArray( TM_charaindex );
		LRetErrInt(M_Script_Lua, -4, "無法實例化NPC對象。");
  }
