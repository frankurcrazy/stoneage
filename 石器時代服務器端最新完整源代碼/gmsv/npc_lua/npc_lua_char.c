...
int NPC_Lua_Char_SetData(lua_State *_NLL)
{
	...
	if(TM_Flg >= 0)
	{
		if(TM_Flg < LUA_DATALINE1)
		{
			...
			{
				saacproto_IncreaseMPoint_send(acfd, -1, CHAR_getChar( TM_Index, CHAR_CDKEY ), "增加金幣", TM_IntVal - CHAR_getInt(TM_Index, TM_Flg));
			}
#endif
		...
		}else if(TM_Flg < LUA_DATALINE5)
		{
			...
			}else LRetErrNull(_NLL, "傳入的標誌是錯誤的。");
		}else
		{
...
int NPC_Lua_Char_RandMsg(lua_State *_NLL)
{
	...
	if(strlen(randtypetime)>2){
		if(getStringFromIndexWithDelim(randtypetime,"-", 3, tempbuff, sizeof(tempbuff)) != FALSE){
			...
			if(timep - randtime < getEditBaseTime()*randwrong){
				char errbuf[256];
				sprintf(errbuf,"您還有%d秒纔可以繼續此操作！",getEditBaseTime()*randwrong-(timep - randtime));
	...
	if(strlen(arg)>2){
		...
		for(n=1;n<=5;n++){
			if(n==randnum1){
				if(qianhounum<=50)
					sprintf(tempret[n-1],"選項( %d %s )",rightnum,randrightanswer);
				else
					sprintf(tempret[n-1],"選項( %s %d )",randrightanswer,rightnum);
			}else{
				if(m==1){
					if(qianhounum<=50)
						sprintf(tempret[n-1],"選項( %d %s )",wrongnum1,randwronganswer1);
					else
						sprintf(tempret[n-1],"選項( %s %d )",randwronganswer1,wrongnum1);
					m = 2;
				}else if(m==2){
					if(qianhounum<=50)
						sprintf(tempret[n-1],"選項( %d %s )",wrongnum2,randwronganswer2);
					else
						sprintf(tempret[n-1],"選項( %s %d )",randwronganswer2,wrongnum2);
					m = 3;
				}else if(m==3){
					if(qianhounum<=50)
						sprintf(tempret[n-1],"選項( %d %s )",wrongnum3,randwronganswer3);
					else
						sprintf(tempret[n-1],"選項( %s %d )",randwronganswer3,wrongnum3);
					m = 4;
				}else if(m==4){
					if(qianhounum<=50)
						sprintf(tempret[n-1],"選項( %d %s )",wrongnum4,randwronganswer4);
					else
						sprintf(tempret[n-1],"選項( %s %d )",randwronganswer4,wrongnum4);
					m = 5;
				}
			}
		}
		...
		sprintf(buf,"%s\n%s\n%s\n%s\n%s\n%s\n輸入正確答案括號內的5位數字或者點選答案",randquestion,tempret[0],tempret[1],tempret[2],tempret[3],tempret[4]);
...
int NPC_Lua_Char_GetDataFromEnemyBase(lua_State *_NLL)
{
	...
	{
		LRetErrInt(_NLL, -1, "傳入的EnemyBase寵物ID不存在。");
	}
...
int NPC_Lua_Char_FindItemIndex(lua_State *_NLL)
{
	...
	{
		LRetErrInt(_NLL, -1, "傳入人物的索引是無效的。");
	}
	if(!ITEM_CHECKINDEX(TM_ItemIndex))
	{
		LRetErrInt(_NLL, -1, "傳入道具的索引是無效的。");
	}
	...
	LRetErrInt(_NLL, -1, "沒有找到對應 ItemIndex 的道具");
...
int NPC_Lua_Char_FindPetIndex(lua_State *_NLL)
{
	...
	LRetErrInt(_NLL, -1, "沒有找到對應 PetIndex 的寵物");
}
...
int NPC_Lua_Char_CharRidNo(lua_State *_NLL)
{
	...
	if (RidNo < 0 || RidNo > 63) LRetErrInt(_NLL, -1, "傳入的騎寵編號是無效的。");
...
int NPC_Lua_Char_GetEnemyBaseIdFromEnemyId(lua_State *_NLL)
{
	...
	if (i == enemynum) LRetErrInt(_NLL, -1, "傳入的enemyID沒有找到！");
...
int NPC_Lua_Char_GetEnemyIdFromEnemyBaseId(lua_State *_NLL)
{
	...
	if (i == enemynum) LRetErrInt(_NLL, -1, "傳入的enemybaseID沒有找到！");
...
int NPC_Lua_Char_GetIp(lua_State *_NLL)
{
	...
	if( CHAR_getInt( TM_Index, CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER ){
		LRetErrInt(_NLL, -2, "傳入的對象並不是人物！");
	}
	int fd = getfdFromCharaIndex( TM_Index ); 
	if( fd == -1 ){
		LRetErrInt(_NLL, -2, "該人物可能已經下綫！");
	}
	if(TM_Flg == 1)//IP
	{
		LRetMsg(_NLL,CONNECT_get_userip2(fd));
	}else if(TM_Flg == 2)//給FD
	{
		LRetInt(_NLL,fd);
	}else if(TM_Flg == 3)//端口
	{
...
int NPC_Lua_Char_SetAllocPoint(lua_State *_NLL)
{
	...
	if (val < 0 || val > 255) LRetErrInt(_NLL, -1, "val必須為0-255。");
	if(!CHAR_CHECKINDEX(petindex))
	{
		LRetErrInt(_NLL, -1, "傳入的索引是無效的。");
	}
	if( CHAR_getInt( petindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
		LRetErrInt(_NLL, -2, "傳入的對象不能是人物！");
	}
...
int NPC_Lua_Char_CharRidNo(lua_State *_NLL)
{
	...
	if (RidNo < 0 || RidNo > 95) LRetErrInt(_NLL, -1, "傳入的騎寵編號是無效的。");
