...
void lssproto_XYD_send(int fd,int x,int y,int dir)
{
	...
	printf("[發送]LSSPROTO_XYD_SEND-x:%d,y:%d,dir:%d\n",x,y,dir);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_EV_send(int fd,int seqno,int result)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_EV_SEND-seqno:%d,result:%d\n",seqno,result);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_EN_send(int fd,int result,int field)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_EN_SEND-result:%d,field:%d\n",result,field);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_RS_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_RS_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_RD_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_RD_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_B_send(int fd,char* command)
{
	...
	char buffer[65500];
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_B_SEND-command:%s\n",command);
#endif
...
void lssproto_I_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_I_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_SI_send(int fd,int fromindex,int toindex)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_SI_SEND-fromindex:%d,toindex:%d\n",fromindex,toindex);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_MSG_send(int fd,int aindex,char* text,int color)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_MSG_send-aindex:%d,text:%s,color:%d\n",aindex,text,color);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_PME_send(int fd,int objindex,int graphicsno,int x,int y,int dir,int flg,int no,char* cdata)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_PME_SEND-objindex:%d,graphicsno:%d,x:%d,y:%d,dir:%d,flg:%d,no:%d,cdata:%s\n",objindex,graphicsno,x,y,dir,flg,no,cdata);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_AB_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_AB_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_ABI_send(int fd,int num,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_ABI_SEND-num:%d,data:%s\n",num,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_TK_send(int fd,int index,char* message,int color)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_TK_SEND-index:%d,message:%s,color:%d\n",index,message,color);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_MC_send(int fd,int fl,int x1,int y1,int x2,int y2,int tilesum,int objsum,int eventsum,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_MC_SEND-fl:%d,x1:%d,y1:%d,x2:%d,y2:%d,tilesum:%d,objsum:%d,eventsum:%d,data:%s\n",fl,x1,y1,x2,y2,tilesum,objsum,eventsum,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_M_send(int fd,int fl,int x1,int y1,int x2,int y2,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_M_SEND-fl:%d,x1:%d,y1:%d,x2:%d,y2:%d,data:%s\n",fl,x1,y1,x2,y2,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_C_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_C_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CA_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CA_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CD_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CD_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_R_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_R_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_S_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_S_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_D_send(int fd,int category,int dx,int dy,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_D_SEND-category:%d,dx:%d,dy:%d,data:%s\n",category,dx,dy,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_FS_send(int fd,int flg)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_FS_SEND-flg:%d\n",flg);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_HL_send(int fd,int flg)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_HL_SEND-flg:%d\n",flg);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_PR_send(int fd,int request,int result)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_PR_SEND-request:%d,result:%d\n",request,result);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_PETS_send(int fd,int petarray,int result)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_PETST_SEND-petarray:%d,result:%d\n",petarray,result);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_KS_send(int fd,int petarray,int result)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_KS_SEND-petarray:%d,result:%d\n",petarray,result);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_SPET_send(int fd, int standbypet, int result)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_SPET_SEND-standbypet:%d,result:%d\n",standbypet,result);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_PS_send(int fd,int result,int havepetindex,int havepetskill,int toindex)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_PS_SEND-result:%d,havepetindex:%d,havepetskill:%d,toindex:%d\n",result,havepetindex,havepetskill,toindex);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_SKUP_send(int fd,int point)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_SKUP_SEND-point:%d\n",point);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_WN_send(int fd,int windowtype,int buttontype,int seqno,int objindex,char* data)
{
	char buffer[65500];
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_WN_SEND-windowtype:%d,buttontype:%d,seqno:%d,objindex:%d,data:%s\n",windowtype,buttontype,seqno,objindex,data);
#endif
...
void lssproto_EF_send(int fd,int effect,int level,char* option)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_EF_SEND-effect:%d,level:%d,char:%s\n",effect,level,option);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_SE_send(int fd,int x,int y,int senumber,int sw)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_SE_SEND-x:%d,y:%d,senumber:%d,sw:%d\n",x,y,senumber,sw);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_ClientLogin_send(int fd,char* result)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CLIENTLOGIN_SEND-result:%s\n",result);
#endif
	char buffer[65500];
	int checksum=0;
...
void _lssproto_CreateNewChar_send(int fd,char* result,char* data,char *file,int line)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CREATENEWCHAR_SEND-result:%s,data:%s\n",result,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CharDelete_send(int fd,char* result,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CHARDELETE_SEND-result:%s,data:%s\n",result,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CharLogin_send(int fd,char* result,char* data)
{
	...
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CHARLOGIN_SEND-result:%s,data:%s\n",result,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CharList_send(int fd,char* result,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CHARLIST_SEND-result:%s,data:%s\n",result,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CharLogout_send(int fd,char* result,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CHARLOGOUT_SEND-result:%s,data:%s\n",result,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_ProcGet_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_PROCGET_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_PlayerNumGet_send(int fd,int logincount,int player)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_PLAYERNUMGET_SEND-logincount:%d,player:%d\n",logincount,player);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_Echo_send(int fd,char* test)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_ECHO_SEND-test:%s\n",test);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_TD_send(int fd, char* message)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_TD_SEND-message:%s\n",message);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_CHATROOM_send ( int fd , char* message )
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_CHATROOM_SEND-message:%s\n",message);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_RESIST_send ( int fd , char* message )
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_RESIST_SEND-message:%s\n",message);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_BATTLESKILL_send ( int fd , char* message )
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_BATTLESKILL_SEND-message:%s\n",message);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_NU_send(int fd, int nu)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_NU_SEND-nu:%d\n",nu);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_FM_send(int fd, char* message)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_FM_SEND-message:%s\n",message);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_WO_send(int fd,int effect)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_WO_SEND-effect:%d\n",effect);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_IC_send(int fd, int x, int y)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_IC_send-x:%d,y:%d\n",x,y);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_NC_send(int fd,int flg)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_NC_SEND-flg:%d\n",flg);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_STREET_VENDOR_send(int fd,char *message)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_STREET_VENDOR_SEND-message:%s\n",message);
#endif
	char buffer[65500];
	int checksum = 0;
...
void lssproto_RCLICK_send(int fd, int type, char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_RCLICK_SEND-type:%d,data:%s\n",type,data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_JOBDAILY_send(int fd,char *data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_JOBDAILY_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum = 0;
...
void lssproto_TEACHER_SYSTEM_send(int fd,char *data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_TEACHER_SYSTEM_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum = 0;
...
void lssproto_S2_send(int fd,char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_S2_SEND-data:%s\n",data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_VIP_SHOP_send(int fd, int num,int BJ,int type, int shoppage, int page, char *data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_VIP_SHOP_send-num:%d,BJ:%d,type:%d,shoppage:%d,page:%d,data:%s\n", num, BJ, type, shoppage, page, data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_ASSESS_ABILITY_send(int fd, char *data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_ASSESS_ABILITY_send-data:%s\n", data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_DENGON_send(int fd, char *data, int color, int num)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_DENGON_send-data:%s,%d,%d\n", data, color, num);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_SAMENU_send(int fd, int index, char* data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_SAMENU_send-data:%d,%s\n", index, data);
#endif
	char buffer[65500];
	int checksum=0;
...
void lssproto_FamilyBadge_send(int fd, char *data)
{
	if( CONNECT_checkfd(fd) == FALSE )return;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_FamilyBadge_send-data:%s\n", data);
#endif
	char buffer[65500];
	int checksum=0;
...
#endif
#ifdef _NEW_TITLE
void lssproto_CharTitle_send(int fd,char* data)//發送新數據
{
	char buffer[1024 * 64];
	int checksum=0;
	strcpy(buffer,"");
...
void lssproto_MoveScreen_send(int fd, BOOL bMoveScreenMove, int iXY)
{
	...
	int iChecksum = 0;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_MOVESCREEN_SEND-fd:%d,bMoveScreenMove:%d,iXY:%d\n",fd,bMoveScreenMove,iXY);
#endif
	szBuffer[0] = '\0';
...
void lssproto_TheaterData_send(int fd, char *pData)
{
	...
	char	szBuffer[65500];
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_TheaterData_SEND-fd:%d,pData:%s\n",fd,pData);
#endif
	strcpy(szBuffer,"");
...
void lssproto_MagiccardAction_send(int fd, char *data)
{
	...
	int checksum = 0;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTO_MagiccardAction_SEND-fd:%d,data:%s\n",fd,data);
#endif
	strcpy(buffer,"");
...
void lssproto_MagiccardDamage_send(int fd, int position ,int damage ,int offsetx,int offsety )
{
	...
	int checksum = 0;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]LSSPROTOMagiccardDamage_SEND-fd:%d,position:%d,damage:%d,offsetx:%d,offsety:%d\n",fd,position,damage,offsetx,offsety);
#endif
	strcpy(buffer,"");
...
void lssproto_DancemanOption_send(int fd, int option )
{
	...
	int checksum = 0;
#ifdef _DEBUG_SEND_CLI
	printf("[發送]lssproto_DancemanOption_send-fd:%d,option:%d\n", fd, option);
#endif
	strcpy(buffer,"");
