...
static CharBase CharBaseWindow[] = {
	{{"對話框"}, 					WINDOW_MESSAGETYPE_MESSAGE}
	,{{"輸入框"}, 				WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT}
	,{{"選擇框"},					WINDOW_MESSAGETYPE_SELECT}
	,{{"寵物框"}, 				WINDOW_MESSAGETYPE_PETSELECT}
	,{{"賣道具框"}, 			WINDOW_MESSAGETYPE_ITEMSHOPMENU}
	,{{"買道具框"}, 			WINDOW_MESSAGETYPE_ITEMSHOPMAIN}
	,{{"學技能框"}, 				WINDOW_MESSAGETYPE_PETSKILLSHOP}
#ifdef _NEW_MANOR_LAW
	,{{"十大氣勢傢族"}, 	WINDOW_FMMESSAGETYPE_10_MEMONTUM}
	,{{"傢族氣勢列錶"}, 	WINDOW_FMMESSAGETYPE_FM_MEMONTUM}
#endif
	,{{"傢族成員列錶"}, 	WINDOW_FMMESSAGETYPE_DENGON}
	,{{"莊園列錶"}, 			WINDOW_FMMESSAGETYPE_POINTLIST}
	,{{"前三大列錶"}, 		WINDOW_FMMESSAGETYPE_TOP30DP}
	,{{"銀行框"}, 				WINDOW_MESSAGETYPE_BANK}
	,{{"寬對話框"}, 			WINDOW_MESSAGETYPE_WIDEMESSAGE}
	,{{"寬輸入框"}, 			WINDOW_MESSAGETYPE_WIDEMESSAGEANDLINEINPUT}
	,{{"拍賣框"}, 				WINDOW_MESSAGETYPE_AUCTIONNEW}
	,{{"拍賣買框"}, 			WINDOW_MESSAGETYPE_AUCTIONLIST_BUY}
	,{{"拍賣查看框"}, 		WINDOW_MESSAGETYPE_AUCTIONSURVEY}
	,{{"拍賣修改框"}, 		WINDOW_MESSAGETYPE_AUCTIONMODIFY}
	,{{"拍賣列錶修改框"}, WINDOW_MESSAGETYPE_AUCTIONLIST_MODIFY}
	,{{"拍賣輸入框"}, 		WINDOW_MESSAGETYPE_WIDEMESSAGEANDLINEINPUT}
}; 

static CharBase CharBaseButton[] = {
	{{"無按鈕"}, 					WINDOW_BUTTONTYPE_NONE}
	,{{"確定"}, 					WINDOW_BUTTONTYPE_OK}
	,{{"取消"},						WINDOW_BUTTONTYPE_CANCEL}
	,{{"YES"}, 						WINDOW_BUTTONTYPE_YES}      
	,{{"NO"}, 						WINDOW_BUTTONTYPE_NO}
	,{{"上一頁"}, 				WINDOW_BUTTONTYPE_PREV|WINDOW_BUTTONTYPE_CANCEL}
	,{{"下一頁"}, 				WINDOW_BUTTONTYPE_NEXT|WINDOW_BUTTONTYPE_CANCEL}
}; 

static CharBase CharBaseSeqNo[] = {
	{{""}, 								-1}
#ifdef _NEW_STREET_VENDOR
	,{{"擺攤類型"}, 			CHAR_WINDOWTYPE_STREET_VENDOR_TYPE}
#endif
#ifdef _PETSKILL_CANNEDFOOD
	,{{"寵物技能選擇"}, 	ITEM_WINDOWTYPE_SELECTPETSKILL_SELECT}
#endif
#ifdef _ITEM_OVER_LAP
	,{{"道具重疊"}, 			CHAR_WINDOWTYPE_ITEMOVERLAP}
#endif	
}; 
