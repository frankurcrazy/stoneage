 ...
 * 仇木分仃及汔及豳箋及  區毛潸曰丹月儀卞卅月［
 **********************************************/

/* 豳箋及襖    及仿件母丞汔及  劑午      ㄠㄟㄟㄟ坌     */
#define		ITEM_GEN_RAND_MIN		700
#define		ITEM_GEN_RAND_MAX		1200

/* 失奶  丞毛腹綢允月豳箋及襖及汔及  劑午    及    (double) */
...
static struct _tagItemRandRangeTableForItem {
	int	num;			/* 豳箋及備仿件弁及湘  襖 */
	int	minnum;			/* 仿件弁  潸曰丹月襖及Min */
	int	maxnum;			/* 仿件弁  潸曰丹月襖及MAX  num +戚及仿件弁及犒*ITEM_GEN_RATE) */
...
static int ITEM_getAtomIndexByName( char *nm ) // 從素材名稱取得素材index
{
	int i;
...
{
	int i;
//	print ( "\n初始化物品緩衝: 物品最大數:%d ", ITEM_getItemMaxIdNum() );
	icache_num = ITEM_getItemMaxIdNum( );
	print(" 緩衝數:%d ", icache_num);
	...
	print("分配 %4.2f MB 空間...", sizeof( struct ingcache ) * icache_num /1024.0/1024.0);
...
/*
  ...
  base 互 100 匹 min 互 0.7 匹 max 互 1.3 分勻凶日｝
  70 ~ 130 卞剽域卞坌  允月［午曰丐尹內剽域［
  剽域元紮卅仁仄凶曰允月井手仄木卅中及匹｝
  失奶  丞毀迕夥□民件午仄化仇仇卞爛聒允月 by ringo 1999Oct1
  1000坌及min_rate,max_rate匹健丹［
  double 支float 反］支支仇仄中及匹銀歹卅中［
 */
static int
...
/*
  矢永玄及ID井日｝剄醒及膜懇澀爛毛潸曰分允［

  int *fixuse : 襖瑛絆婁醒［    及    醒毛午曰｝爛聒醒毛中木化井尹允

  int *fixatom : 升及豳箋卞覆允月膜懇卅及井
  int *baseup : 湘  襖卞凶中允月笛遙
  int *minadd : 井仃遙及  凝卞凶中允月笛遙
  int *maxadd : 井仃遙及    卞凶中允月笛遙
...
#define MAXMATCH 2048
	...
	while( 1 ) {
		...
		if( first ) {
			for( i = 0; i < icache_num; i++ ){ //  搜尋全部ITEM
				if( icache[i].use && icache[i].canmergeto) { // 如果是可閤成的ITEM
					icache[i].hitnum = 0;
					for( j = 0; j < icache[i].inguse; j ++ ) { // 這個ITEM的材料數
						for( k = 0; k < ingnum ; k ++){ // 閤成的材料數
							if( icache[i].ingind[j] == ingindtable[k] ) { // 材料符閤
								if( searchtable == 0 ) { // 道具閤成的話
									int tablenum = ITEM_getTableNum( ingtable[k]); // 材料等級
									// shan begin
									if( petindex != -1){
										if( CHAR_getInt(petindex, CHAR_PETFAMILY) == 1 ){ // 如果是守護獸的話
											int ownerindex = CHAR_getWorkInt(petindex, CHAR_WORKPLAYERINDEX);
											...
											if( toplimit > 2500) toplimit = 2500;
											// 需符閤上下限
...
#ifdef _COMPOUND_EXP
char mergech[][16] = {"石", "木", "骨", "牙", "皮", "貝殼" , "殼", "爪", "花", "葉", "綫", "黏土", "鱗", "毒", "石化", "混亂", "酒醉", "睡眠", "迴復", "復活", "水晶", "地", "水", "火", "風"};
#endif
...
//#ifdef _VERSION_80
	...
	if( nowtime - CHAR_getWorkInt( charaindex, CHAR_WORKLASTMERGETIME) < 1 ) {
		...
		CHAR_talkToCli( charaindex, -1 ,"閤成料理過於頻繁，休息一下比較好喲。", CHAR_COLORRED);
		//print(" 閤成頻繁 ");
	...
	for(i=0; i<num ; i++ ){
		...
		for(k=0;k<25;k++){
			if(strcmp(items[i].string[ITEM_INGNAME0].string, mergech[k])==0){
				int value = CHAR_getInt(charaindex, CHAR_MATERIAL01 + k) + (items[i].data[ITEM_INGVALUE0]/(150- (rand() % 100)));
				if(value>100000)value=100000;
				CHAR_setInt(charaindex, CHAR_MATERIAL01 + k, value);
			}
			if(strcmp(items[i].string[ITEM_INGNAME1].string, mergech[k])==0){
				int value = CHAR_getInt(charaindex, CHAR_MATERIAL01 + k) + (items[i].data[ITEM_INGVALUE1]/(150- (rand() % 100)));
				if(value>100000)value=100000;
				CHAR_setInt(charaindex, CHAR_MATERIAL01 + k, value);
			}
			if(strcmp(items[i].string[ITEM_INGNAME2].string, mergech[k])==0){
				int value = CHAR_getInt(charaindex, CHAR_MATERIAL01 + k) + (items[i].data[ITEM_INGVALUE2]/(150- (rand() % 100)));
				if(value>100000)value=100000;
				CHAR_setInt(charaindex, CHAR_MATERIAL01 + k, value);
			}
			if(strcmp(items[i].string[ITEM_INGNAME3].string, mergech[k])==0){
				int value = CHAR_getInt(charaindex, CHAR_MATERIAL01 + k) + (items[i].data[ITEM_INGVALUE3]/(150- (rand() % 100)));
				if(value>100000)value=100000;
				CHAR_setInt(charaindex, CHAR_MATERIAL01 + k, value);
			}
			if(strcmp(items[i].string[ITEM_INGNAME4].string, mergech[k])==0){
				int value = CHAR_getInt(charaindex, CHAR_MATERIAL01 + k) + (items[i].data[ITEM_INGVALUE4]/(150- (rand() % 100)));
				if(value>100000)value=100000;
				CHAR_setInt(charaindex, CHAR_MATERIAL01 + k, value);
			}
		}
			...
			for( i = 0 ; i < 5; i++ ) {
				...
				if( created >= 0 ) {
					...
					if (ITEMTBL_getInt( created, ITEM_LEVEL)>= getMaxMergeLevel()){
						CHAR_talkToCli( charaindex, -1 ,"無法閤成超越設定的裝備哦。", CHAR_COLORRED);
						return items[RAND( 0, (num-1))].data[ITEM_ID];
					}else{
						return created;
					}
...
int ITEM_mergeItem_merge( int charaindex, int petid, char *data, int petindex, int alchemist)
{
	...
	
	{
		...
		if( emptyindex == -1 ){
			CHAR_talkToCli( charaindex, -1, "閤成時，最少需空一格物品欄位！", CHAR_COLORYELLOW);
			return -1;
		}
	}
		...
		if( CHAR_CHECKITEMINDEX( charaindex, haveitemindex) ) {
			...
			if( ITEM_CHECKINDEX( itemindex)) {
				...
				if( (code = ITEM_getChar( itemindex, ITEM_TYPECODE)) == "\0" )	{
					if( strcmp( code, "\0") )	{
						char token[256];
						snprintf( token, sizeof( token), "似乎對%s沒有興趣。",
...
#endif
				if( ITEM_getmergeItemFromFromITEMtabl( ITEM_getInt( itemindex, ITEM_ID)) == TRUE ){ //檢查是否可閤成
					ITEM_Item *itm;
					...
					char msgbuf[128];
					snprintf( msgbuf, sizeof( msgbuf), "似乎對%s沒有興趣。",
						ITEM_getChar( itemindex, ITEM_NAME));
	...
	if( cnt > 1 ) {
		...
		if( ret == -10)
			CHAR_talkToCli( charaindex, -1, "非法的閤成方法", CHAR_COLORWHITE);
						...
						if( CHAR_getInt( ownerindex, CHAR_FMLEADERFLAG) != FMMEMBER_LEADER)
							...
							if( randtable == 1){
								...
								if( ITEM_getInt( createitemindex, ITEM_TYPE) == 20){								
									if( (p = strstr( ITEM_getChar(createitemindex, ITEM_ARGUMENT), "氣")) ){
										strcpy( argbuf, (p+2));
										arg1 = atoi(argbuf);
										arg1 = arg1 * FOOD_MP_RATE / 100;
									}
									if( (p = strstr( ITEM_getChar(createitemindex, ITEM_ARGUMENT), "體")) ){
										strcpy( argbuf, (p+2));
										arg2 = atoi(argbuf);
										arg2 = arg2 * FOOD_HP_RATE / 100;
									}
									if( arg1 != 0 && arg2 != 0 ){								
										sprintf( charbuf1, "氣%d體%d", arg1, arg2);
									}
									else if( arg1 != 0){
										sprintf( charbuf1, "氣%d", arg1);
									}
									else if( arg2 != 0){
...
#endif
{
	...
	char codeTemp[][32]={ "NULL", "NULL", "NULL", "NULL", "NULL"
											, "NULL", "NULL", "NULL", "NULL", "NULL"
											, "NULL", "NULL", "NULL", "NULL", "NULL"
											, "NULL", "NULL", "NULL", "NULL", "NULL"
											, "NULL", "NULL", "NULL", "NULL", "NULL"};
	char TypeName[][256]={
		"攻", "防", "敏", "HP", "MP", "幸運", "傷", "吸", "魔法ID"
	};
	int worktyp[] = {
...
#define MAXCANINSLAY 3
	...
	if( i == MAXCANINSLAY )	{
		char token[256];
		sprintf( token, "武器或防具已經鑲滿%d顆或%d顆以上。", MAXCANINSLAY, MAXCANINSLAY);
...
//	print("buf2:%s\n", buf2);
...

	{
		...
		if( (funstr = ITEM_getChar( itemindex, ITEM_ARGUMENT)) != NULL )	{ //參數
		...
		if( ITEM_getInt( inslayindex, ITEM_MAGICID) > 0 )	{	//取得魔法名稱
...
#define MAXFIXNUM 2
	...
	if( crushes >= (maxcrushes*0.80) )	{
		char buff[256];
		sprintf( buff, "%s並沒有損壞到需要修復。", ITEM_getChar( fixindex, ITEM_NAME));
		CHAR_talkToCli( charindex, -1, buff, CHAR_COLORYELLOW);
		return FALSE;
	}else	{
		char *buf1;
		if( maxcrushes < 500 ){
			CHAR_talkToCli( charindex, -1, "此物品已不能修復", CHAR_COLORYELLOW);
			return FALSE;
		}
