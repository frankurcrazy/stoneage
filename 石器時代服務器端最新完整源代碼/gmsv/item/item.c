...
BOOL ITEM_initExistItemsArray( int num )
{
	...
	print("第二次分配 %4.2f MB 空間...", sizeof( ITEM_exists ) * num /1024.0/1024.0);
	return TRUE;
}
...
#define ITEM_ID_TOKEN_INDEX 15
	...
	print("ITEM_tbl分配 %4.2f MB 空間...", sizeof(ITEM_table) * ITEM_tblen /1024.0/1024.0);
	print("ITEM_idx分配 %4.2f MB 空間...", sizeof(ITEM_index) * ITEM_idxlen /1024.0/1024.0);
...
char*  ITEM_makeItemStatusString( int haveitemindex, int itemindex )
{
	...
	if( haveitemindex == -1 ) {
#ifdef _ADD_SHOW_ITEMDAMAGE  // WON ADD 顯示物品耐久度
		char buff1[256];
				...
				if( i == CHAR_EQSHIELD && CHAR_getInt( index, CHAR_WHICHTYPE) != CHAR_TYPEPET){
					...
					if( (ITEM_MODIFYATTACK==itemEffect[j].itemdataintindex && strstr( arg, "攻" ) != NULL )
					  || ( ITEM_MODIFYDEFENCE==itemEffect[j].itemdataintindex && strstr( arg, "防" ) != NULL )
						...
						|| ( ITEM_MODIFYHP==itemEffect[j].itemdataintindex && strstr( arg, "體" ) != NULL )
					  || ( ITEM_MODIFYMP==itemEffect[j].itemdataintindex && strstr( arg, "氣" ) != NULL )
						|| ( ITEM_MODIFYLUCK==itemEffect[j].itemdataintindex && strstr( arg, "運" ) != NULL )
					  || ( ITEM_MODIFYCHARM==itemEffect[j].itemdataintindex && strstr( arg, "魅" ) != NULL )
					  || ( ITEM_MODIFYAVOID==itemEffect[j].itemdataintindex && strstr( arg, "迴" ) != NULL )){
...
void Other_DefcharWorkInt( int index)
{
	...
	//profession fix
	//使用迴避招式時,會將防值減去30%然後設定成迴避值,奇怪的設定,也造成原本設定的迴避值跟沒設一樣,因此我(Change)先把整段拿掉
	...
	if( CHAR_getWorkInt( index, CHAR_WORK_WEAPON) > 0 ){	// 武器專精
		int mpower, mdef;
		mpower = CHAR_getWorkInt( index, CHAR_WORKFIXSTR);
...
void ITEM_reChangeItemToPile( int itemindex)
{
	...
	if( itemID == 20284 ) return;//鐵槍三另外處理
