...
int ITEM_eventDrop( int itemindex, int charaindex, int itemcharaindex )
{
	...
	snprintf( szBuffer, sizeof( szBuffer), "%s 消滅瞭。",
...
static ITEM_EFFECTPARAM ITEM_restorableParam[] = {
    {"hp", "HP迴復瞭。", "HP降低瞭。",CHAR_HP, CHAR_WORKMAXHP},
    {"mp", "MP迴復瞭。", "MP降低瞭。",CHAR_MP, CHAR_WORKMAXMP},
};
    ...
    for( i=0 ; i<arraysizeof(ITEM_restorableParam); i++){
        if( ! strcmp(cmd, ITEM_restorableParam[i].cmd) ){
            ...
            if(value == ITEM_EFFECT_RESTORE_NORMAL){
                ...
                if(curv + amount < 0) amount = -curv;
                CHAR_setInt(charaindex, ITEM_restorableParam[i].element, min((curv+amount), maxv));
                snprintf( ansmsg, sizeof(ansmsg), "%s迴復瞭。",
                ...
                snprintf( ansmsg, sizeof(ansmsg), "%s已完全迴復。", ITEM_restorableParam[i].onmessage );
                CHAR_talkToCli( charaindex, -1, ansmsg, CHAR_COLORWHITE );
                return TRUE;
            }else
                return FALSE;
...
#undef ITEM_STATUSCHANGEVALUE
    strcpysafe( ansmsg, sizeof(ansmsg),"什麽也沒發生。");
    CHAR_talkToCli( charaindex, -1, ansmsg, CHAR_COLORWHITE );
    return FALSE;
}
...
void ITEM_MedicineUsed(int charaindex, int to_charaindex, int itemindex)
{
    ...
    snprintf( ansmsg, sizeof(ansmsg), "抓到瞭%s 。", ITEM_getChar(itemid, ITEM_NAME) );
    CHAR_talkToCli( charaindex, -1, ansmsg, CHAR_COLORWHITE );
    for( p=effectarg ; *p != '\0'; ){
        int i;
        if(*p == SEPARATORI) p++;
...
void ITEM_SandClockDetach( int charaindex , int itemid )
{
    ...
    for( i=0 ; i<CheckCharMaxItem(charaindex) ; i++ ){
        if( CHAR_getItemIndex(charaindex,i ) == itemid ){
            ...
            CHAR_talkToCli( charaindex, -1, "一卸下沙漏，竟忽然壞瞭！", CHAR_COLORWHITE );
	...
	if( OBJECT_getType( moveobjindex ) == OBJTYPE_CHARA ){
    	...
    	if( CHAR_getInt( moveindex , CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
    		snprintf( szBuffer, sizeof( szBuffer ), "%s",
    							ITEM_getAppropriateName(itemindex) );
            CHAR_talkToCli( moveindex	, -1, "%s 消滅瞭。", CHAR_COLORWHITE );
    	}
    }
...
void ITEM_useEffectTohelos( int charaindex, int to_charaindex, int haveitemindex)
{
    ...
    snprintf( msgbuf, sizeof(msgbuf),
              "抓到瞭%s 。", ITEM_getChar(itemindex, ITEM_NAME) );
...
void ITEM_useMic_Field( int charaindex, int to_charaindex, int haveitemindex )
{
	if( CHAR_getWorkInt( charaindex, CHAR_WORKFLG ) & WORKFLG_MICMODE ){
		CHAR_setWorkInt( charaindex, CHAR_WORKFLG,
			CHAR_getWorkInt( charaindex, CHAR_WORKFLG ) & ~WORKFLG_MICMODE );
		CHAR_talkToCli( charaindex, -1, "將麥剋風設定為OFF。", CHAR_COLORWHITE);
	}else{
		CHAR_setWorkInt( charaindex, CHAR_WORKFLG,
			CHAR_getWorkInt( charaindex, CHAR_WORKFLG ) | WORKFLG_MICMODE );
		CHAR_talkToCli( charaindex, -1, "將麥剋風設定為ON。", CHAR_COLORWHITE);
	}
}
#if 1
	...
	if( (p = strstr( arg, "LVUPUP" ) ) != NULL ){
		if (CHAR_getInt(toindex, CHAR_WHICHTYPE)==CHAR_TYPEPET){
			...
			if( CHAR_getInt(toindex, CHAR_LIMITLEVEL) == 140 ){
			    ...
			    CHAR_talkToCli( charaindex,-1,"封印魔咒解除瞭",CHAR_COLORWHITE);
			}
			CHAR_DelItemMess( charaindex, haveitemindex, 0);
			CHAR_complianceParameter( toindex );
		    CHAR_sendCToArroundCharacter( CHAR_getWorkInt( toindex , CHAR_WORKOBJINDEX ));
	...
	if( (p = strstr( arg, "GETEXP" )) != NULL ){
		if (CHAR_getInt(toindex, CHAR_WHICHTYPE)==CHAR_TYPEPET){
			...
			if( atoi(msgbuf) == CHAR_getInt( toindex, CHAR_PETID) ){
				getStringFromIndexWithDelim( arg, "|", 3, msgbuf,sizeof( msgbuf));//等級上限(等級超過纔可用)
				if( CHAR_getInt( toindex, CHAR_LV ) >= atoi(msgbuf) ){
					...
					if( CHAR_getInt( toindex, CHAR_LV) < CHAR_MAXUPLEVEL ){
						int UpLevel = 0;
						CHAR_setWorkInt( toindex, CHAR_WORKGETEXP, atoi(msgbuf) );//迴存CHAR_WORKGETEXP
						CHAR_AddMaxExp( toindex, CHAR_getWorkInt( toindex, CHAR_WORKGETEXP ) );
						sprintf( msgbuf,"這石頭裏麵不知道有什麽樣的礦，讓我看看。(增加經驗%d)", CHAR_getWorkInt( toindex, CHAR_WORKGETEXP ) );
						CHAR_talkToCli( charaindex,-1,msgbuf,CHAR_COLORWHITE);  
				...
				*/
					...
					CHAR_talkToCli( charaindex,-1,"這石頭....(無法判彆，無法增加經驗)",CHAR_COLORWHITE);
			}
			CHAR_DelItemMess( charaindex, haveitemindex, 0);
			CHAR_complianceParameter( toindex );
			CHAR_sendCToArroundCharacter( CHAR_getWorkInt( toindex , CHAR_WORKOBJINDEX ));
...
#define RAND(x,y)   ((x-1)+1+ (int)( (double)(y-(x-1))*rand()/(RAND_MAX+1.0)) )
		...
		if( CHAR_getInt( toindex, CHAR_PETID) == 718 
			...
			if( CHAR_getInt( toindex,CHAR_LV ) < 74 )	{
				sprintf(buf1,"給我的嗎？好美麗的項鏈喔！〈能力起瞭變化〉");
...
void ITEM_usePetSkillCanned_WindowResult( int charaindex, int seqno, int select, char * data)
{
	...
	if(strcmp(ITEM_getChar( itemindex, ITEM_USEFUNC),"ITEM_useSkillCanned")){
		CHAR_talkToCli( charaindex, -1, "禁止非法學習寵物技能！", CHAR_COLORRED);
		return;
	}
	...
	if (isPassiveSkill(SkillID) == 1) {
		...
		if (j <= 0 && getSkillCount() == 1 && SkillNo != getSkillPos()) {
			sprintf(msg, "被動技能隻能學在第%d技能格！", getSkillPos() + 1);
			CHAR_talkToCli(charaindex, -1, msg, CHAR_COLORYELLOW);
			return;
		}

		if (j >= getSkillCount()) {
			sprintf(msg, "被動技能不可超過%d個！", getSkillCount());
			CHAR_talkToCli(charaindex, -1, msg, CHAR_COLORYELLOW);
			return;
		}
	}
	...
	if( seqno != CHAR_WINDOWTYPE_SELECTRENAMEITEM_RENAME ) {
		...
		if( !flg ) {
			...
			if( remain != 0 ) {
				remain --;
				if( remain <= 0 ) {
					snprintf( msgbuf, sizeof(msgbuf),"%s 消失瞭",
...
void ITEM_useLottery( int charaindex, int toindex, int haveitemindex)
{
	...
	if( count >= 6 ) {
		if( hit != ITEM_LOTTERY_NONE ) {
			...
			CHAR_sendItemDataOne( charaindex, haveitemindex);
            snprintf( msgbuff, sizeof( msgbuff), "中瞭第%d奬", hit+1 );
...
//                            CHAR_COLORWHITE );
		...
		if( flg) {
			...
			char space[17];
			space[spc] = '\0';
			snprintf( strbuff, sizeof( strbuff), "%s%s快中瞭", buff, space);
		}
		else {
			strcpy( strbuff, buff);
		}
...
BOOL ITEM_WarpForAny(int charaindex, int ff, int fx, int fy,int flg)
{
	...
	if( CHAR_getWorkInt( charaindex, CHAR_WORKPARTYMODE ) == CHAR_PARTY_LEADER ){
		int i;
		if( flg == 0 )	{//單體
			CHAR_talkToCli(charaindex, -1, "隻能單人使用。", CHAR_COLORYELLOW);
			return FALSE;
		}
...
void ITEM_useWarpForNum( int charaindex, int toindex, int haveitemindex )
{
	...
	int MapPoint[12]={
		...
		char MapString[12][256]={
			...
			"伊甸大陸西方","尼剋斯城區","伊甸大陸南方","亞伊歐城區","伊甸大陸東方",
			"伊甸大陸北方","塔耳塔城區"};
...
void ITEM_petFollow( int charaindex, int toindex, int haveitemindex )
{
	...
	if( CHAR_getWorkInt( charaindex, CHAR_WORKPETFOLLOW ) != -1 ){
		if( CHAR_CHECKINDEX( CHAR_getWorkInt( charaindex, CHAR_WORKPETFOLLOW ) ) ){
			CHAR_talkToCli( charaindex, -1, "請先收迴放齣的寵物！", CHAR_COLORWHITE );
			return;
		}
...
void ITEM_useSkup( int charaindex, int toindex, int haveitemindex)
{
	...
	if(CHAR_getInt(charaindex,CHAR_SUPER)>=1){
		CHAR_talkToCli(charaindex, -1, "已經是極品人。不可以再吃祝福。", CHAR_COLORYELLOW);
		return;
	}
#endif
	...
	CHAR_talkToCli(charaindex, -1, "你感受到自己的能力被提升瞭。", CHAR_COLORWHITE);
...
void ITEM_useNoenemy( int charaindex, int toindex, int haveitemindex)
{
	...
	setNoenemy(fd);
	CHAR_talkToCli(charaindex, -1, "你感受到周邊的殺氣消失瞭。", CHAR_COLORWHITE);
	CHAR_DelItem( charaindex, haveitemindex);
...
void ITEM_equipNoenemy( int charaindex, int itemindex )
{
  ...
  if (evadelevel>=200) {
    setEqNoenemy(fd, 200);
    CHAR_talkToCli(charaindex, -1, "一道奇異的光芒隱藏瞭你的行蹤。", CHAR_COLORWHITE);
    return;
  ...
  CHAR_talkToCli(charaindex, -1, "什麽事也沒有發生。", CHAR_COLORWHITE);
}
...
void ITEM_randEnemyEquipOne( int charaindex, int toindex, int haveitemindex)
{
	...
	if( (RandNum=atoi( buf)) > 0 ){
		...
		CHAR_talkToCli(charaindex, -1, "遇敵率降低瞭。", CHAR_COLORWHITE);
...
void ITEM_RerandEnemyEquip( int charaindex, int itemindex)
{
  ...
  if (RandNum > 0 ) {
    clearEqRandenemy( fd);
    CHAR_talkToCli(charaindex, -1, "遇敵率迴復。", CHAR_COLORWHITE);
    return;
  }
}
#endif
...
void ITEM_WaterWordStatus( int charaindex, int toindex, int haveitemindex)
{
	...
	if( ITEM_getArgument( ITEM_getChar( itemindex, ITEM_ARGUMENT),"time", itemarg, sizeof(itemarg) ) != FALSE ){
		...
		if( nums < 0 ) nums = 0;
		if( nums > 0 ){
			CHAR_talkToCli( charaindex, -1, "水中呼吸時間不得纍積。", CHAR_COLORYELLOW);
			return;
		}
		...
		CHAR_talkToCli(charaindex, -1, "什麽事也沒發生。", CHAR_COLORYELLOW);
	}
}
#endif
...
void ITEM_ChikulaStone( int charaindex, int toindex, int haveitemindex)
{
	...
	CHAR_talkToCli(charaindex, -1, "接受奇剋拉的祝福。", CHAR_COLORWHITE);
	CHAR_setItemIndex(charaindex, haveitemindex ,-1);
	CHAR_sendItemDataOne( charaindex, haveitemindex);
	ITEM_endExistItemsOne(itemindex);
}
...
#endif
	CHAR_talkToCli(charaindex, -1, "你感受到周邊突然充滿殺氣！", CHAR_COLORYELLOW);
	CHAR_DelItem( charaindex, haveitemindex);
}
...
void ITEM_UseDeathCounter( int charaindex, int toindex, int haveitemindex)
{
	...
	if(okfloor != 0){   // 奇怪的石頭
		...
		if( itemmaxuse != -1 )	{
			...
			if( itemmaxuse < 1 )	{
				sprintf( buf1, "%s消失瞭。", ITEM_getChar( itemindex, ITEM_NAME) );
				CHAR_talkToCli(charaindex, -1, buf1, CHAR_COLORYELLOW);
				CHAR_DelItem( charaindex, haveitemindex);
				if(Useflag==FALSE) {
	...
	if( itemmaxuse != -1 )	{
		...
		ITEM_setInt( itemindex, ITEM_DAMAGEBREAK, itemmaxuse);
		if( itemmaxuse < 1 )	{
			sprintf( buf1, "道具 %s消失瞭。", ITEM_getChar( itemindex, ITEM_NAME) );
			CHAR_talkToCli(charaindex, -1, buf1, CHAR_COLORYELLOW);
			CHAR_DelItem( charaindex, haveitemindex);
		}else{
			sprintf( buf1, "原地遇敵，可使用次數剩餘%d次。", itemmaxuse);
...
void ITEM_useMaxRedSocks( int charaindex, int toindex, int haveitemindex)
{
	...
	if( nowtimes >= itemtimes && nowtimes <= itemtimes+(60*60*24) ){ //限製時間內可換禮物
		int si=0, ret;
		char token[256];
		//刪除
		CHAR_setItemIndex(charaindex, haveitemindex ,-1);
		...
		if( nowtimes < itemtimes ){
			...
			second = defTimes;
			sprintf( token,"%s還差%d天%d小時%d分%d秒纔可使用!",
...
void ITEM_metamo( int charaindex, int toindex, int haveitemindex )
{
	...
	if( haveindex == -1 && charaindex != toindex ){
		CHAR_talkToCli( charaindex, -1, "無法變身，隻能變成自己的寵物！", CHAR_COLORYELLOW );
		return;
	}
...
void ITEM_Cracker(int charaindex,int toindex,int haveitemindex)
{
	...
	else CHAR_talkToCli(charaindex,-1,"什麽也沒發生。",CHAR_COLORWHITE);
}
#endif
...
void ITEM_AddPRSkillPercent( int charaindex,int toindex,int haveitemindex)
{
    ...
    CHAR_sendCToArroundCharacter( CHAR_getWorkInt( charaindex , CHAR_WORKOBJINDEX ));
	sprintf( token, "所有職業技能熟練度上升%d％。", level);
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
#endif
}
...
void ITEM_AddPRSkillPoint( int charaindex,int toindex,int haveitemindex)
{
	...
	CHAR_talkToCli( charaindex, -1, "增加一點職業技能點數。", CHAR_COLORYELLOW);
	sprintf( token, "增加%d點職業技能點數。", point);
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
#endif
}
...
void ITEM_Addexp(int charaindex,int toindex,int haveitemindex)
{
	// 檢查玩傢是否有效
	if(CHAR_CHECKINDEX(charaindex) == FALSE) return; //失敗
	if(CHAR_CHECKINDEX(toindex) == FALSE) return; //失敗
	int i;
	for( i =0; i < CHAR_MAXPETHAVE; i++){
		if( CHAR_getCharPet( charaindex, i) == toindex ){
			break;
	...
	if(i == CHAR_MAXPETHAVE){
		if(charaindex != toindex){
			CHAR_talkToCli(charaindex,-1,"隻能為自己或自己的寵物使用！",CHAR_COLORYELLOW);
			return;
		}
	}
#if 1
...
void ITEM_firecracker(int charaindex,int toindex,int haveitemindex)
{
	...
	if( battlemode // 檢查是否在戰鬥中
		...
		CHAR_talkToCli(charaindex,-1,"什麽也沒發生。",CHAR_COLORWHITE);
}
#endif
//Terry end
...
{//CHAR_LOWRIDEPETS
	...
	if(getRideMode()==2 || getRideMode()==4){
		CHAR_talkToCli( charaindex, -1, "服務端為2.0非騎證模式，所以你無法使用該物品", CHAR_COLORYELLOW);
		return;
	}
#endif
	int itemindex, i;
		...
		if( !strcmp( NewRides[i].arg, buf1))	{
			...
			if(CHAR_getInt( charaindex, CHAR_TRANSMIGRATION) < ridetrans){
				...
				if( getStringFromIndexWithDelim(itemarg,"|", 2, buf1, sizeof(buf1)) != FALSE ){
					sprintf( token, "必須%d轉人以上纔能學習騎%s。",ridetrans,buf1);
					CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
					return;
				}
			}
...
void ITEM_useFusionEditBase( int charaindex, int toindex, int haveitemindex)
{
	...
	if( CHAR_getInt( toindex, CHAR_FUSIONBEIT) >=1 &&
		CHAR_getInt( toindex, CHAR_FUSIONRAISE) > 0 ){//檢查是否為融閤寵
		int time_l;
...
//				sscanf( arg, "消 %d", &deltime);
				...
				CHAR_DelItem( charaindex, haveitemindex);
				sprintf( msg, "加快寵物蛋的消化速度 %s 分鍾。", deltime);
				CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW);
				if( (time_l + anhour) <= nowTime ) {
					...
					min = deftime/60;
					sec = deftime%60;
					sprintf( msg, "寵物蛋已經可以喂食瞭，超過喂食時間%d分%d秒。", min, sec);
					CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW);
					return;
				}
			}
...
#endif
		if( (time_l + anhour) > nowTime ){ //現在的時間要超過(time_l+anhour)纔可以  食
			char Mess1[256];
			int min, sec, deftime;
			...
			min = deftime/60;
			sec = deftime%60;
			sprintf( Mess1, "寵物蛋需過%d分%d秒纔可喂食。", min, sec);
...
void ITEM_LoverSelectUser( int charaindex, int toindex, int haveitemindex)
{
	...
	if( !strcmp( ITEM_getChar( itemindex, ITEM_FORUSERNAME), "") ||
		!strcmp( ITEM_getChar( itemindex, ITEM_FORUSERCDKEY), "") ){//尋找設定對象
		if( charaindex == toindex ){
		...
		ITEM_setInt( itemindex, ITEM_TARGET, 0);
		{
			...
			sprintf( token, "傳送對象設定為%s。", CHAR_getChar( toindex, CHAR_NAME));
			CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
		}
		CHAR_sendItemDataOne( charaindex, haveitemindex);
	}else{
		...
		for( i=0; i<playernum; i++)	{
			...
			//	floor == 5032 || floor == 6032 ||floor == 7032 || floor == 8032 || floor == 9032 ){ 
				if( checkUnlawWarpFloor( floor) ) {
					CHAR_talkToCli( charaindex, -1, "對象所在地方無法傳送。", CHAR_COLORYELLOW );
					return;
				}
				...
				if( itemmaxuse != -1 )	{
					...
					if( itemmaxuse < 1 )	{
						...
						sprintf( buf1, "傳送至目標對象所在位置，可使用次數剩馀%d次。", itemmaxuse);
						ITEM_setChar( itemindex, ITEM_EFFECTSTRING, buf1);
					}
				}else{
					itemmaxuse = 10;
					ITEM_setInt( itemindex, ITEM_DAMAGEBREAK, itemmaxuse);
					sprintf( buf1, "傳送至目標對象所在位置，可使用次數剩餘%d次。", itemmaxuse);
					ITEM_setChar( itemindex, ITEM_EFFECTSTRING, buf1);
				}
		...
		CHAR_talkToCli( charaindex, -1, "對象不在此伺服器或不在綫上。", CHAR_COLORYELLOW );
	}
}
#endif
...
void ITEM_changePetOwner( int charaindex, int toindex, int haveitemindex)
{
	...
	if( CHAR_getInt( toindex, CHAR_WHICHTYPE) != CHAR_TYPEPET ) {
		CHAR_talkToCli( charaindex, -1, "這隻能用在寵物身上喔。", CHAR_COLORYELLOW );
		return;
	}
	...
	if( !strcmp( CHAR_getChar( toindex, CHAR_NPCARGUMENT), "") || 
		...
		CHAR_talkToCli( charaindex, -1, "這隻寵物本來就是你的，並不需要使用這個呀。", CHAR_COLORYELLOW);
		return;
	}
	...
	CHAR_talkToCli( charaindex, -1, "現在你可以幫這隻寵物改名字瞭。", CHAR_COLORYELLOW );
...
void ITEM_timeticketEx( int charaindex, int toindex, int haveitemindex, int flag)
{
	...
	if( tickettime > nowtime+20 ) {
		CHAR_talkToCli( charaindex, -1, "時間必須剩下２０秒以內纔可使用。", CHAR_COLORYELLOW);
		return;
	}
	// 第一次使用的話
	if( tickettime == 0 ) {
...
void ITEM_SetLoverUser( int charaindex, int toindex, int haveitemindex)
{
   ...
   if( !strcmp( CHAR_getChar( charaindex, CHAR_LOVE), "YES"))
        {
            CHAR_talkToCli( charaindex, -1, "重婚是違法的！您已經結婚瞭哦~", CHAR_COLORYELLOW);
            return;
        }
   if( !CHAR_CHECKINDEX( charaindex ) ) return;
   if( !CHAR_CHECKINDEX( toindex ) ) return;
   if( CHAR_getInt( toindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
   {
   		 CHAR_talkToCli( charaindex, -1, "隻能和玩傢結婚哦", CHAR_COLORYELLOW);
       return;
   }
   if( CHAR_getInt(charaindex,CHAR_TRANSMIGRATION)<3 ||CHAR_getInt(toindex,CHAR_TRANSMIGRATION)<3)
   {
   	   CHAR_talkToCli( charaindex, -1, "如果想結婚的話，雙方必須3轉以上哦~", CHAR_COLORYELLOW);
       return;
   }
   if( charaindex == toindex )
   {
       CHAR_talkToCli( charaindex, -1, "難道您想和自己結婚嗎？", CHAR_COLORYELLOW);
       return;
   }
   if ( IsMale(charaindex)==IsMale(toindex))
   {
       CHAR_talkToCli( charaindex, -1, "同性怎麼結婚呢？", CHAR_COLORYELLOW);
       return;    
   }
   if( !ITEM_CHECKINDEX(itemindex) ) return;
			int id = ITEM_getInt( itemindex, ITEM_ID);
   ...
   if( strcmp( CHAR_getChar( charaindex, CHAR_LOVE), "YES") &&
   			strcmp( CHAR_getChar( toindex, CHAR_LOVE), "YES"))//第二次判斷雙方是否已經結婚
   {
	    if( itemindex != -1 ){
	    	 ...
	    	 if (strcmp(CHAR_getChar( toindex, CHAR_LOVERID), CHAR_getChar( charaindex, CHAR_CDKEY))==0 &&
         		...
         		int i;
            sprintf( token, "已經答應瞭%s的求婚", CHAR_getChar( toindex, CHAR_NAME));
		        CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
		        sprintf( token, "%s已經響應瞭你的求婚", CHAR_getChar( charaindex, CHAR_NAME));
            ...
            {    
		           ...
		           sprintf( token, "恭喜%s和%s新婚之喜，恭祝他們百年好閤.白頭到老.",
														           CHAR_getChar( charaindex, CHAR_NAME),
														           CHAR_getChar( toindex, CHAR_NAME));
		        ...
		        return;
						...
						if( emptyitemindexinchara < 0 ){
								sprintf( token, "%s物品欄已滿。求婚失敗" , CHAR_getChar( toindex, CHAR_NAME));
								CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
								sprintf( token, "%s嚮您發起求婚，但您的物品欄已滿。" , CHAR_getChar( charaindex, CHAR_NAME));
								CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);}
						else{
						 		CHAR_setItemIndex( toindex, emptyitemindexinchara, itemindex);
						   	sprintf( token, "%s嚮%s求婚的戒指!同意求婚請使用!", CHAR_getChar( charaindex, CHAR_NAME)
																																							,CHAR_getChar( toindex, CHAR_NAME));
						...
						sprintf( token, "%s嚮您求婚，獲得對方的%s", CHAR_getChar( charaindex, CHAR_NAME),ITEM_getChar( itemindex, ITEM_NAME));
						CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);
						sprintf( token, "已經嚮%s發起求婚", CHAR_getChar( toindex, CHAR_NAME));
						CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
						CHAR_DelItem( charaindex, haveitemindex);
				}
				CHAR_talkToCli( charaindex, -1, "請耐心等待對方迴應！", CHAR_COLORYELLOW );
			}else{
					sprintf( token, "求婚失敗，戒指無法創建!" );
					CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
					return;
			}
   }
...
void ITEM_LoverWarp( int charaindex, int toindex, int haveitemindex)
{
  ...
  {   
    for(i=0;i<playernum;i++)
    {
        ...
        if (i>=playernum)
          {
            CHAR_talkToCli( charaindex, -1, "很抱歉，您的愛人不在綫哦~", CHAR_COLORYELLOW );
            return;
          }
  ...
  if( !ITEM_CHECKINDEX(itemindex) ) return;
    ...
    if( checkUnlawWarpFloor( CHAR_getInt( charaindex, CHAR_FLOOR) ) ) {
        CHAR_talkToCli( charaindex, -1, "很抱歉，您的愛人在神秘遙遠地方，無法傳送。", CHAR_COLORYELLOW );
        return;
    }//UNWARP地圖禁止使用
    if( checkUnlawWarpFloor( CHAR_getInt( i, CHAR_FLOOR) ) ) {
        ...
        if(strcmp(CHAR_getChar(i,CHAR_LOVERID),CHAR_getChar(charaindex,CHAR_CDKEY)) == 0 &&
          ...
          CHAR_warpToSpecificPoint( charaindex, floor, x, y );
          sprintf( token, "%s:親愛的，我來瞭！", CHAR_getChar( charaindex, CHAR_NAME));
          CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW );
          sprintf( token, "已傳送到愛人%s身邊！", CHAR_getChar( i, CHAR_NAME));
          CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
          print("\n玩傢%s 傳送到玩傢%s身邊", CHAR_getChar( charaindex, CHAR_NAME),CHAR_getChar(i, CHAR_NAME));
          return;
        }
    }else
    CHAR_talkToCli( charaindex, -1, "您沒有結婚哦~", CHAR_COLORYELLOW );
}
...
void ITEM_LoverUnmarry( int charaindex, int toindex, int haveitemindex)
{
  ...
  if( !ITEM_CHECKINDEX(itemindex) ) return;
  	 ...
  	 if(strcmp(ITEM_getChar( itemindex, ITEM_SECRETNAME), "同意離婚戒指")){
				int id = ITEM_getInt( itemindex, ITEM_ID);
				itemindex = ITEM_makeItemAndRegist( id );
		    ...
		    if( itemindex != -1 ){
		   			...
		   			int emptyitemindexinchara = CHAR_findEmptyItemBox( i );
						...
						if( emptyitemindexinchara < 0 ){
								sprintf( token, "%s物品欄已滿。離婚失敗" , CHAR_getChar( i, CHAR_NAME));
								CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
								sprintf( token, "%s嚮您發起離婚，但您的物品欄已滿。" , CHAR_getChar( charaindex, CHAR_NAME));
								CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW);}
					 	else{
					 		CHAR_setItemIndex( i, emptyitemindexinchara, itemindex);
				      ITEM_setChar( itemindex, ITEM_NAME, "同意離婚戒指");
				      ITEM_setChar( itemindex, ITEM_SECRETNAME, "同意離婚戒指");
			      	sprintf( token, "%s嚮你申請離婚，如果同意請使用該戒指!", CHAR_getChar( charaindex, CHAR_NAME));
				      ITEM_setChar( itemindex, ITEM_EFFECTSTRING, token);
			      	CHAR_sendItemDataOne( i, emptyitemindexinchara);
			      	ITEM_endExistItemsOne(itemindex);
		       		sprintf( token, " %s嚮您提齣離婚並把 %s 給你！", CHAR_getChar( charaindex, CHAR_NAME),ITEM_getChar( itemindex, ITEM_NAME));
		       		CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW);
		       		sprintf( token, "已經嚮%s發起離婚", CHAR_getChar( i, CHAR_NAME));
		       		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
			      	CHAR_DelItem( charaindex, haveitemindex);
	          	return;}}
      }else{
		      ...
		      CHAR_setChar( charaindex, CHAR_LOVERNAME,"");
	       	CHAR_talkToCli( i, -1, "雙方離婚成功！", CHAR_COLORYELLOW);
	      	CHAR_talkToCli( charaindex, -1, "雙方離婚成功！", CHAR_COLORYELLOW);}
	      	CHAR_DelItem( charaindex, haveitemindex);
    }else
    CHAR_talkToCli( charaindex, -1, "您沒有結婚哦~", CHAR_COLORYELLOW );
}
...
{
		if(CHAR_getInt( charaindex , CHAR_RIDEPET) !=1 ){
			CHAR_talkToCli( charaindex, -1, "騎乘中無法使用該道具~", CHAR_COLORYELLOW);
		}
		int MetamoList[13][7]={
		/*{  造型 ,   紅  ,   綠  ,   黃  ,   灰  ,   白  ,   黑  },  //此行為說明行*/
			{ 100000, 100000, 100005, 100010, 100015, 100700, 100705},	//小矮子
			{ 100020, 100025, 100030, 100035, 100020, 100710, 100715},	//賽亞人
			{ 100040, 100055, 100050, 100045, 100040, 100720, 100725},	//辮子男孩
			{ 100060, 100060, 100065, 100070, 100075, 100730, 100735},	//酷哥
			{ 100080, 100095, 100085, 100090, 100080, 100740, 100745},	//熊皮男
			{ 100100, 100100, 100115, 100110, 100105, 100750, 100755},	//大個
			...
			{ 100180, 100190, 100195, 100185, 100180, 100790, 100795},	//短發夾妹
		...
		for(i=0;i<12;i++)
			if((OldMetamoId>=MetamoList[i][0] && OldMetamoId<MetamoList[i+1][0])
				  || (OldMetamoId>=MetamoList[i][5] && OldMetamoId<MetamoList[i+1][5]))
					break;
		if(i==12){
				CHAR_talkToCli( charaindex, -1, "變色失敗，你造型並不是人物造型", CHAR_COLORYELLOW);
				return;
		}
		if( strstr( itemarg, "紅" ) ){
				CHAR_talkToCli( charaindex, -1, "變成紅色", CHAR_COLORYELLOW);
				NewMetamoId=MetamoList[i][1];
		}else if( strstr( itemarg, "綠" ) ){
				CHAR_talkToCli( charaindex, -1, "變成綠色", CHAR_COLORYELLOW);
				NewMetamoId=MetamoList[i][2];
		}else if( strstr( itemarg, "黃" ) ){
				CHAR_talkToCli( charaindex, -1, "變成黃色", CHAR_COLORYELLOW);
				NewMetamoId=MetamoList[i][3];
		}else if( strstr( itemarg, "灰" ) ){
				CHAR_talkToCli( charaindex, -1, "變成灰色", CHAR_COLORYELLOW);
				...
				switch( CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) ){
					...
					case 10031:
						...
						{
#endif
							CHAR_talkToCli( charaindex, -1, "變成白色", CHAR_COLORYELLOW);
							NewMetamoId=MetamoList[i][5];
#ifdef _FM_METAMO
							break;
						}
					default:
						CHAR_talkToCli( charaindex, -1, "該戒指隻能擁有莊園的光明傢族成員使用！", CHAR_COLORYELLOW);
						return;
				}
#endif
				...
				switch( CHAR_getWorkInt( charaindex, CHAR_WORKFMFLOOR) ){
					...
					case 10031:
						...
						{
#endif
							CHAR_talkToCli( charaindex, -1, "變成黑色", CHAR_COLORYELLOW);
							NewMetamoId=MetamoList[i][6];
#ifdef _FM_METAMO
							break;
						}
					default:
						CHAR_talkToCli( charaindex, -1, "該戒指隻能擁有莊園的黑暗傢族成員使用！", CHAR_COLORYELLOW);
						return;
				}
#endif
		}else if( strstr( itemarg, "隨" ) ){
				srand(time(0));
				do
						NewMetamoId=MetamoList[i][rand()%4+1];
				while(NewMetamoId==OldMetamoId);
				CHAR_talkToCli( charaindex, -1, "隨機變色成功", CHAR_COLORYELLOW);
		}else{
				CHAR_talkToCli( charaindex, -1, "變色失敗", CHAR_COLORYELLOW);
			return;
		}
...
void ITEM_CharaMetamo( int charaindex, int toindex, int haveitemindex)
{
		...
		if((OldMetamoId >= 100000 && OldMetamoId < 100240 )
			...
			if( strstr( itemarg, "豆丁囝" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					CHAR_talkToCli( charaindex, -1, "更改造型為豆囝", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[0][rand()%4+1];
			}else if( strstr( itemarg, "賽亞人" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					CHAR_talkToCli( charaindex, -1, "更改造型為賽亞人", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[1][rand()%4+1];
			}else if( strstr( itemarg, "辮子男孩" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					CHAR_talkToCli( charaindex, -1, "更改造型為辮子男孩", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[2][rand()%4+1];
			}else if( strstr( itemarg, "酷哥" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					CHAR_talkToCli( charaindex, -1, "更改造型為酷哥", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[3][rand()%4+1];
			}else if( strstr( itemarg, "熊皮男" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					CHAR_talkToCli( charaindex, -1, "更改造型為熊皮男", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[4][rand()%4+1];
			}else if( strstr( itemarg, "大隻佬" ) && OldMetamoId >= 100000 && OldMetamoId < 100120){
					CHAR_talkToCli( charaindex, -1, "更改造型為大隻佬", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[5][rand()%4+1];
			}else if( strstr( itemarg, "豆丁囡" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為豆丁囡", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[6][rand()%4+1];
			}else if( strstr( itemarg, "熊皮妹" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為熊皮妹", CHAR_COLORYELLOW);
  				NewMetamoId=MetamoList[7][rand()%4+1];
			}else if( strstr( itemarg, "帽子妹" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為帽子妹", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[8][rand()%4+1];
			}else if( strstr( itemarg, "短發夾妹" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為短發夾妹", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[9][rand()%4+1];
			}else if( strstr( itemarg, "手套女" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為手套女", CHAR_COLORYELLOW);
					NewMetamoId=MetamoList[10][rand()%4+1];
			}else if( strstr( itemarg, "辣妹" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					CHAR_talkToCli( charaindex, -1, "更改造型為辣妹", CHAR_COLORYELLOW);
			...
			}else if( strstr( itemarg, "賽亞人" ) && OldMetamoId >= 100700 && OldMetamoId < 100760){
					CHAR_talkToCli( charaindex, -1, "更改造型為賽亞人", CHAR_COLORYELLOW);
					if( ( OldMetamoId - 100700 ) % 10 == 0 )
						  NewMetamoId=MetamoList[1][5];
					else if( ( OldMetamoId - 100700 ) % 10 == 5 )
						  NewMetamoId=MetamoList[1][6];
			}else if( strstr( itemarg, "辮子男孩" ) && OldMetamoId >= 100700 && OldMetamoId < 100760){
					CHAR_talkToCli( charaindex, -1, "更改造型為辮子男孩", CHAR_COLORYELLOW);
					if( ( OldMetamoId - 100700 ) % 10 == 0 )
			...
			}else if( strstr( itemarg, "大隻佬" ) && OldMetamoId >= 100700 && OldMetamoId < 100760){
					CHAR_talkToCli( charaindex, -1, "更改造型為大隻佬", CHAR_COLORYELLOW);
					if( ( OldMetamoId - 100700 ) % 10 == 0 )
			...
			}else if( strstr( itemarg, "短發夾妹" ) && OldMetamoId >= 100760 && OldMetamoId < 100820){
					CHAR_talkToCli( charaindex, -1, "更改造型為短發夾妹", CHAR_COLORYELLOW);
								if( ( OldMetamoId - 100700 ) % 10 == 0 )
			...
			}else if( strstr( itemarg, "隨機" ) && OldMetamoId >= 100120 && OldMetamoId < 100240){
					do
					...
					while(NewMetamoId==OldMetamoId);
					CHAR_talkToCli( charaindex, -1, "隨機更改造型成功", CHAR_COLORYELLOW);
			}
		}else{
			CHAR_talkToCli( charaindex, -1, "你並不是人物造型!", CHAR_COLORYELLOW);
			return;
		}
		if(NewMetamoId==0){
			CHAR_talkToCli( charaindex, -1, "你的性彆不同，無沒更改此造型!", CHAR_COLORYELLOW);
			return;}
		CHAR_setInt( charaindex , CHAR_BASEIMAGENUMBER , NewMetamoId );
		CHAR_setInt( charaindex , CHAR_BASEBASEIMAGENUMBER , NewMetamoId );
...
void ITEM_SexMetamo( int charaindex, int toindex, int haveitemindex)
{
		...
		if(OldMetamoId >=100000 && OldMetamoId <100020){                 //豆丁囝
				...
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成豆丁囡!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100020 && OldMetamoId <100040){           //賽亞人
				NewMetamoId=OldMetamoId+160;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成短發夾妹!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100040 && OldMetamoId <100060){           //辮子男孩
				NewMetamoId=OldMetamoId+120;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成帽子妹!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100060 && OldMetamoId <100080){           //酷哥
				NewMetamoId=OldMetamoId+160;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成辣妹!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100080 && OldMetamoId <100100){           //熊皮男
				NewMetamoId=OldMetamoId+60;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成熊皮妹!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100100 && OldMetamoId <100120){           //大隻佬
				NewMetamoId=OldMetamoId+100;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成手套女!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100120 && OldMetamoId <100140){           //豆丁囡
				NewMetamoId=OldMetamoId-120;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成豆丁囝!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100140 && OldMetamoId <100160){           //熊皮妹
				NewMetamoId=OldMetamoId-60;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成熊皮男!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100160 && OldMetamoId <100180){           //帽子妹
				NewMetamoId=OldMetamoId-120;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成辮子男孩!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100180 && OldMetamoId <100200){           //短發夾妹
				NewMetamoId=OldMetamoId-160;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成賽亞人!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100200 && OldMetamoId <100220){           //手套女
				NewMetamoId=OldMetamoId-100;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成大隻佬!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100220 && OldMetamoId <100240){           //辣妹
				NewMetamoId=OldMetamoId-160;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成酷哥!", CHAR_COLORYELLOW);
		}else if(OldMetamoId >=100700 && OldMetamoId <100710){          //豆丁囝
				NewMetamoId=OldMetamoId+60;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成豆丁囡!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100710 && OldMetamoId <100720){           //賽亞人
				NewMetamoId=OldMetamoId+80;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成短發夾妹!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100720 && OldMetamoId <100730){           //辮子男孩
				NewMetamoId=OldMetamoId+60;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成帽子妹!", CHAR_COLORYELLOW);
				...
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成熊皮妹!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100750 && OldMetamoId <100760){           //大隻佬
				...
				NewMetamoId=OldMetamoId-60;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成辮子男孩!", CHAR_COLORYELLOW);
		}else if(OldMetamoId>=100790 && OldMetamoId <100800){           //短發夾妹
				NewMetamoId=OldMetamoId-80;
				CHAR_talkToCli( charaindex, -1, "恭喜你變性成賽亞人!", CHAR_COLORYELLOW);
				...
				CHAR_talkToCli( charaindex, -1, "你並不是人物造型，無法變性!", CHAR_COLORYELLOW);
				return;}
		if(NewMetamoId==0)return;
		CHAR_setInt( charaindex , CHAR_BASEIMAGENUMBER , NewMetamoId );
		CHAR_setInt( charaindex , CHAR_BASEBASEIMAGENUMBER , NewMetamoId );
...
void ITEM_GMFUNCTION( int charaindex, int toindex, int haveitemindex)
{
		...
		sprintf( token, "獲得使用%s權限%d!", gmfunction, atoi(gmtime));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
		sprintf( token, "相關使用請查看[help %s]無需GM密碼!",CHAR_getChar( charaindex, CHAR_GMFUNCTION));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
		CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_AddMemberPoint( int charaindex, int toindex, int haveitemindex)
{
		...
		sprintf( token, "獲得積分點數%d,目前你擁有積分點數為%d!", atoi(itemarg),point+atoi(itemarg));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
		CHAR_DelItem( charaindex, haveitemindex);
...
void ITEM_NewName( int charaindex, int toindex, int haveitemindex)
{
		...
		sprintf( token, "恭喜你,你已獲得〖%s〗新稱號!", itemarg);
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
		int i;
		int playernum = CHAR_getPlayerMaxNum();
		char NameMsg[256];
		...
		for( i = 0 ; i < playernum ; i++) 
		{
       sprintf( NameMsg, "恭喜玩傢 %s 獲得≮%s≯稱號!", MyName, itemarg );
...
void ITEM_VipRide( int charaindex, int toindex, int haveitemindex)
{
		...
		if(atoi(token)>0){
			...
			sprintf( token, "您的會員有效使用期至 %d年%d月%d日 %d:%d:%d\n",
																											tm1.tm_year + 1900,
																											tm1.tm_mon + 1,
																											tm1.tm_mday,
																											tm1.tm_hour,
			...
			CHAR_talkToCli( charaindex, -1, "您的會員有效使用期為永久！", CHAR_COLORYELLOW );
		}
#ifdef _NEW_RIDEPETS	
		...
		for( i = 0 ; i < playernum ; i++) 
		{
	    sprintf( token, "恭喜玩傢 %s 成為 『%s』 正式 %d級 會員！", CHAR_getChar( charaindex,CHAR_NAME ), getGameservername(), CHAR_getInt( charaindex , CHAR_VIPRIDE));
...
void ITEM_AddFame( int charaindex, int toindex, int haveitemindex)
{
		...
		sprintf( token, "聲望點數提升%d,目前你的聲望是%d!", atoi(itemarg),fame/100+atoi(itemarg));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
...
void ITEM_Luck( int charaindex, int toindex, int haveitemindex)
{
		...
		sprintf( token, "你現在的運氣為%s", luck[i]);
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
		CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_MetamoTime( int charaindex, int toindex, int haveitemindex )
{
	...
	CHAR_setWorkInt( charaindex, CHAR_WORKITEMMETAMO, NowTime.tv_sec + metamoTime);
	if(metamoTime>60)
		sprintf( msg, "變身成%s，可持續%d小時！", buff, metamoTime / 60 );
	else
		sprintf( msg, "變身成%s，可持續%d分鍾！", buff, metamoTime );
...
void ITEM_Gold( int charaindex, int toindex, int haveitemindex)
{
	  ...
	  CHAR_send_P_StatusString( charaindex, CHAR_P_STRING_GOLD);
		sprintf( token, "你的石幣增加%dS", atoi(itemarg));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );		
		CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_MysteriousGift( int charaindex, int toindex, int haveitemindex)
{
	...
	itemarg = ITEM_getChar( itemindex, ITEM_ARGUMENT);
	if( itemarg == "" ){
		CHAR_talkToCli(charaindex, -1, "這是個神秘禮物!", CHAR_COLORYELLOW);
		return;
	}
  ...
  if( i == CHAR_MAXPETHAVE ){
		snprintf( token,sizeof( token), "寵物已滿！！");
		CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORYELLOW);
		return;
	}	
...
void ITEM_BattlePK( int charaindex, int toindex, int haveitemindex)
{
	...
	if( CHAR_getInt( toindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
	{
	   CHAR_talkToCli( charaindex, -1, "隻能挑戰玩傢哦！", CHAR_COLORYELLOW);
	   return;
	}
	if( charaindex == toindex )
	{
	   CHAR_talkToCli( charaindex, -1, "難道有自己挑戰自己的嗎？", CHAR_COLORYELLOW);
	   return;
	}
	CHAR_DischargePartyNoMsg( charaindex );//解散團隊
	...
	sprintf( token,"成功嚮對方%s挑戰!",CHAR_getChar( toindex, CHAR_NAME));
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
	sprintf( token,"對方%s正式嚮你挑戰!",CHAR_getChar( charaindex, CHAR_NAME));
	CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);
	CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_PetLevelItem( int charaindex, int toindex, int haveitemindex)
{
  ...
  if( CHAR_getInt( toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
  {
  	CHAR_talkToCli( charaindex, -1, "該物品隻能給寵物使用！", CHAR_COLORYELLOW);
    return;
  }
	if( CHAR_getInt( toindex, CHAR_PETID) == 718 || CHAR_getInt( toindex, CHAR_PETID) == 401 )
	{
		CHAR_talkToCli( charaindex, -1, "該物品不能給MM使用！", CHAR_COLORYELLOW);
		return;
	}
	int itemindex = CHAR_getItemIndex( charaindex, haveitemindex);
	if(!ITEM_CHECKINDEX(itemindex)) return;
	...
	if(CHAR_getInt( toindex, CHAR_LV)<atoi(low) && (CHAR_getInt(toindex, CHAR_LIMITLEVEL)>0 && CHAR_getInt(toindex, CHAR_LIMITLEVEL)<atoi(low))){
		CHAR_talkToCli( charaindex, -1, "目前你的寵物無法使用該物品！", CHAR_COLORYELLOW);
		return;
	}else	if(CHAR_getInt( toindex, CHAR_LV)>=atoi(hight) && CHAR_getInt(toindex, CHAR_LIMITLEVEL)>=atoi(hight)){
		CHAR_talkToCli( charaindex, -1, "目前你的寵物不必再使用該物品！", CHAR_COLORYELLOW);
		return;
	}
	...
	char token[256];
	sprintf(token, "寵物%s等級已突破%d級！",CHAR_getChar( toindex, CHAR_NAME), atoi(hight));
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
	CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
void ITEM_efMetamo( int charaindex, int toindex, int haveitemindex)
{
	...
	char *itemarg = ITEM_getChar( itemindex, ITEM_ARGUMENT);
	if( itemarg == "" ){
		CHAR_talkToCli(charaindex, -1, "這是個永久變身戒指!", CHAR_COLORYELLOW);
		return;
	}
...
void PET_BEATITUDE( int charaindex, int toindex, int haveitemindex)
{
  ...
  if( CHAR_getInt( toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
  {
	...
	if(strcmp( beatitude, "體")==0){
		if(CHAR_getInt( toindex, CHAR_BEATITUDE)&BEATITUDE_VITAL){
			CHAR_talkToCli( charaindex, -1, "您的寵物該項屬性已提升過瞭！", CHAR_COLORYELLOW);
			return;
		}else{
			sprintf(token, "%s體力上升%d", CHAR_getChar( toindex, CHAR_NAME),atoi(mun));
		...
		if(CHAR_getInt( toindex, CHAR_BEATITUDE)&BEATITUDE_STR){
			...
			sprintf(token, "%s攻擊力上升%d", CHAR_getChar( toindex, CHAR_NAME),atoi(mun));
		...
		if(CHAR_getInt( toindex, CHAR_BEATITUDE)&BEATITUDE_TOUGH){
			...
			sprintf(token, "%s防禦力上升%d", CHAR_getChar( toindex, CHAR_NAME),atoi(mun));
		...
		sprintf(token, "%s當前屬性為體:%d,攻:%d,防:%d,敏:%d", CHAR_getChar( toindex, CHAR_NAME),
	...
	CHAR_talkToCli( charaindex, -1, "主人，這東西太棒瞭，我已充滿力量瞭！", CHAR_COLORYELLOW);
...
void ITEM_GetMultiItem( int charaindex, int toindex, int haveitemindex)
{
  ...
  int i;
	...
	itemarg = ITEM_getChar( itemindex, ITEM_ARGUMENT);
	if( itemarg == "" ){
		CHAR_talkToCli(charaindex, -1, "這是個空盒子!", CHAR_COLORYELLOW);
		return;
	}

	if( getStringFromIndexWithDelim( itemarg, "|", 1, buf, sizeof( buf)) == FALSE ){
		CHAR_talkToCli(charaindex, -1, "不明東西!", CHAR_COLORYELLOW);
		return;
	}
  itemnum = atoi(buf);
	if( itemnum > 10 ){
	...
	if( (itemnum - 1) <= CHAR_findSurplusItemBox( charaindex )){
		...
		sprintf( token, "盒子裏有%d件物品，你的物品欄位不足。", itemnum);
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
		return;
	}
	...
	if( battlemode == BATTLE_CHARMODE_INIT ){
		...
		if( battlemode ){
				...
				if( CHAR_getWorkInt( charaindex, CHAR_WORKFLG ) & WORKFLG_SUPERMICMODE ){
					...
					CHAR_talkToCli( charaindex, -1, "將超級麥剋風設定為OFF。", CHAR_COLORWHITE);
				}else{
					CHAR_setWorkInt( charaindex, CHAR_WORKFLG,
						CHAR_getWorkInt( charaindex, CHAR_WORKFLG ) | WORKFLG_SUPERMICMODE );
					CHAR_talkToCli( charaindex, -1, "將超級麥剋風設定為ON。", CHAR_COLORWHITE);
				}
		}
}
#endif
...
void ITEM_ItemPetLocked( int charaindex, int toindex, int haveitemindex)
{
	if(CHAR_getInt(charaindex,CHAR_LOCKED)==0){
		...
		CHAR_talkToCli( charaindex, -1, "安全鎖已經永久解鎖！", CHAR_COLORYELLOW);
	}else if(CHAR_getInt(charaindex,CHAR_LOCKED)==-1){
		CHAR_setInt(charaindex,CHAR_LOCKED, 1);
		CHAR_talkToCli( charaindex, -1, "安全鎖已經永久鎖定！", CHAR_COLORYELLOW);
	}else if(CHAR_getInt(charaindex,CHAR_LOCKED)==1){
		char message[256];
		char	buf[256];
...
void ITEM_OldToNew( int charaindex, int toindex, int haveitemindex)
{
	if(CHAR_getInt(charaindex, CHAR_AMPOINT)>0){
		...
		if(osfd == -1){
			OtherSaacConnect();
			CHAR_talkToCli( charaindex, -1, "點捲服務器未正常連接!", CHAR_COLORRED );
			return;
		}else{
...
#endif
		CHAR_talkToCli( charaindex, -1, "正在轉換，請稍後", CHAR_COLORYELLOW );
	...
	CHAR_setInt(charaindex, CHAR_AMPOINT, 0);
		...
		CHAR_talkToCli( charaindex, -1, "你身上都沒積分點，無需轉換", CHAR_COLORYELLOW );
	}
...
void ITEM_BoundTime( int charaindex, int toindex, int haveitemindex)
{
	...
	sprintf( token, "你的特殊權限有效使用期至 %d年%d月%d日 %d:%d:%d\n",
																									tm1.tm_year + 1900,
																									tm1.tm_mon + 1,
																									tm1.tm_mday,
																									tm1.tm_hour,
...
void ITEM_TalkSize( int charaindex, int toindex, int haveitemindex)
{
	...
	sprintf( token, "您的字號設為%d！", atoi(itemarg));
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
...
void ITEM_SuperManItem( int charaindex, int toindex, int haveitemindex)
{
	...
	char *itemarg = ITEM_getChar( itemindex, ITEM_ARGUMENT);
	char token[256];
	float table[]={437,490,521,550,578,620,700}; //各轉最高點數(減10)
	int trans, lv, point;
  ...
  CHAR_setInt(charaindex, CHAR_SUPER, 1);
	...
	sprintf(token, "恭喜你！新的極品人誕生咯~你將獲得%d點的牛人！", point);
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
	CHAR_DelItem( charaindex, haveitemindex);
}
#endif
...
#endif
		sprintf( token, "獲得重迴幣數%d，正讀取您當前重迴幣數據...", atoi(itemarg));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
...
void ITEM_EXP( int charaindex, int toindex, int haveitemindex)
{
  ...
  if( !CHAR_CHECKINDEX( toindex ) ) return;
	...
	sprintf( token, "%s 增加 %d 經驗", CHAR_getChar( toindex, CHAR_NAME), atoi(exp));
	CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
...
void ITEM_NullCheck( int charaindex, int toindex, int haveitemindex)
{
	...
	if(strlen(itemarg)>0){
		...
		sprintf( token, "獲得積分點數%d,目前你擁有積分點數為%d!", atoi(itemarg), CHAR_getInt( charaindex , CHAR_AMPOINT));
		CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW );
		...
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT, 
						...
						-1,
						makeEscapeString( "請輸入你要填寫的積分支票金額！(請輸入數字，禁止輸入字符！)\n", buf, sizeof(buf)));
	}
}
#endif
...
void ITEM_WarpItem( int charaindex, int toindex, int haveitemindex )
{
	...
	if( CHAR_CheckInItemForWares( charaindex, 0) == FALSE ){
		...
		if( --usenum <= 0 )	{
			CHAR_talkToCli(charaindex, -1, "該道具已經沒有使用次數瞭，永久消失！", CHAR_COLORYELLOW);
			CHAR_DelItem( charaindex, haveitemindex);
		}else{
			sprintf( buf, "該道具使用次數剩餘%d次", usenum);
			CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORYELLOW);
		}
		ITEM_setInt( itemindex, ITEM_DAMAGEBREAK, usenum);
...
void ITEM_SpecialSuitEquip( int charaindex, int itemindex)
{
	...
	getStringFromIndexWithDelim( showstr, "|", 1, escapeshowstring, sizeof( escapeshowstring));
	sprintf( buf, "此裝備坐標記錄點位於(%s,%d,%d)", escapeshowstring, fx, fy);
	CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORYELLOW);
...
void ITEM_FindTreasures( int charaindex, int toindex, int haveitemindex )
{
	...
	if( sscanf( arg, "%d %d %d", &ff, &fx, &fy) != 3 ){
		...
		sprintf( buf, "尋寶坐標位於(%s,%d,%d)", escapeshowstring, Mx, My);
		ITEM_setChar( itemindex, ITEM_EFFECTSTRING, buf);
		CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORYELLOW);
		CHAR_sendItemDataOne( charaindex, haveitemindex);
	}else{
		if(CHAR_getInt( charaindex, CHAR_FLOOR ) != ff
			...
			CHAR_talkToCli(charaindex, -1, "尋寶坐標不對！", CHAR_COLORYELLOW);
			return;
		}
				...
				if( emptyitemindexinchara >= 0 ){
					if( itemindex != -1 ){
					  ...
					  CHAR_sendItemDataOne( charaindex, emptyitemindexinchara);
						...
						sprintf( buf, "恭喜你，尋找到寶物品瞭 %s", ITEM_getChar( itemindex, ITEM_NAME));
						CHAR_talkToCli( charaindex, -1,buf, CHAR_COLORGREEN);
					}
				}else{
					CHAR_talkToCli( charaindex, -1, "很抱歉，由於你身上物品已滿！",  CHAR_COLORYELLOW);
					return;
				}
			}
		}
...
void ITEM_PoolItem( int charaindex, int toindex, int haveitemindex )
{
	...
	if(CHAR_getInt(charaindex, CHAR_AMPOINT)<atoi(arg)){
		CHAR_talkToCli( charaindex, -1, "很抱歉，由於你身上不足重迴幣使用該物品！",  CHAR_COLORRED);
		return;
	}
...
void ITEM_ShowPetAbl( int charaindex, int toindex, int haveitemindex )
{
  if( !CHAR_CHECKINDEX( toindex ) ) return;
  if( CHAR_getInt( toindex, CHAR_WHICHTYPE) != CHAR_TYPEPET){
   	 CHAR_talkToCli( charaindex, -1, "請選擇你的寵物吧!", CHAR_COLORRED);
      return;
  }
 	int work[4];
	int LevelUpPoint;
	char token[128];
	char buf[][32]={"腕力成長率","耐久力成長率","速度成長率","體力成長率"};
	...
	sprintf(token, "%s 各項成長率如下:", CHAR_getChar( toindex, CHAR_NAME));
  ...
  if( !ITEM_CHECKINDEX( itemindex ) ) return;
	...
	if(getStringFromIndexWithDelim( arg, "|", 1, token,  sizeof( token))==TRUE){
		if( CHAR_getInt( toindex, CHAR_PETID) != atoi(token) ){
			CHAR_talkToCli( charaindex, -1, "您選擇的寵物不對!",  CHAR_COLORRED);
			return;
		}
	}else{
		return;
	...
	if(getStringFromIndexWithDelim( arg, "|", 2, token,  sizeof( token))==TRUE){
		...
		if(NPC_EventCheckFlg(toindex, flg)==TRUE){
			CHAR_talkToCli( charaindex, -1, "該寵物已經無法再次使用該物品瞭!",  CHAR_COLORRED);
			return;
		}
	}else{
		return;
	...
	sprintf(token, "%s 各項成長率變化如下:", CHAR_getChar( toindex, CHAR_NAME));
	CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORGREEN);
	for(i=0;i<4;i++){
		work[i] += abi[i];
		if(abi[i]>0){
...
void ITEM_OlympicTorck( int charaindex, int toindex, int haveitemindex )
{
  ...
  if( !ITEM_CHECKINDEX( itemindex ) ) return;
	if(strlen(ITEM_getChar(itemindex, ITEM_FORUSERNAME)) == 0 || strlen(ITEM_getChar(itemindex, ITEM_FORUSERCDKEY)) == 0){
		for(i=0;i<3;i++){
			...
			if(CHAR_CHECKINDEX(findindex) == TRUE){
				if(findindex!=charaindex){
					...
					sprintf(token, "尋找到閤適的接棒人 %s,當前所在地圖號:%d 坐標X:%d 坐標Y:%d", CHAR_getChar(findindex, CHAR_NAME)
		...
		CHAR_talkToCli( charaindex, -1, "很抱歉,找不到閤適的接棒人!",  CHAR_COLORYELLOW);
	}else{
		int playernum = CHAR_getPlayerMaxNum();
		for(findindex=0;findindex<playernum;findindex++){
			if(CHAR_CHECKINDEX(findindex) == TRUE){
				if(strcmp(CHAR_getChar(findindex, CHAR_CDKEY), ITEM_getChar(itemindex, ITEM_FORUSERCDKEY))==0){
					sprintf(token, "接棒人 %s,現位於地圖號:%d 坐標X:%d 坐標Y:%d", CHAR_getChar(findindex, CHAR_NAME)
																																	,CHAR_getInt(findindex, CHAR_FLOOR)
																																	,CHAR_getInt(findindex, CHAR_X)
																																	,CHAR_getInt(findindex, CHAR_Y));
					CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORYELLOW);
...
void ITEM_PlayerDiyMapObj( int charaindex, int toindex, int haveitemindex )
{
  ...
  if( !ITEM_CHECKINDEX( itemindex ) ) return;
	...
	if(strlen(arg)==0){
		...
		sprintf( buf, "已裝載圖片號為 %d 製作地圖首具", obj);
		ITEM_setChar( itemindex, ITEM_EFFECTSTRING, buf);
		...
		if(MAP_savePlayerMap( charaindex, ff, fx, fy, -1, atoi(arg) )==TRUE){
			...
			sprintf( buf, "成功將地圖:%d,坐標X:%d,坐標Y:%d,對象%d換成%d", ff, fx, fy, obj, atoi(arg));
...
void ITEM_PlayerDiyMapTile( int charaindex, int toindex, int haveitemindex )
{
  ...
  if( !ITEM_CHECKINDEX( itemindex ) ) return;
	...
	if(strlen(arg)==0){
		...
		sprintf( buf, "已裝載圖片號為 %d 製作地圖首具", tile);
		ITEM_setChar( itemindex, ITEM_EFFECTSTRING, buf);
		...
		if(MAP_savePlayerMap( charaindex, ff, fx, fy, atoi(arg), -1 )==TRUE){
			...
			sprintf( buf, "成功將地圖:%d,坐標X:%d,坐標Y:%d,圖片%d換成%d", ff, fx, fy, tile, atoi(arg));
...
void ITEM_ShowItem( int charaindex, int toindex, int haveitemindex )
{
	...
	for( object = MAP_getTopObj( CHAR_getInt( charaindex, CHAR_FLOOR),x,y); object; object = NEXT_OBJECT(object ) ){
		...
		if( CHAR_getInt( showtoindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
			...
			char *arg = ITEM_getChar(itemindex, ITEM_ARGUMENT );
			if(atoi(arg)==0){
				char show[CHAR_EQUIPPLACENUM][32]={"頭盔", "盔甲", "左武", "左飾", "右飾", "腰帶", "右武", "鞋子", "手套"};
				for(i=0; i<CHAR_EQUIPPLACENUM;i++){
				...
				char show[CHAR_EQUIPPLACENUM][32]={"頭", "牙", "爪", "胸", "背", "翅", "腳"};
	...
	CHAR_talkToCli( charaindex, -1, "前方沒有玩傢",  CHAR_COLORRED);
}
#endif
#ifdef _ITEM_LUA
typedef enum
