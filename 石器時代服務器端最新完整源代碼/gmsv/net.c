...
FuncKk TimerLua[50];
void SetTimerLua(void)//初始化LUA定時器
{
	int i;
...
typedef struct tagCONNECT
{
  ...
  struct timeval lastprocesstime;    /* 卞皿夫玄戊夥毛質A仄凶凜棉*/

  struct timeval lastreadtime;       /* 卞read仄凶凜棉z曉午反切互丹*/
...
#ifdef _ITEM_ADDEXP //vincent 經驗提N
  int EDTime;
#endif
  //    unsigned int      BEO;
  int BEOspool;
...
SINGLETHREAD int lsrpcClientWriteFunc( int fd , char* buf , int size )
{
  ...
  if ( ( r < 0 ) && ( fd != acfd ) ) {
    ...
    Connect[ fd ].appendwb_overflow_flag = 1;
		...
		char token[128];
		sprintf(token,"appendWB錯誤  r=%d    Connect[ fd ].wbuse + size=%d",r,Connect[ fd ].wbuse + size);
...
SINGLETHREAD BOOL initConnect( int size )
{
  ...
  for ( i = 0 ; i < size ; i ++ ) {
	...
	print( "預約 %d 接連...分配 %.2f MB 空間...", size, (sizeof( CONNECT ) * size + RBSIZE * size + WBSIZE * size) / 1024.0 / 1024.0 );
...
//		print("\n關閉請求設置為 FILE:%s,LINE:%d ", file, line);
    CONNECT_LOCK(fd);
...
/*------------------------------------------------------------
 * fdに充り碰てられた儡魯がアカウントサ〖バだと、BOOLを手す
 * 嘿いエラ〖チェックはしない。
 * 苞眶
 *  fd  int     ファイルディスクリプタ
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * 鏈鎊の儡魯を磊って、アカウントサ〖バにデ〖タを瘦賂しようとする。
 * 苞眶、手り猛
 *  なし
 ------------------------------------------------------------*/
...
void CONNECT_SysEvent_Loop( void)
{
	...
	if( checkT != NowTimes && (checkT+10) <= NowTimes )	{
		...
		if(getLuckStarTime()>0){
			if ( chikulatime % getLuckStarTime() == 0 ) { // 每小時
				LuckStar();
			}
		}
#endif
			...
			if( !CHAR_CHECKINDEX(charaindex) )continue;
				...
				if(CHAR_getWorkInt(charaindex,CHAR_WORKSTREETVENDOR) == 3){
					if(CHAR_getWorkInt(charaindex,CHAR_WORK_STREET_VENDOR_TIME) < time( NULL) - 90){
						...
						CHAR_talkToCli(charaindex,-1,"查看擺攤時間過長,賣傢自動取消你的查看",CHAR_COLORYELLOW);
					}
				}
#endif
              ...
              CHAR_setInt( charaindex, CHAR_BECOMEPIG, CHAR_getInt( charaindex, CHAR_BECOMEPIG ) - 10 );
...
#endif
				{
						...
						if(CHAR_getInt(charaindex,CHAR_BOUNDTIME) <= NowTimes ) {
							...
							CHAR_talkToCli( charaindex, -1, "由於你的特權時間已到，所以把您送迴記錄點！", CHAR_COLORYELLOW );
						}
#endif
					}
        }
        ...
        if ( chikulatime % 30 == 0 ) { // 每300秒
          ...
          if ( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ) {
          	...
          	if ( CHAR_getWorkInt(charaindex, CHAR_WORKITEM_ADDEXP ) > 0 ){
	            ...
	            if ( exptime <= 0 ) {
	              ...
	              if ( ( exptime % ( 60 * 60 ) ) < 300 && exptime >= ( 60 * 60 ) ) {
	                char msg[ 1024 ];
									sprintf( msg, "您的提升學習經驗的能力剩大約 %d 小時。", (int)(exptime/(60*60)) );
          	...
          	int i;
			          ...
			          if ( exptime <= 0 ) {
			            ...
			            if ( ( exptime % ( 60 * 60 ) ) < 300 && exptime >= ( 60 * 60 ) ) {
			              char msg[ 1024 ];
										sprintf( msg, "寵物%s提升學習經驗的能力剩大約 %d 小時。", CHAR_getChar( petindex, CHAR_NAME), (int)(exptime/(60*60)) );
							      CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW );
			            }
			          }
		          }
          ...
          if ( CHAR_getWorkInt( charaindex, CHAR_WORKITEMMETAMO ) < NowTime.tv_sec
            ...
            CHAR_setWorkInt( charaindex, CHAR_WORKNPCMETAMO, 0 ); //與npc對話後的變身也要變迴來
            CHAR_complianceParameter( charaindex );
            CHAR_sendCToArroundCharacter( CHAR_getWorkInt( charaindex , CHAR_WORKOBJINDEX ) );
            CHAR_send_P_StatusString( charaindex , CHAR_P_STRING_BASEBASEIMAGENUMBER );
        ...
        if ( CHAR_getWorkInt( charaindex, CHAR_WORK_MAP_TIME ) > 0
          ...
          if ( CHAR_getWorkInt( charaindex, CHAR_WORK_MAP_TIME ) <= 0 ) {
            ...
            char szMsg[ 64 ];
            sprintf( szMsg, "在這高熱的環境下你隻能再待 %d 秒。", CHAR_getWorkInt( charaindex, CHAR_WORK_MAP_TIME ) );
...
/*
	...
	*/
							...
							sprintf( buff1, "上傳:[%2.3f K]平均[%2.3f K]"
															"下載:[%2.3f K]平均[%2.3f K]"
															"消時%2.3f秒\n"
...
*/
          ...
          // Syu ADD 每小時重新更新英雄戰廠排行榜資料
          if ( h_counter > 60*60 ){//36000 約 3600秒=60分鍾
          h_counter=0;
          ...
          */
          if ( i_timeNu != time( NULL ) )  // 每整秒執行一次
          {
            ...
            //if( total_count % 60*10 == 0 ) { //每10分鍾執行
            //}

            //if( total_count % 60*60 == 0 ) { //每60分鍾執行
            //}
...
void sigusr2(int i)
{
  ...
  lssproto_Shutdown_recv( 0, "hogehoge", 5 ); // 5分鍾後維修
}
...
*/
  ...
  //Connect[fd].BDTime = (NowTime+20)+delayTime;
	...
	if(getCharloops()>0)
  	Connect[fd].BDTime = NowTime + rand() % getCharloops() ; // 惡寶等待時間
  else
...
#ifdef _NETLOG_
		char log[512];
		sprintf(log,"saveforsaac 寫入連接錯誤:%d %s",errno, strerror( errno ));
		LogCharOut("","",__FILE__,__FUNCTION__,__LINE__,log);
#endif
      	CONNECT_endOne_debug(acfd );
      }
...
void quick(int a[][2],int i,int j)
{
	...
	k=a[(i+j)/2][0]; /*選取的參照*/
	do{
		while(a[m][0]<k&&m<j) m++; /* 從左到右找比k大的元素*/
		while(a[n][0]>k&&n>i) n--; /* 從右到左找比k小的元素*/
		if(m<=n) { /*若找到且滿足條件，則交換*/
			temp1=a[m][0];
			temp2=a[m][1];
			a[m][0]=a[n][0];
...
void DelList(Node **pHead)//刪除頭部鏈錶
{
	Node *pTmp;
	Node *pTmp1;
	pTmp = *pHead;
...
void insertSendPlayDataLastList(Node **pNode,int fd)
{
	if(*pNode==NULL){
		...
		*pNode = pInsert;
		...
		pHead->next = pInsert;   //將鏈錶末尾節點的下一結點指嚮新添加的節點
		*pNode = pTmp;
	}
}
...
CONNECT_RecvLOCK(sockfd);
		...
		if( appret == -1 && getErrUserDownFlg() == 1){
			...
			CONNECT_getCharname(sockfd,charname,32);
			CONNECT_getCdkey(sockfd,cdkey,16);
			LogCharOut(charname,cdkey,__FILE__,__FUNCTION__,__LINE__,"玩傢使用非法手段");
			CONNECT_endOne_debug(sockfd );
	...
	while(TRUE)
	{
		...
		if(nfds>-1){
			...
			for (n = 0; n < nfds; ++n){
				...
				if (eventsEpoll[n].data.fd == bindedfd){//接收到新連接
				...
				}else if(eventsEpoll[n].events&EPOLLOUT){//發送消息
