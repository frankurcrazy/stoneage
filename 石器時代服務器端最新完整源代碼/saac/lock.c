...
#ifdef _LOCK_ADD_NAME
		log("進入遊戲:目錄:char/0x%x 賬號:%s 名稱:%s 服務器:%s\n", entry, cdkey, name, server);
#else
		log("進入遊戲:目錄:%x 賬號:%s 服務器:%s\n", entry, cdkey, server);
#endif
...
int DeleteMemLock(int entry, char *cdkey, int *process)
{
	...
	log("刪除內存信息 位置=%x 賬號=%s ..", entry, cdkey);
...
int GetMemLockState(int entry, char *cdkey, char *result)
{
	...
	while (ln!=NULL) {
		if (ln->use != 0) {
			if (strcmp(ln->cdkey, cdkey)==0) {
				sprintf(result, "%s 是在 %s 被鎖的.",cdkey, ln->server);
				return 1;
			}
		}
		ln=ln->next;
	}
	sprintf(result, "%s 沒有被鎖.", cdkey);
	return 0;
}
