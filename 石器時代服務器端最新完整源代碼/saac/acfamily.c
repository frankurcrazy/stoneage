...
struct FMPOINT
{
   ...
   int		fmfl;						// 莊園族長圖層
   int 		village;					// 莊園鄰近之村莊
   int		hadfmindex;					// 占領莊園之傢族 index
   char		hadfmname[CHARNAME_MAX];	// 占領莊園之傢族 name
   int		hadfmpopular;				// 占領莊園之傢族綜閤聲望值
#ifdef _FIX_FMPOINTTIME
   int		fmpointtime;				//占領莊園時間
#endif
};
...
#endif
#ifdef _FM_MODIFY
	 int		gsnum;			// 記錄玩傢是在那一個game server
#endif
}  MEMBERDATA;

struct	FAMILY
{
   int 		fmindex;			// 傢族 index
   char		fmname[CHARNAME_MAX];		// 傢族名稱
   ...
   int		fmnum; 			// 傢族人數，包含申請退齣、加入人數
   int		fmjoinnum; 		// 真正加入傢族人數
   int		fmacceptflag; 		// 是否接收成員 0:不接受 1:接受
   int		fmsetupflag; 		// 0:尚未成立 1:已成立
   char		fmrule[256];		// 傢族主旨
   int		fmsprite; 		// 0:光明精靈 1:黑暗精靈
   int		fmpointindex;		// 傢族據點 index
   int 		fmpopular;		// 傢族綜閤聲望
#ifdef _PERSONAL_FAME	// Arminius: 傢族個人聲望
   int		fmtotalfame;		// 傢族綜閤 + 傢族個人聲望總閤
#endif
#ifdef _NEW_MANOR_LAW
	 int		fmmomentum;			// 傢族氣勢
#endif
   int		fmgold;			// 傢族基金
   int		fmmemberindexmaxnum;	// 傢族目前最大人數
   int		apply_time;		// 傢族申請時間
   int		setup_time;		// 傢族成立時間
   int		predel_time;		// 預計刪除傢族時間

   int		memonowwritenum;	// 目前留言位置
   int 		memonum;		// 留言數量
   int		fmadv;			// 冒險任務聲望
   int		fmfeed;			// 飼育聲望
   int		fmsynthesize;		// 閤成加工聲望
   int		fmdealfood;		// 料理聲望
   int		fmpk;			// ＰＫ聲望
   char		memolist[FAMILY_MEMONUM][FAMILY_MEMOLEN];	// 留言內容
   MEMBERDATA	fmmemberindex[MAX_MEMBERNUM];			// 成員資料
#ifdef _SET_FMPOINT
	int		fmpointtime;			// 申請莊園時間
#endif
#ifdef _FAMILY_TOTEM
	char familytotem[649];
#endif
#ifdef _FAMILY_PAY_POINT
	int fmpaypoint;
#endif
...
// getFMsortedlist
// arg: buf=returned list (format: "index 名次 傢族名 族長名 popular|...")
//      bufsize=sizeof(buf)
...
// 取得傢族各聲望值列錶
// kindflag 1:綜閤 2:冒險 3:飼育 4:閤成 5:料理 6:PK 7:總閤(綜閤+個人)
...
#endif
#ifdef _NEW_MANOR_LAW
	 else if(kindflag == 8){ // 十大氣勢傢族(全部氣勢排名都傳給game server)
		 for(i=0;i<MAX_FAMILY;i++){
...
int writeFamily(char *dir)
{
   ...
   for (i = 0; i < MAX_FAMILY; i++)
   {
      ...
      sprintf(filename, "%s/Family.%d", dir, family[i].fmindex - 1);
      fp = fopen(filename, "w");
      if (fp == NULL)	continue;
...
#ifdef _FIX_FMPOINTTIME
   	      easyGetTokenFromBuf(line, "|", 9, data, sizeof(data));
   	      fmpoint[i].fmpointtime = atoi(data);
#endif
   	      i++;
   	   }
   	   fclose(fp);
      ...
      for (j = 1; j < family[i].fmmemberindexmaxnum; j++){
         ...
         if ((family[i].fmmemberindex[j].predeltime < time)
            ...
            db_familyupdate[i] = 1;
						...
						{
							char buf[256];
							sprintf(buf, "(%s)因太久未上綫而離開您的傢族瞭！目前傢族人數：%4d人",
...
#ifdef _FAMILY_TOTEM
	strcpy(family[index].familytotem, "");
#endif
	...
	for (i = 1; i < MAX_MEMBERNUM; i++){
		if ((strcmp(family[index].fmmemberindex[i].charname, "") == 0)
			...
			if (family[index].fmmemberindex[0].onlineflag > 0){
				char buf[256];
				sprintf(buf, "(%s lv:%d)正要求加入您的傢族喔！目前傢族人數：%4d人",
	...
	char *charname, char* charid)
{
	...
	for (i = 1; i < family[index].fmmemberindexmaxnum; i++)
	{
			...
			&& (strcmp(family[index].fmmemberindex[i].charid, charid) == 0))
   	{
			...
			{
				char buf[256];
				sprintf(buf, "(%s)已經離開您的傢族瞭！目前傢族人數：%4d人",
...
// 修正傢族資料
// kindflag 1:是否繼續招募傢族成員 2:傢族ＰＫ聲望 3:傢族守護獸 4:傢族主旨
//	    5:解散傢族時間 6:傢族基金 7:傢族冒險聲望 8:傢族飼育聲望
//	    9:傢族閤成、加工聲望 10:傢族料理聲望 11:族長禪讓
   	...
   	if (family[index].fmsynthesize > MAXFMPOPNUM)	family[index].fmsynthesize = MAXFMPOPNUM;
		...
		if (strcmp(family[index].fmmemberindex[charindex].charname, data2) != 0
		 ...
		 if (family[index].fmmemberindex[charindex].onlineflag > 0)
		 {
			...
			family[index].fmmemberindex[charindex].momentum = recvdata;
			if(family[index].fmmemberindex[charindex].momentum < 0) family[index].fmmemberindex[charindex].momentum = 0;
			if(family[index].fmmemberindex[charindex].momentum > MAXFMMOMENTUM)	family[index].fmmemberindex[charindex].momentum = MAXFMMOMENTUM;
		}
		else if(kindflag == 13){
		 family[index].fmmemberindex[charindex].popular = recvdata;
			...
			family[index].fmbadge = atoi(data1);
		}
#endif
...
#ifdef _NEW_MANOR_LAW
			 // 傢族氣勢 = (傢族人數平方) / 10 + 1 + 傢族成員個人氣勢加總
			 family[index].fmmomentum += ((family[index].fmjoinnum * family[index].fmjoinnum) / 10 + 1) * 100;
			 if(family[index].fmmomentum>2000000000)family[index].fmmomentum=2000000000;
	...
	char* losefmname, int losefmindex)
{
   ...
   {
#if 1 // Robin 2003/11/27 修正輸方傢族PK聲望到達下限時勝方可洗聲望
		int overpoint;
...
int ACFMDetail(int index, char *fmname, int fmindex, char *data)
{
   ...
   strcpy(data, "");
   sprintf(data, "%d|%d|%s|%s|%d|%s|%s|%d|%d|%s|%d"
#ifdef _FAMILYBADGE_
	"|%d"
#endif
	   ,index, family[index].fmindex,
...
#endif	
{
	...
	if (strcmp(family[index].fmmemberindex[charindex].charname, charname) == 0)
	{
		...
		if (family[index].fmmemberindex[charindex].onlineflag > 0)
		{
			...
			{
#ifdef _FMVER21
				sprintf(buf, "%s已經將你踢齣傢族瞭！", family[index].fmmemberindex[meindex].charname);
#else
				sprintf(buf, "%s族長已經將你踢齣傢族瞭！", family[index].fmname);
#endif
				saacproto_ACFMAnnounce_send(family[index].fmmemberindex[charindex].onlineflag,
					SUCCESSFUL, fmname, fmindex, index, 3, buf,
					family[index].fmmemberindex[charindex].charfdid);
		...
		if (family[index].fmjoinnum < MIN_MEMBERNUM){
			...
			if (family[index].fmmemberindex[charindex].onlineflag > 0 ){
				char token[256];
				sprintf( token, "在七天之內要趕快召集１０名成員，否則傢族會被解散喔！剩馀%d天。",
...
#endif
   	   	// 通知玩傢
   	   	sprintf(buf, "%s族長已經將你的傢族職等改為%s！", family[index].fmname,
...
int ACFMPointList(char *data)
{
   ...
   {
#ifdef _FIX_FMPOINTTIME
      sprintf(msg, "%d|%d|%d|%d|%d|%s|%d|%d|%d|%d ",
#else
...
#ifdef _SET_FMPOINT
{
   for (i = 0; i < MAX_FMPOINT; i++)
   {
	   	if (fmpoint[i].hadfmindex == index + 1)
	   	{
	   		return	-2;
	   	}
   }
...
#ifdef _SET_FMPOINT
	if (family[index].fmpointtime > (int) time(NULL)) return	-6;
#endif
	...
	time_t t1;
	time(&t1);
	fmpoint[fmpointindex].fmpointtime=t1;
#endif
	// 更新綫上傢族成員狀況
	for (i = 0; i < MAX_MEMBERNUM; i++){
...
{
	int i = 0;
	//printf("\n修復莊園信息 %d|%s|%d|%d|%s|%d|%d",winindex, winfmname, winfmindex, loseindex,
	//		losefmname,  losefmindex,  village);
#ifdef _SET_FMPOINT
	if(winindex == -1 || winfmindex == -1  || loseindex == -1  || losefmindex == -1)
	{
		

		family[fmpoint[village -1].hadfmindex - 1].fmpointindex = -1;
		time_t t1;
		time(&t1);
	//	printf("\n莊園占據時間為%d",t1);
		family[fmpoint[village -1].hadfmindex - 1].fmpointtime =  t1 + 60 * 30;

		fmpoint[village -1].hadfmindex = -1;
   	strcpy(fmpoint[village -1].hadfmname, "-1");
   	fmpoint[village -1].hadfmpopular = -1;
	...
	if(fmpoint[village -1].hadfmindex == winfmindex){
		...
		// 獲得金錢 = (傢族氣勢 * 10000) - (傢族成員個人氣勢總閤 * 5000)
		for(i=0;i<MAX_MEMBERNUM;i++){
	...
	if(fmpoint[village -1].hadfmindex != winfmindex){
		...
		time(&t1);
	//	printf("\n莊園占據時間為%d",t1);
		fmpoint[village -1].fmpointtime =  t1;
	}
	//GM指令給莊園
...
#ifdef _FAMILY_PAY_POINT
						case 22:
							family[index].fmpaypoint = atoi(value);
							break;
#endif
#ifdef _FAMILYBADGE_
					case 23:
						family[index].fmbadge = atoi(value);
						break;
#endif
      	    default:
      	    	break;
...
#ifdef _FAMILY_TOTEM
      	    case 6:
      	    	strcpy(family[index].familytotem, value);
      	    	break;
#endif
      	    default:
      	    	break;
...
#ifdef _FAMILY_TOTEM
               case 6:
                  strncpy(family[index].familytotem, data,sizeof(family[index].familytotem));
                  log("familytotem:%s\n", data);
                  break;
#endif
               default:
...
void addFmPayPoint(int fmindex, char *fmname, int paypoint)
{
	...
	printf("傢族[%s]充值積分為：%d\n", family[index].fmname, family[index].fmpaypoint);
}
#endif
