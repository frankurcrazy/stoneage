...
/*
		...
		仇及白央奶夥匹爛聒今木化中月楮醒反兮氏少  據毛勾井丹［
		
			
				char *c0 : ID
				char *c1 : Passwd  手丹  據允氏匹月井日銀歹卅中
				char *c2 : Charname
				char *c3 : opt
...
#ifdef _WAEI_KICK
		saacproto_ACKick_recv( ti, id, 10, -1);	//踢其他星係
#endif
}
											 ...
											 char *c2 , char *c3 , char *c4 , int i0 , int i1,int charlistflg )
{
	...
	if( auth != 0 ){
		char data[100];
		snprintf( data, sizeof( data ) , "%d" , auth );
		saacproto_ACCharList_send( ti , FAILED , "你的賬號暫時無法登陸！" , mesgid );
		total_ng_charlist++;
		return;
	}
	/*
	...
	if(charlistflg == 0){
    if( isLocked( id ) ){
			saacproto_ACCharList_send( ti , FAILED , "《 登陸遊戲失敗，請嘗試重新登陸 》" , mesgid );
  ...
  */
	// 取消下列 unlock 動作
    if( isLocked( id ) ){
			saacproto_ACCharList_send( ti , FAILED , "你的賬號處理鎖定狀態，請重新登陸，如果三次都不能進入，請聯係管理員為您解鎖！" , mesgid );
			saacproto_ACKick_recv( ti, id, 1, -1);
			checkGSUCheck( id );
			total_ng_charlist++;
			return;
		...
		if (index != -1)
		{
			if (fmcharindex == 0)
			{
				...
				if (ChangeFMLeader(index, fmname, fmindex) >= 0)
				{
					if (ACDelFM(index, fmname, fmindex) >= 0)
						log("刪除傢族成員:%d 傢族名:%s 傢族索引:%d 人物:%s, 賬號:%s, 傢族人物索引:%d\n",
			...
			else 
			{
				...
				if (ACMemberLeaveFM(index, fmname, fmindex, charname, 0, 0, fmcharindex) >= 0)
					log("ACMemberLeaveFM index:%d 傢族名:%s 傢族索引:%d 人物:%s, 賬號:%s, 傢族人物索引:%d\n",
					index, fmname, fmindex, charname, id, fmcharindex);
#else
...
static void getCharOptionFromString( char *str , char *out )
{
	...
	for(c=0;;c++){
		...
		if( IS_2BYTEWORD( str[c] ) ){
			c++;
			if( str[c] == '\0' )break;	// 公及  儂互允匹卞NULL卅日蔽
			continue;
		}
		...
		if( str[c]== SPACE ){
			...
			for(;;c++){
				...
				if( str[c] == '\0' ){	// 蔽  分勻凶日 NULL   木化蔽歹月
					out[outc] = '\0';
					break;	
				}
				
				// 仇仇手ㄡ田奶玄  儂及ㄠ田奶玄  卅日手丹域田奶玄戊疋□允月
				//----   For 2Byte Code ----
				if( IS_2BYTEWORD( str[c] ) ){
...
/*
  ...
  平乓仿及丐中化中月旦夫永玄毛腹綢允月
		  ...
		  卅日丐中化中卅中［0動曉卅日丐五旦夫永玄及index.
		
			
				簽卞夫□玉匹五卅井勻凶日公木毛井尹允分仃［
				手勻午切紮氏午匹五月支欠仃升手［
...
#endif
{
	...
	if( lock ){
		...
		if( DeleteMemLock( getHash(id) & 0xff, id, &ret) ) {
			...
			snprintf( result , resultlen, FAILED );
			snprintf( retdata , retdatalen, "不能移除鎖定" );
