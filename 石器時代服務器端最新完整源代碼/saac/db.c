...
{
    int use;
//    unsigned int keyhash;       /* 腹綢平□及甩永撲亙戊□玉 */
    int ivalue;                  /* 旦戊失［玄永皿 NODE 反  -1 匹｝
                                 允屯化及旦戊失反 0 動曉匹卅中午中仃卅中*/
//    int nextind;                /* -1 分勻凶日    毛啦  允月 */
    // Spock 2000/10/12
    int prev;	// 前一個dbentry, -1錶示此項為head
    int next;	// 下一個dbentry, -1錶示此項為tail
    char key[KEY_MAX];
    char charvalue[CHARVALUE_MAX];
//    char key[64];               /* 腹綢平□午卅月  儂   */
//    int charvalue_index;        /*   儂  田永白央毛今允index */
    // Spock end
};
...
struct table
{
    ...
    int ent_finder;	// 指嚮最後一次配置的 hashentry
    // Spock end
};

struct dbentry *master_buf;     /* 巨件玄伉筏盛迕 */
int dbsize = 0;                 /*   賡0匹｝1,2,4,8,16...*/
...
/*
  DB及雲雲五今互凶日氏仁卅勻凶日歹曰丐化卅雲允［中引及扔奶術及2  卞允月
  0分勻凶日1卞允月
 */
static int
reallocDB( void )
...
*/
    ...
    */
    ...
    dbent_finder = dbsize;	// 將 dbent_finder 指嚮未使用的 entry
    dbsize = new_dbsize;
    master_buf = newbuf;
    
    log( "重新分配數據: 新數據大小:%d 舊地址: %x 新地址:%x\n",
...
dbAllocNode()
{
    ...
    for(i=0;i<dbsize;i++){
        ...
        if( master_buf[dbent_finder].use == 0 ){
            ...
            /* Spock deleted 2000/10/12
            // int 匹手尥笛樹  及啃卞stringbuffer毛  勾儀卞允月 kawata
...
reallocHash( int dbi )
{
    ...
    log( "重新分配無用信息: 新無用信息大小:%d 舊地址: %x 新地址:%x\n",
...
/*
  伉件弁及玄永皿毛芨尹日木凶日｝襖毛  曰卞用□玉毛腹綢仄化
  贋濠卅午仇欠卞 Insert 允月［切中今中  井日雲雲五中  卞卅日氏匹中月午
  移爛
...
/*
  犯□正矛□旦及  蟆毛  月［db反醒互劑卅中及匹  騷卞strcmp仄化方中
  DBTYPE :潘  ［

  雲卅元卅引尹及  反｝幫醒午  儂  及    卞綉箕允月仇午互匹五月［
    ...
    for(i=0;i<MAXTABLE;i++){
        if( dbt[i].use == 0 ){
            ...
            */
            ...
            /* 玄永皿及用□玉毛賡渝祭允月卅曰［
               儂  及樺寜手幫醒及樺寜手雲卅元匹方中［
             0x7fffffff午中丹襖反｝  儂  及樺寜反今幻升啦  毛手凶卅中及分［*/
...
/*
  ...
  午曰丐尹內 strtol 匹五月襖仄井扔禾□玄仄卅中方

  腹綢仄化葦勾仃凶用□玉卞覆仄化｝
  伉件弁井日  五午勻化井日｝漆蘸反末□玄允月及卞銀丹襖毛凶方曰卞
  腹綢仄化｝中中樺赭毛心勾仃凶日｝公仇卞犒仄仇戈［
...
dbGetEntryRank( char *table, char *key , int *rank_out, int *count_out)
{
    ...
    int cur;
    int now_score = 0x7fffffff;     /*int 匹中切壬氏匹井中襖 */
    int r = -1 , i=0;
...
{
#define MAXHITS 1024        /* 贋癲支卅丐［匹手仇木匹蝸坌日仄中冗 ringo */
    struct hitent{          /* 仇及厭瞻  卞甲永玄仄凶支勾毛凶戶化中仁 */
        int entind;
        int rank;
    };
...
/* 隙爛仄凶匏  井日隙爛仄凶蜊醒潸曰分允［
 撩  仄凶日  ｝嶽  仄凶日0［嶽  仄化手塢及請  及午五互丐月冗［
   “num互0及午五午井｝竟癲允月巨件玄伉互卅中午五［
