...
#if _ATTESTAION_ID == 1
void saacproto_ACServerLogin_recv( int ti, int id, char* servername , char* serverpas )
{
    login_game_server( ti , id, servername , serverpas,
...
{
	if( !is_game_server_login(ti) )return;
    // Spock 2000/10/31
...
/*
   ...
   char *pas : 天允歹□升
   char *charname : 平乓仿
   char *opt : 左皿撲亦件  儂  ［ CharList及午五卞丟乒午仄化忒允樹
   char *charinfo : 白央奶夥卞忡綉允月平乓仿弁正及旦  □正旦樹  ［
   int id : 丟永本□斥id
*/
#ifdef _NewSave
...
void saacproto_ACCharList_recv( int ti,char* id,char* pas ,char* ip,char* mac , int mesgid,int charlistflg)
{
	if( !is_game_server_login(ti) )return;
    static int process=0;
    char buf[10];
    ...
    snprintf(buf, sizeof(buf), "%d", process);
    if( !is_game_server_login( ti ) ){
        saacproto_ACCharList_send( ti , FAILED , "無法登陸遊戲！" , mesgid );
        return;
    }

#ifdef _SASQL
	if (strlen(id)==0 || strlen(pas)==0){
	  saacproto_ACCharList_send( ti , FAILED , "賬號密碼不能為空！" , mesgid );
	  return;
	}
	if (sasql_chehk_lock(id)){
		saacproto_ACCharList_send( ti , FAILED , "你的遊戲賬號被鎖定，無法登陸！" , mesgid );
	  return;
	}
...
#endif
		{
			saacproto_ACCharList_send( ti , FAILED , "您的賬號尚未注冊，請登陸服務器官網進行注冊！" , mesgid );
	  	return;
		}
	}else if (res!=1){
...
#endif
		{
			saacproto_ACCharList_send( ti , FAILED , "您的賬號或密碼錯誤！" , mesgid );
		  return;
		}
	}
	...
	if (strlen(id)==0 || strlen(pas)==0 || strlen(ip)==0){
	  printf("登陸信息有錯誤！\n");
	  saacproto_ACCharList_send( ti , FAILED , "賬號或密碼不能為空！" , mesgid );
	  return;
	}
	if (sasql_chehk_lock(id)){
		printf("該賬號%s禁止登陸!\n",id);
	...
	if (sasql_chehk_lock(ip)){
		printf("該IP%s禁止登陸!\n",ip);
	  saacproto_ACCharList_send( ti , FAILED , "你的遊戲IP已被鎖定，無法登陸！", mesgid );
	  return;
	}
	if(strlen(mac)>0){
...
#endif
{
	...
	r = ACAddFM(fd, &index, fmname, fmleadername, fmleaderid, fmleaderlv,
	   petname, petattr, fmrule, fmsprite, fmleadergrano, fame,
#ifdef _FAMILYBADGE_
		fmbadge,
#endif
	   charfdid);
...
void saacproto_ACShowMemberList_recv(int fd, int index)
{
	...
	if (r < 0){
		saacproto_ACShowMemberList_send(fd, FAILED, index, r,
			fmacceptflag, fmjoinnum, "Nothing",badge);
	}else{
		saacproto_ACShowMemberList_send(fd, SUCCESSFUL, index, r,
			fmacceptflag, fmjoinnum, data,badge);
	}
#endif
}
...
void saacproto_NewVipShop_recv( int fd, int clifd, char *id, int point, char *buf, int flag)
{
...
void saacproto_ItemPetLocked_recv( int fd, int clifd, char *id, char *safepasswd )
{
	...
	if(strcmp(data, "安全鎖已經成功解鎖！")==0){
		saacproto_ItemPetLocked_send(fd, clifd, 1, data);
	}else if(strcmp(data, "您還未設置安全鎖解鎖密碼，為瞭確保安全，請輸入一次六位以上密碼做為安全鎖密碼並勞勞記住！")==0){
		saacproto_ItemPetLocked_send(fd, clifd, 0, data);
	}else{
		saacproto_ItemPetLocked_send(fd, clifd, -1, data);
	}
}
void saacproto_ItemPetLockedPasswd_recv( int fd, int clifd, char *id, char *safepasswd )
{
...
#ifdef _ONLINE_COST
void saacproto_OnlineCost_recv( int fd, int clifd, char *id, char *costpasswd, int fmindex, char *fmname )
{
...
void saacproto_OldToNew_recv( int fd, int clifd, char *id, int point )
{
	int vippoint = sasql_add_vippoint( id, point );
	char data[256];
	sprintf(data, "舊會員點%d已轉換至SQL裏，你當前會員點共%d",point, vippoint);
...
void saacproto_FormulateAutoPk_recv( int fd, int clifd, char *id, int point )
{
	...
	char data[256];
	if(pkpoint == -1){
		sprintf(data, "您的亂舞積分%d無法轉換進SQL裏，請與管理員聯係！", point);
	}else{
		sprintf(data, "已將您的亂舞積分%d已轉換至SQL裏，你當前亂舞積分共%d", point, pkpoint);
	}
...
void saacproto_CharadataSaveSQL_recv( int fd, int clifd, char *id, Charadata charadata, int saveindex, int flag )
{
	sasql_charadata_Save(id, "list", charadata.CHAR_list_String, saveindex, DELETE);
	...
	sasql_charadata_Save(id, "list_count", charadata.CHAR_list_count_String, saveindex, DELETE);
	sasql_charadata_Save(id, "list_info3", charadata.CHAR_list_info3_String, saveindex, DELETE);
	...
	sasql_charadata_Save(id, "list_attackmagic", charadata.CHAR_list_attackmagic_String, saveindex, DELETE);
	...
	sasql_charadata_Save(id, "list_skill", charadata.CHAR_list_skill_String, saveindex, DELETE);
	sasql_charadata_Save(id, "list_item", charadata.CHAR_list_item_String, saveindex, DELETE);
	sasql_charadata_Save(id, "list_title", charadata.CHAR_list_title_String, saveindex, DELETE);
	sasql_charadata_Save(id, "list_mail", charadata.CHAR_list_mail_String, saveindex, DELETE);
}
#endif
