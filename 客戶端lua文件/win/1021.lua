...
function ServerWindowType()
	...
	sa.StockFontBuffer(winX + 20,winY + 40, 1, 10, "“莊    園”        “守莊傢族”       “下書傢族”          “族戰時間”  “族戰狀態”", 0)
	for i = 1, 10 do
		if string.len(win.getMessageData(i)) > 0 then
			...
			elseif tonumber(str[1]) == 9 then
				sa.StockFontBuffer(winX + 160 - 6,winY + 40 + i * 20, 1, 2, "無", 0)
				sa.StockFontBuffer(winX + 295 - 6,winY + 40 + i * 20, 1, 2, "無", 0)
				sa.StockFontBuffer(winX + 365,winY + 40 + i * 20, 1, 1, os.date("%m月%d日 %H:%M", tonumber(str[2])), 0)
				sa.StockFontBuffer(winX + 480,winY + 40 + i * 20, 1, 0, string.format("截止休戰期", str[5], str[5]), 0)
			elseif tonumber(str[1]) == 15 then
				sa.StockFontBuffer(winX + 160 - 6,winY + 40 + i * 20, 1, 2, "無", 0)
				sa.StockFontBuffer(winX + 295 - 6,winY + 40 + i * 20, 1, 2, "無", 0)
				sa.StockFontBuffer(winX + 365,winY + 40 + i * 20, 1, 1, os.date("%m月%d日 %H:%M", tonumber(str[2])), 0)
				sa.StockFontBuffer(winX + 480,winY + 40 + i * 20, 1, 0, string.format("截止下書期", str[5], str[5]), 0)
			else
				sa.StockFontBuffer(winX + 124,winY + 40 + i * 20, 1, 7, "   無挑戰排程           無挑戰排程              無挑戰排程      無挑戰排程", 0)
			end
		end
	end
	if btnCnt > 0 then
...
function data()
	...
	winX = 0
	winY = 0
	fmpoint = {"薩姆吉爾", "瑪麗娜絲", "加　　加", "卡魯它那", "伊 甸 園", "塔 爾 塔", "尼 剋 斯", "弗 烈 頓", "亞 依 歐", "瑞爾亞斯"}
end
