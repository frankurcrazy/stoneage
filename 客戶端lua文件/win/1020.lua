...
function ServerWindowType()
	...
	sa.StockFontBuffer(winX + 20,winY + 40, 1, 10, " “莊    園”          “傢族名字”              “占據時間”      “占據時長”", 0)
	for i = 1, 10 do
		if string.len(win.getMessageData(i)) > 0 then
			...
			if fmnamelen > 0 then
				...
				if hadday > 0 then
					local strhadday = string.format("%d天%02d小時", getIntPart(hadday/(3600*24)), getIntPart(hadday%(3600*24)/(3600)))
					local strhaddaylen = string.len(strhadday) - 1
					sa.StockFontBuffer(winX + 442 - strhaddaylen * 4,winY + 40 + i * 20, 1, 0, strhadday, 0)
				end
			else
				sa.StockFontBuffer(winX + 185 - 24,winY + 40 + i * 20, 1, 7, "未占據                          未占據                 未占據", 0)
			end
		end
	end
	if btnCnt > 0 then
