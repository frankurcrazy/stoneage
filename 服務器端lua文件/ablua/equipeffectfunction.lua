function EquipEffectFunction( charaindex, id )
	if id == 2 then
		...
		if itemindex > -1 then
			...
			if itemlv>0 then
				char.setWorkInt(charaindex,"莊園特效",itemshow[itemlv][2])
				return 1
			end
		else
			char.setWorkInt(charaindex,"莊園特效",0)
		end
	end
	if id == 6 then
		itemindex = char.getItemIndex(charaindex,id)
		if itemindex > -1 then
			...
			char.setInt(charaindex,"法寶人物特效",effectid)
		else
			char.setInt(charaindex,"法寶人物特效",0)
		end
	end
	return 1
...
function GetItemShowIndex(itemindex)
	local name = item.getChar(itemindex,"名稱")
