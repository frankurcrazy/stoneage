...
int pattern( ACTION *a0, int anim_spd, int loop_flg)
{
	...
	ATR_CHR_NO(a0) != ATR_CHR_NO_OLD(a0)){
		...
		ATR_CHR_TIM(a0) = 0;//frame counter設為一
		ATR_CHR_CNT(a0) = 0;//這個動畫的第幾張圖
	}
	if(ATR_CHR_TIM(a0)){
		ATR_CHR_TIM(a0)--;	//frame counter減一
		return 0;
	}
	if(ATR_CHR_NO(a0) <= CG_INVISIBLE){
		ATR_PAT_NO(a0) = 0;	//指定為第0張圖
#ifdef _NPC_PICTURE
		picture_play(a0);
#endif
		return 0;
