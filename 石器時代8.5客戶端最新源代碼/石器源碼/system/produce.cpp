...
no = -1;
	...
	case PRODUCE_LEFT_ACCELE:
		ret = ProduceAccele( 2 );
		break;
	case PRODUCE_RIGHT_ACCELE:	// ??﹩?去
		ret = ProduceAccele( 3 );
		break;
	case PRODUCE_LEFT_RIGHT_ACCELE:	// ???﹩?去
		ret = ProduceAccele( 4 );
		break;
	case PRODUCE_UP_DOWM_ACCELE:	// ??﹩?去
		ret = ProduceAccele( 5 );
		break;
	case PRODUCE_UP_BRAKE:		// ?﹩?去
		//ret = ProduceBrake( 0 );
		break;
	case PRODUCE_DOWN_BRAKE:	// ??﹩?去
		ret = ProduceBrake( 1 );
		break;
		
	case PRODUCE_LEFT_BRAKE:	// ??﹩?去
		ret = ProduceBrake( 2 );
		break;
		
	case PRODUCE_RIGHT_BRAKE:	// ??﹩?去
		ret = ProduceBrake( 3 );
		break;
		
	case PRODUCE_LEFT_RIGHT_BRAKE:	// ??﹩?去
		ret = ProduceBrake( 4 );
		break;
		
	case PRODUCE_UP_DOWM_BRAKE:	// // ??﹩?去
		ret = ProduceBrake( 5 );
		break;
	case PRODUCE_4WAY_OUT:		// ???去????
		ret = Produce4Way( 0 );
		break;
	case PRODUCE_4WAY_IN:		// ???去????
		ret = Produce4Way( 1 );
		break;
