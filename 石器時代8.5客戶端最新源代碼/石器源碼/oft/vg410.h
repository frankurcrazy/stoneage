...
#ifdef _SKILL_ROAR  
#define ATR_ROAR(a)		    a->petRoar		        //大吼(剋年獸)
#endif 
#ifdef _SKILL_SELFEXPLODE //自爆
...
#ifdef _SFUMATO
#define ATR_SFUMATO(a)		    a->sfumato	 // 二次渲染圖層色彩		
#endif
...
#define ATT_JUJUTSU			'J'				//鰉
#ifdef __NEW_BATTLE_EFFECT
#define ATT_EFFECT			'j'				//其它的戰鬥特效
#endif
//#ifdef _BATTLESKILL				// (不可開) Syu ADD 戰鬥技能介麵
#define ATT_PROSKILL		'+'
//#endif
...
#endif
#ifdef _PROFESSION_ADDSKILL//結界
#define ATT_BOUNDARY		'a'				
#endif
#ifdef _PETSKILL_BATTLE_MODEL
#define ATT_BATTLE_MODEL 'b'	// 寵物技能戰鬥模組
#endif
...
//andy_mp
#define ATT_MPDAMAGE		'p'				//MP傷害
#define ATT_DAMAGETOHP		'h'
#define ATT_MAGICSTATUS		'm'				//特殊狀態
#ifdef _MAGIC_DEEPPOISION
#define	ATT_DEEPPOISION     'd'             //劇毒
#endif
#define ATT_BOOMERANG		'O'				//?????
#define ATT_SYNCHRONOUS		'Y'				//i???
#define ATT_MALFUNCTION		'M'				//Ol鰉
...
#define ATT_STEAL			'#'				//??
#define ATT_TALK			'%'				//????
#define ATT_TOCALL			'$'				// 召喚

#define ATT_DEATH			(1 << 0)		//死亡
#define ATT_NORMAL			(1 << 1)		//普通攻擊
#define ATT_SATISFACTORY	(1 << 2)		//會心一擊
#define ATT_GUARD			(1 << 3)		//防禦
#define ATT_COUNTER			(1 << 4)		//反擊
#define ATT_DODGE			(1 << 5)		//閃避
...
#define ATT_FALL			(1 << 14)		//落馬
#define ATT_TOOEH			(1 << 15)		//嚙齒
#define ATT_ATTDOUBLE		(1 << 16)		//屬性強化攻擊
#ifdef _SKILL_ROAR  
#define ATT_ATTROAR		    (1 << 17)		//大吼(剋年獸)
#endif 
#ifdef _SKILL_SELFEXPLODE //自爆
#define ATT_ATTSELFEXPLODE	(1 << 18)		//自爆
#endif
#ifdef _PETSKILL_EXPLODE
#define ATT_EXPLODE	(1 << 29)		//爆彈(跟自爆一樣,隻是角色不會消失)
#endif 

#define BCF_DEFMAGICATT		(1 << 19)
#define BCF_SUPERWALL		(1 << 20)
#define BCF_MODIFY			(1 << 21)		//屬性轉換


//#ifdef _BATTLESKILL				// (不可開) Syu ADD 戰鬥技能介麵
#define ATT_ATTPREPROSKILL		(1 << 22)		//職業技能
#define ATT_ATTNEXTPROSKILL		(1 << 23)		//職業技能
#define ATT_TRAP				(1 << 25)		//陷阱
#define ATT_NOMISS				(1 << 26)		//暴擊第一下擊空
//#endif

#ifdef _EQUIT_ARRANGE
#define ATT_ARRANGE				(1 << 24)	//格擋秀圖#endif
#endif
...
#ifdef _MAGIC_WEAKEN
#define BC_WEAKEN           (1 << 11)      //虛弱
#endif
#ifdef _MAGIC_DEEPPOISION
#define BC_DEEPPOISON       (1 << 12)      //劇毒   
#endif
...
#endif
#ifdef _PETSKILL_LER
#define BC_CHANGE			(1<<30) // 雷爾變身
#endif
#ifdef _CHAR_PROFESSION					   // WON ADD
#define	BC_DIZZY			(1 << 16)	   // 暈眩 
#define BC_ENTWINE			(1 << 17)	   // 樹根纏繞
#define BC_DRAGNET			(1 << 18)	   // 天羅地網
#define BC_ICECRACK			(1 << 19)	   // 冰爆術	
#define BC_OBLIVION 		(1 << 20)	   // 遺忘
#define BC_ICEARROW         (1 << 21)	   // 冰箭
#define BC_BLOODWORMS		(1 << 22)	   // 嗜血蠱
#define BC_SIGN				(1 << 23)	   // 一針見血
#define BC_CRAZY			(1 << 24)      // 挑撥
#define BC_F_ENCLOSE		(1 << 25)	   // 火附體
#define BC_I_ENCLOSE		(1 << 26)      // 冰附體
#define BC_T_ENCLOSE		(1 << 27)      // 雷附體

#ifdef _PROFESSION_ADDSKILL
#define BC_WATER            (1 << 28)      // 水附體
#define BC_FEAR				(1 << 29)	   // 恐懼
//#define BC_F_I_T_ENCLOSE	(1 << 28)      // 火冰雷附體
#endif
#ifdef _PETSKILL_LER
#define BC_CHANGE			(1<<30) // 雷爾變身
#endif
