...
ITEMSMALL* ItemsMall::GetAllOtherItemsByIndex(int nIndex, int nType)
{
	...
	for (; iter != g_mapItemsMall.end(); iter++)
	{
		if (nType == iter->second.currencyType && 3 == iter->second.itemFlag)  //其它貨品
		{
			if (nNums == nIndex)
			{
				return &(iter->second);
...
int ItemsMall::GetCurrentItemNum(int nCurrencyType, int nType)
{
	...
	else/* if (nType == 2 || nType == 3) */ //裝備貨品或者寵物貨品或者其它貨品
	{
		int nFlag = -1;
		if (nType == 2)
		{
...
//int ItemsMall::GetAllLatestNum() //最新上價貨品數量
//{
//	return m_nAllLatestNum;
//}
//
//int ItemsMall::GetAllItemsNum() //裝備貨品數量
//{
//	return m_nAllItemsNum;
//}
//
//int ItemsMall::GetAllPetsNum() //寵物貨品數量
//{
//	return m_nAllPetsNum;
//}
//
//int ItemsMall::GetAllOthersNum() //其它貨品數量
//{
